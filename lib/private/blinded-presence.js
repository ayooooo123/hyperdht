...
//   NS.MUTABLE_PUT || generichash(encode(mutableSignable{seq,value})), checked
//   before the AEAD is opened. That signature is the only barrier against a
//   forged record: a reader-credential holder can derive the record key
//   (readerSecret, A', period are all in its hands) and re-seal any body, but
//   cannot sign it. The AEAD only hides the body from parties without the
//   credential. Never open a record's body on a path that skipped verifyMutable.
// - Unlinkability holds only against parties that do not know the stable key A.
//   h is a public function of (A, period), so anyone holding A confirms
//   A' = h·A for every period at one scalar multiplication, past and future,
//   as in Tor v3. A is a public key and is not treated as a secret here.
...
function isRecordTombstoneTarget(value) {
  return isFixed(value, 32) && !isAllZero(value)
}
...
function encodeBody({ type, period, tombstoneScope, tombstoneTarget, descriptor }) {
  ...
  if (type === RECORD_TYPE.DESCRIPTOR) {
    ...
    if (tombstoneScope === TOMBSTONE_SCOPE.RECORD && !isRecordTombstoneTarget(tombstoneTarget)) {
      throw PrivateRouteError.INVALID_DESCRIPTOR()
    }
...
function encodePresenceRecord(options) {
  ...
  if (type === RECORD_TYPE.DESCRIPTOR) {
    ...
    if (tombstoneScope === TOMBSTONE_SCOPE.RECORD) {
      if (!isRecordTombstoneTarget(tombstone.target)) {
        throw PrivateRouteError.INVALID_DESCRIPTOR()
      }
  ...
  try {
    ...
    const boundValue = b4a.from(value)
    ...
    return Object.freeze({
      ...
      signMutable(seq, recordValue, keyPair) {
        ...
        if (!b4a.equals(recordValue, boundValue)) throw PrivateRouteError.UNAUTHORIZED()

        const signable = b4a.allocUnsafe(64)
        signable.set(NS.MUTABLE_PUT, 0)
        sodium.crypto_generichash(
          signable.subarray(32),
          c.encode(m.mutableSignable, { seq: frozenRevision, value: boundValue })
        )
...
function resolvePresenceState({ previous, opened }) {
  ...
  // Once a newer signed period is observed, missing storage cannot reopen an older one.
  if (previous != null && isUint64(previous.period) && opened.period < previous.period) {
