...
function resolvePresenceState({ previous, opened }) {
  ...
  if (!isUint32(opened.revision)) throw PrivateRouteError.INVALID_DESCRIPTOR()
  ...
  if (opened.type === RECORD_TYPE.TOMBSTONE) {
    ...
    if (scope === TOMBSTONE_SCOPE.RECORD) {
      ...
      return Object.freeze({ present: null, ignoredTombstone: true })
    }
    throw PrivateRouteError.INVALID_DESCRIPTOR()
  }
  ...
  if (opened.type === RECORD_TYPE.DESCRIPTOR) {
    ...
    if (!isFixed(opened.recordDigest, 32)) throw PrivateRouteError.INVALID_DESCRIPTOR()
    return Object.freeze({
      present: true,
