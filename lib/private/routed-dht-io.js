...
class RoutedDHTIO {
  ...
  request(message) {
    ...
    try {
      ...
      // `budget` goes on the wire; `deadline` never does. It is the same budget resolved in
      // THIS host's monotonic domain, for the endpoint's own timer and referral binding, so it
      // still has to fit a uint64. The exit derives its own absolute the same way from its own
      // clock, which is what KI-15 was about.
