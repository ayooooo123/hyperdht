  ...
  async _collectAsync(records, limit, branch, epoch) {
    ...
    try {
      ...
      for (let count = 0; ; count++) {
        ...
        if (asynchronous) {
          result = await realPromise(result)
          this._assertEpoch(epoch)
        }
        let done
        let value
  ...
  _operationFields(operation) {
    ...
    try {
      return this._withGuard(() => {
        ...
        return { promise: realPromise(promiseValue), cancel }
      })
    } catch (error) {
      if (cancel !== null) {
        try {
