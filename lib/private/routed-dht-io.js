...
const { BRANCH_CLASS, M3_MESSAGE_ID, REPLY_MODE } = require('./protocol')
const { buildSurbBatch, revokeSurbBatch } = require('./surb-path')
const { EXIT_ORIGIN_SERVICE_POLICY } = require('./exit-policy')
const REQUEST_ID_SIZE = 16
const ID_SIZE = 32
const DESTINATION_REF_SIZE = 172
const MAX_CANDIDATES = 20
// Terms of the routed operation budget minted into every request as `operationBudgetMs`, a
// RELATIVE count of milliseconds. It was an absolute instant on this endpoint's monotonic
// clock until KI-15: a monotonic clock is host-local, so the exit compared this endpoint's
// reading against its own and refused every request whose peer had a different boot time. A
// relative budget belongs to no clock domain, so each host converts it into its own.
//
// The magnitude was a single 3000ms constant, which is only defensible where every hop is
// loopback: a routed request crosses the public internet four times in each direction, so the
// bound has to be derived from the path rather than picked. Each term is a distinct cost.
//
// ROUTE_HOPS - links crossed in ONE direction. A lookup branch is guard, middle, exit:
// `createBranch` in `branch-path-authority.js` carries exactly one `middle` and one `exit`,
// and the guard comes from the lease, which `validatePathDiversity` there holds to three
// distinct operators. The exit then makes its own request to a DHT node, which is the fourth
// link. It is a constant because this topology has no variable-length path - and since KI-15
// that constancy is a PRIVACY REQUIREMENT, not merely a topology fact. The minted budget is
// the same 3000ms for every command today, so the field carries no endpoint-distinguishing
// entropy and a malicious exit learns nothing from it; `forHops` is monotone at +500ms per
// hop, so feeding a real path length in would hand that exit a hop-count oracle. A
// variable-length path must therefore pad the budget to a fixed value rather than derive it.
//
// PER_HOP_ONE_WAY_MS - one link in one direction, including that hop's crypto. The
// runner-to-runner mesh in `docs/private-routing-migration.md` measured a single link at p50
// 31.7ms and p95 115.7ms ROUND trip, so one-way p95 is about 58ms. A deadline is a worst case
// and not a median, so this is roughly four times that: it also has to absorb cross-region
// spread, which reached p50 92.1ms on some pairs in the same document, and event-loop
// scheduling on a shared runner, where that document measured a 1,406ms worst case for a
// link-level operation.
//
// EXIT_DHT_REFERRAL_MS - the exit's own DHT work beyond the link already counted. When its
// first DHT reply is a referral, `dht-exit-io.js` caps each referral probe at
// `current + 1_000n`, so one referral round is the exit's bounded extra cost.
const ROUTE_HOPS = 4n
const PER_HOP_ONE_WAY_MS = 250n
const EXIT_DHT_REFERRAL_MS = 1000n
const MAX_RESPONSE_BYTES = 4706
const MAX_UINT64 = 0xffff_ffff_ffff_ffffn
const ALLOWED_OPTIONS = Object.freeze(['authority', 'contexts', 'now', 'randomBytes', 'wallNow'])
...
const REQUEST_FIELDS_REQUIRED = Object.freeze([
  'context',
  'command',
  'to',
  'token',
...
const REQUEST_FIELDS = Object.freeze([...REQUEST_FIELDS_REQUIRED, 'replyMode'])
...
function routeOperationBudgetMs(hops) {
  return PER_HOP_ONE_WAY_MS * hops * 2n + EXIT_DHT_REFERRAL_MS
}

// The exit refuses a budget past its own advertised ceiling: `validateRoutedRequestForExit`
// rejects `operationBudgetMs > policy.timeoutMs`. Since KI-15 that is a comparison of two
// durations, so it needs no agreement between the two hosts' clocks; only agreement on the
// ceiling. A budget the exit will not admit is worse than a short one, because the request is
// refused outright instead of answered late, so the endpoint clamps to that ceiling. Reading it
// from the same policy table the exit enforces is what stops the two numbers drifting apart in
// separate files, which is how they came to be equal by coincidence rather than by construction.
function exitAdmittedBudgetMs(commandId) {
  let ceiling = null
  for (let index = 0; index < EXIT_ORIGIN_SERVICE_POLICY.length; index++) {
    if (EXIT_ORIGIN_SERVICE_POLICY[index].commandId === commandId) {
      ceiling = BigIntConstructor(EXIT_ORIGIN_SERVICE_POLICY[index].timeoutMs)
    }
  }
  if (ceiling === null) throw unsupportedCommand()
  const budget = routeOperationBudgetMs(ROUTE_HOPS)
  return budget < ceiling ? budget : ceiling
}

// Exported so the derivation is auditable rather than only observable: a test can evaluate it
// at hop counts this topology does not use and watch the budget follow the path, and can check
// the value actually minted against the ceiling the exit will admit.
const ROUTE_OPERATION_BUDGET = objectFreeze({
  hops: ROUTE_HOPS,
  perHopOneWayMs: PER_HOP_ONE_WAY_MS,
  exitDhtReferralMs: EXIT_DHT_REFERRAL_MS,
  forHops: routeOperationBudgetMs,
  admitted: exitAdmittedBudgetMs
})
...
function releaseQueryScope(io, scope) {
  if (scope.released) return false
  scope.released = true
  reflectApply(setDelete, io._admissionScopes, [scope])
  revokeLiveRouteQueryAdmission(scope.token)
  return true
}
...
class RoutedDHTIO {
  constructor(options) {
    ...
    if (fields.wallNow !== undefined && typeof fields.wallNow !== 'function') invalid()
    ...
    // `now` bounds operations (monotonic); `wallNow` is compared with the wall-clock
    // capability times that relays advertise, which SURB descriptors are built against.
    this._now = fields.now || DateNow
    this._wallNow = fields.wallNow || DateNow
    this._randomBytes =
      fields.randomBytes || ((buffer) => reflectApply(randombytesBuf, sodium, [buffer]))
    this._destinations = new SetConstructor()
    this._admittedContexts = new WeakMapConstructor()
    this._admissionScopes = new SetConstructor()
    ...
    this._requiredReplyHolds = 0
  }

  // A caller that needs required SURB replies holds the mode for the whole query,
  // retries included, and releases it afterwards; overlapping holds are counted so
  // one caller's release never downgrades another caller's in-flight request. The
  // dht-rpc query path carries no per-request option, which is why this is a hold
  // on the IO rather than a field on the message.
  holdRequiredReplyMode() {
    this._assertNoReentry()
    this._assertUsable()
    this._requiredReplyHolds++
    let released = false
    return () => {
      if (released) return
      released = true
  ...
  admitQuery(primaryContext, commitContext = null) {
    this._assertNoReentry()
    this._assertUsable()
    let scope = null
    try {
      return this._withGuard(() => {
        const primaryPolicy = this._classifyContext(primaryContext)
        const commitPolicy = commitContext === null ? null : this._classifyContext(commitContext)
        if (commitPolicy !== null && commitPolicy.branch !== primaryPolicy.branch) invalid()
        const commandIds = [primaryPolicy.commandId]
        if (commitPolicy !== null && commitPolicy.commandId !== primaryPolicy.commandId) {
          reflectApply(arrayPush, commandIds, [commitPolicy.commandId])
        }
        const token = createLiveRouteQueryAdmission(
          this._authority,
          primaryPolicy.branch,
          commandIds
        )
        scope = { token, released: false }
        reflectApply(setAdd, this._admissionScopes, [scope])
        const context = objectFreeze(objectCreate(null))
        reflectApply(weakMapSet, this._admittedContexts, [
          context,
          { policy: primaryPolicy, scope }
        ])
        let admittedCommit = null
        if (commitPolicy !== null) {
          admittedCommit = objectFreeze(objectCreate(null))
          reflectApply(weakMapSet, this._admittedContexts, [
            admittedCommit,
            { policy: commitPolicy, scope }
          ])
        }
        return objectFreeze({
          context,
          commitContext: admittedCommit,
          release: () => releaseQueryScope(this, scope)
        })
      })
    } catch (error) {
      // The guard can reject after minting when an injected callback caught reentry.
      if (scope !== null) releaseQueryScope(this, scope)
      throw error
    }
  }
  ...
  bootstrap(options) {
    ...
    this._withGuard(() => {
      ...
      admittedScope = this._admittedScope(discovery.context)
      try {
        const authorityOpts = {
          target: discovery.target,
          limit: discovery.limit,
          branch: discovery.policy.branch
        }
        if (admittedScope !== null) {
          if (admittedScope.released)
            throw reflectApply(PrivateRouteError.ERR_REPLAY, PrivateRouteError, [])
          authorityOpts.queryAdmission = admittedScope.token
          authorityOpts.commandId = discovery.policy.commandId
        }
        returned = this._callAuthority('bootstrap', [authorityOpts])
      } finally {
  ...
  closest(options) {
    ...
    return this._withGuard(() => {
      ...
      const admittedScope = this._admittedScope(discovery.context)
      let returned
      try {
        const authorityOpts = {
          target: discovery.target,
        ...
        returned = this._callAuthority('closest', [authorityOpts])
      } finally {
        clear(discovery.target)
      }
      this._assertEpoch(epoch)
  ...
  request(message) {
    ...
    try {
      this._withGuard(() => {
        fields = exactOwnData(message, REQUEST_FIELDS, REQUEST_FIELDS_REQUIRED)
        ...
        // Announce-only policies address announce-branch destinations only.
      ...
      const budget = exitAdmittedBudgetMs(policy.commandId)
      const current = this._clock()
      if (current > MAX_UINT64 - budget) invalid()
      const deadline = current + budget
      ...
      const replyMode =
        this._requiredReplyHolds > 0 ||
        fields.replyMode === REPLY_MODE.SURB_REQUIRED ||
        fields.replyMode === 'SURB_REQUIRED'
          ? REPLY_MODE.SURB_REQUIRED
          : REPLY_MODE.CORRELATED
      v1Request = encodeRoutedRequest({
        requestId,
      ...
      if (replyMode === REPLY_MODE.SURB_REQUIRED) {
        const hops = liveRouteReturnPath(this._authority, policy.branch)
        if (!hops || hops.length < 1) authentication()
        batchId = allocate(REQUEST_ID_SIZE)
        const batchResult = this._callCallback(this._randomBytes, null, [batchId])
        if (batchResult !== undefined || bufferLength(batchId) !== REQUEST_ID_SIZE) invalid()
        // Eight SURBs cover the 3936-byte ceiling.
        surbBatch = buildSurbBatch({
          hops,
          batchId,
          requestId,
          messageId: requestId,
          surbCount: 8,
          now: normalizedNow(this._callCallback(this._wallNow, null, [])),
          randomBytes: (n) => {
            const buf = allocate(n)
            try {
              const result = this._callCallback(this._randomBytes, null, [buf])
              if (result !== undefined || bufferLength(buf) !== n) invalid()
              return buf
            } catch (err) {
              clear(buf)
              throw err
        ...
        encodedRequest = encodeRoutedRequestV2({
          replyMode: REPLY_MODE.SURB_REQUIRED,
          batchId,
          request: v1Request,
          surbDescriptors: surbBatch.descriptors
        })
        clear(v1Request)
        v1Request = null
        clear(batchId)
      ...
      const admittedScope = this._admittedScope(fields.context)
      if (admittedScope !== null && admittedScope.released) {
        throw reflectApply(PrivateRouteError.ERR_REPLAY, PrivateRouteError, [])
      }
      let operation
      try {
        ...
        if (surbBatch !== null) requestOpts.surbBatch = surbBatch
        if (admittedScope !== null) {
          requestOpts.queryAdmission = admittedScope.token
        }
        operation = this._callAuthority('request', [requestOpts])
      } catch (cause) {
        if (cause && cause.code === 'ERR_REPLAY') throw cause
        throw unavailable(cause)
      }
      const operationFields = this._operationFields(operation)
      // A valid returned operation owns the batch, including cancellation cleanup.
      surbBatch = null
      try {
        this._assertEpoch(epoch)
        this._assertUsable()
  ...
  _admittedScope(context) {
    if (context !== null && (typeof context === 'object' || typeof context === 'function')) {
      const admitted = reflectApply(weakMapGet, this._admittedContexts, [context])
      if (admitted !== undefined) {
        if (admitted.scope.released)
          throw reflectApply(PrivateRouteError.ERR_REPLAY, PrivateRouteError, [])
  ...
  _discoveryOptions(options) {
    ...
    return { policy, target, limit, context: fields.context }
  }
  ...
  _deliverResponse(active, response) {
    ...
    try {
      ...
      let value = null
      if (
        active.commandId === M3_MESSAGE_ID.IMMUTABLE_PUT_V1 ||
        active.commandId === M3_MESSAGE_ID.MUTABLE_PUT_V1
      ) {
        value = null
      } else if (active.commandId === M3_MESSAGE_ID.MUTABLE_GET_V1) {
        value =
          bufferLength(validated.reply.encodedResponse) > 0
            ? copy(validated.reply.encodedResponse)
            : null
      } else {
  ...
  _teardown(reason) {
    const scopes = reflectApply(setValues, this._admissionScopes, [])
    while (true) {
      const next = reflectApply(setIteratorNext, scopes, [])
      if (next.done) break
      releaseQueryScope(this, next.value)
    }
...
module.exports = { RoutedDHTIO, ROUTE_OPERATION_BUDGET }
