...
class RoutedDHTIO {
  constructor(options) {
    ...
    this._pendingReplyMode = REPLY_MODE.CORRELATED
  }
  ...
  setReplyMode(mode) {
    ...
    this._assertUsable()
    if (
      mode !== REPLY_MODE.CORRELATED &&
      mode !== REPLY_MODE.SURB_REQUIRED &&
      mode !== 'CORRELATED' &&
      mode !== 'SURB_REQUIRED'
    ) {
      invalid()
    }
    this._pendingReplyMode =
      mode === REPLY_MODE.SURB_REQUIRED || mode === 'SURB_REQUIRED'
  ...
  request(message) {
    ...
    try {
      ...
      const replyMode =
        this._pendingReplyMode === REPLY_MODE.SURB_REQUIRED
          ? REPLY_MODE.SURB_REQUIRED
          : fields.replyMode === REPLY_MODE.SURB_REQUIRED || fields.replyMode === 'SURB_REQUIRED'
            ? REPLY_MODE.SURB_REQUIRED
            : REPLY_MODE.CORRELATED
      let surbBatch = null
