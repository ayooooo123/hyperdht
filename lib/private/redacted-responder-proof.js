...
const { BRANCH_CLASS, M3_MESSAGE_ID, M3_PROTOCOL_VERSION, decodeM3Object, encodeM3Object } = require('./protocol')
const {
  signRedactedResponderProof: signRedactedResponderProofWithIdentitySigner
} = require('./relay-identity-signer')

const REDACTED_RESPONDER_PROOF_SIZE = 378
const MAX_LIVE_VERIFIED_RESPONDER_PROOFS = 16
const MAX_VERIFIED_RESPONDER_PROOF_STATES = 96
...
function createRedactedResponderProofAuthority(options = {}) {
...
function assembleRedactedResponderProof(value, sign) {
  let projection = null
  let body = null
  let input = null
  let signature = null
  ...
  try {
    ...
    signature = sign(body, input, projection.responderIdentity)
    if (
      !fixed(signature, 64) ||
...
function signRedactedResponderProof(value, identitySecretKey) {
  if (!fixed(identitySecretKey, 64)) invalid()
  return assembleRedactedResponderProof(value, (body, input) =>
    cryptoSuite.sign(input, identitySecretKey)
  )
}

function signRedactedResponderProofWithSigner(value, extensionResponderSigner) {
  return assembleRedactedResponderProof(value, (body, input, responderIdentity) =>
    signRedactedResponderProofWithIdentitySigner(
...
function decodeRedactedResponderProof(encoded) {
  let object = null
  try {
    if (!fixed(encoded, REDACTED_RESPONDER_PROOF_SIZE)) invalid()
...
function verifyRedactedResponderProof(verifier, encoded) {
  return verifyProof(verifier, null, encoded, null)
}

function verifyExpectedRedactedResponderProof(verifier, consumer, encoded, expected) {
  return verifyProof(verifier, consumer, encoded, expected)
}

function consumeVerifiedRedactedResponderProof(consumer, capability, expected) {
...
function revokeVerifiedRedactedResponderProof(consumer, capability) {
  const owner = safeObject(consumer) ? CONSUMERS.get(consumer) : null
  const operation = begin(owner)
  try {
    const binding = safeObject(capability) ? CAPABILITIES.get(capability) : null
...
module.exports = {
  ...
  MAX_LIVE_VERIFIED_RESPONDER_PROOFS,
