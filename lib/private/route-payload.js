...
const TEST_ONLY_NONCE_REGISTRY = Symbol('test-only-nonce-registry')
...
const TEST_NONCE_REGISTRIES = new WeakMap()
const MAX_UINT64 = (1n << 64n) - 1n
const MAX_LOGICAL_COUNTER = (1n << 63n) - 1n
const MAX_NONCE_DOMAIN_CLAIMS = 4096
// Gate 3A prototype ceiling only. Claims are deliberately never evicted after
// activation, so exceeding this process-local registry fails closed. Gate 3B
// must move durable nonce-domain ownership and freshness into the generation/
// circuit manager before this mechanism is used for live or mobile routing.
const DEFAULT_NONCE_REGISTRY = {
  claims: new Map(),
  capacity: MAX_NONCE_DOMAIN_CLAIMS
}
...
function clearOperationResult(result) {
  try {
    ...
    if (Array.isArray(result)) {
      for (const value of result) clear(ownDataField(value, 'payload'))
      return
    }
    clear(ownDataField(result, 'payload'))
    clear(ownDataField(result, 'descriptorId'))
    clear(ownDataField(result, 'circuitId'))
  } catch {
    // Teardown continues through hostile adapter results.
  }
}

function ownDataField(target, name) {
...
function contextFields(options) {
  ...
  const registryToken = option(options, TEST_ONLY_NONCE_REGISTRY, false)
  let registry = DEFAULT_NONCE_REGISTRY
  if (registryToken !== undefined) {
    try {
      registry = TEST_NONCE_REGISTRIES.get(registryToken)
    } catch {
      invalid()
    }
    if (!registry) invalid()
...
function createTestNonceRegistry(options) {
  options = optionsObject(options)
  const capacity = option(options, 'capacity')
  if (!Number.isInteger(capacity) || capacity < 1 || capacity > MAX_NONCE_DOMAIN_CLAIMS) invalid()
  const token = Object.freeze(Object.create(null))
  TEST_NONCE_REGISTRIES.set(token, { claims: new Map(), capacity })
  return token
}
...
function nonceDomainClaim(owned) {
  ...
  if (registry.claims.has(claimKey)) invalid()
  if (registry.claims.size >= registry.capacity) throw PrivateRouteError.CIRCUIT_LIMIT()
  const claim = { key: claimKey, registry, state: 'pending' }
  registry.claims.set(claimKey, claim)
  return claim
}
...
function releasePendingClaim(owned) {
  ...
  if (claim.registry.claims.get(claim.key) === claim) claim.registry.claims.delete(claim.key)
  claim.state = 'released'
}

function activateClaim(owned) {
  const claim = owned && owned.claim
  if (!claim || claim.state !== 'pending' || claim.registry.claims.get(claim.key) !== claim) {
    invalid()
  }
  claim.state = 'active'
  return claim
...
function mintCreatedRoutePayloadContext(options) {
  ...
  const owned = { endpointRole: fields.endpointRole, registry: fields.registry, claim: null }
  try {
    for (const name of [
      'descriptorId',
      'circuitId',
...
class RoutePayloadCodec {
  ...
  [ROUTE_PAYLOAD_BINDING]() {
    return this.#mutate(() => this.#binding())
  }
  ...
  #seal(options) {
    ...
    try {
      ...
      ownedKey = copy(state.key)
      this.#assertNoDeferredDestroy()
      ownedNoncePrefix = copy(state.noncePrefix)
      this.#assertNoDeferredDestroy()
  ...
  #open(options, frame) {
    ...
    try {
      ...
      this.#assertNoDeferredDestroy()
      data = associatedData(this.#descriptorId, this.#circuitId, direction, counter)
      const candidate = this.#openCrypto({
        key: ownedKey,
        noncePrefix: ownedNoncePrefix,
        counter,
        associatedData: data,
      ...
      plaintext = candidate
      if (!isBuffer(plaintext, ROUTE_PLAINTEXT_SIZE)) invalid()
