...
// every budget closed.
const IMMUTABLE_GET_BUDGET_CEILING_MS = EXIT_ORIGIN_SERVICE_POLICY.reduce(
  (ceiling, entry) =>
    entry.commandId === M3_MESSAGE_ID.IMMUTABLE_GET_V1 ? BigInt(entry.timeoutMs) : ceiling,
  0n
)
...
function requestProduction(authority, state, options) {
  ...
  try {
    ...
    if (
      ...
      request.operationBudgetMs > IMMUTABLE_GET_BUDGET_CEILING_MS ||
