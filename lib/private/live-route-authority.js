...
function reportOwnedRouteFailure(state, branchClass) {
  ...
  try {
    ...
    return reportRouteManagerBranchLoss(state.routeManager, branchClass)
  } catch {
    return false
  }
}
...
// The endpoint mints `absoluteDeadlineMs` itself and the exit already enforces it, but
// nothing held the endpoint to it: the receive loop below awaits a reply with no bound, so a
// hop that died silently left the operation hanging until branch material expired. The
// deadline already carried on the request is reused rather than a new tunable, and it is
// read in the same clock domain as the admission check in `requestProduction`.
function armOperationDeadline(state, owner, operationState, operation, request, started) {
  const remaining = request.absoluteDeadlineMs - started
  ...
  try {
    operationState.cancelDeadline = scheduleM3RouteTransportTimer(
      ...
      () => {
        ...
        reportOwnedRouteFailure(state, BRANCH_CLASS.LOOKUP)
...
function requestProduction(authority, state, options) {
  ...
  try {
      ...
      try {
        for (const payload of outbound) {
          ...
          try {
            ...
            if (operationState.live) reportOwnedRouteFailure(state, BRANCH_CLASS.LOOKUP)
            throw err
          } finally {
    ...
    try {
      armOperationDeadline(state, owner, operationState, operation, ownedRequest, started)
    } catch (err) {
