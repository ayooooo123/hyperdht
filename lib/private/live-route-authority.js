...
const { fragment, Reassembler, SURB_REPLY_FRAGMENT_PROFILE } = require('./fragments')
const {
...
const { BRANCH_CLASS, CELL_CLASS, DIRECTION, M3_MESSAGE_ID, REPLY_MODE } = require('./protocol')
const {
  clearRoutedReply,
  clearRoutedRequest,
...
const { copyReturnPath, revokeSurbBatch } = require('./surb-path')
const {
  ROUTE_ENDPOINT,
  RoutePayloadCodec,
...
const QUERY_ADMISSION_TOKENS = new WeakMap()
const SPENT_QUERY_ADMISSIONS = new WeakSet()
...
function bindSurbReturnPath(authority, branch, hops) {
  const state = stateFor(authority)
  const owner = ownerFor(state, branch)
  owner.returnPathHops = copyReturnPath(hops)
  return true
}

function liveRouteReturnPath(authority, branch) {
  const state = stateFor(authority)
  const owner = ownerFor(state, branch)
  if (!owner.returnPathHops) return null
  return copyReturnPath(owner.returnPathHops)
}
...
function assertProductionDrain(state) {
  if (state.resourcesRevoked) throw PrivateRouteError.ERR_DESTROYED()
  if (!state.real) return
  const { assertLiveRoutePairDrain } = require('./route-manager')
  assertLiveRoutePairDrain(state.lease)
}

function readQueryAdmission(state, token, branch, commandId) {
  if (token === null || typeof token !== 'object') {
    authentication()
  }
  if (SPENT_QUERY_ADMISSIONS.has(token)) {
    throw PrivateRouteError.ERR_REPLAY()
  }
  const record = QUERY_ADMISSION_TOKENS.get(token)
  if (!record) {
    authentication()
  }
  if (
    record.state !== state ||
    record.lease !== state.lease ||
    record.branch !== branch ||
    state.owners.get(branch) !== record.owner ||
    record.owner.state !== state ||
    record.owner.generation !== record.generation
  ) {
    authentication()
  }
  if (!state.admissions || !state.admissions.has(token)) {
    throw PrivateRouteError.ERR_REPLAY()
  }
  if (commandId !== null) {
    if (!record.commandIds.has(commandId) || !branchAllowsCommand(branch, commandId)) {
      authentication()
    }
  }
  return record
...
function exactRequestOptions(options) {
  ...
  const required = ['branch', 'destinationRef', 'encodedRequest', 'attempt', 'operationDeadlineMs']
  const snap = Object.create(null)
  for (const name of required) {
    const descriptor = Object.getOwnPropertyDescriptor(options, name)
    if (!descriptor || !Object.prototype.hasOwnProperty.call(descriptor, 'value')) invalid()
    snap[name] = descriptor.value
  }
  for (const key of Reflect.ownKeys(options)) {
    if (required.includes(key)) continue
    if (key === 'surbBatch') {
      const descriptor = Object.getOwnPropertyDescriptor(options, 'surbBatch')
      if (!descriptor || !Object.prototype.hasOwnProperty.call(descriptor, 'value')) invalid()
      snap.surbBatch = descriptor.value
      continue
    }
    if (key === 'queryAdmission') {
      const descriptor = Object.getOwnPropertyDescriptor(options, 'queryAdmission')
      if (!descriptor || !Object.prototype.hasOwnProperty.call(descriptor, 'value')) invalid()
      snap.queryAdmission = descriptor.value
      continue
    }
    invalid()
  }
...
function exactDiscoveryOptions(options) {
  if (options === null || typeof options !== 'object' || Array.isArray(options)) invalid()
  const hasAdmission = Object.prototype.hasOwnProperty.call(options, 'queryAdmission')
  const hasCommand = Object.prototype.hasOwnProperty.call(options, 'commandId')
  if (hasAdmission !== hasCommand) invalid()
  const required = ['branch', 'target', 'limit']
  if (hasAdmission) {
    required.push('queryAdmission', 'commandId')
  }
  if (Reflect.ownKeys(options).length !== required.length) invalid()
  const snap = Object.create(null)
  for (const name of required) {
    const descriptor = Object.getOwnPropertyDescriptor(options, name)
    if (!descriptor || !Object.prototype.hasOwnProperty.call(descriptor, 'value')) invalid()
  ...
  if (
    !b4a.isBuffer(snap.target) ||
    snap.target.byteLength !== 32 ||
    !Number.isSafeInteger(snap.limit) ||
    snap.limit < 0 ||
    snap.limit > 20
  ) {
    invalid()
  }
  if (hasAdmission && !Number.isSafeInteger(snap.commandId)) invalid()
  return snap
}
...
function revokeAllStateAdmissions(state) {
  if (!state || !state.admissions) return
  for (const token of state.admissions) {
    const record = QUERY_ADMISSION_TOKENS.get(token)
    if (record) {
      QUERY_ADMISSION_TOKENS.delete(token)
      SPENT_QUERY_ADMISSIONS.add(token)
      record.state = null
      record.lease = null
      record.owner = null
      record.commandIds = null
    }
  }
...
function revokeProductionResources(state, reason) {
  ...
  revokeAllStateAdmissions(state)
  for (const owner of state.owners.values()) {
    try {
      destroyLiveOpaqueDestinations(owner.capability)
    } catch {}
...
function requestProduction(authority, state, options) {
  ...
  const hasAdmission = Object.prototype.hasOwnProperty.call(options, 'queryAdmission')
  if (hasAdmission) {
    readQueryAdmission(state, options.queryAdmission, options.branch, null)
  } else {
  ...
  let terminalAdmission = null
  try {
    request = decodeAnyRoutedRequest(options.encodedRequest)
    if (hasAdmission) {
      readQueryAdmission(state, options.queryAdmission, options.branch, request.commandId)
      assertProductionDrain(state)
      readQueryAdmission(state, options.queryAdmission, options.branch, request.commandId)
      if (state.suspended) throw PrivateRouteError.ERR_DESTROYED()
    }
    ...
    const surbBatch = options.surbBatch || null
    if (replyMode === REPLY_MODE.SURB_REQUIRED) {
      if (
        !surbBatch ||
        !Array.isArray(surbBatch.openAuthorities) ||
        !Array.isArray(surbBatch.terminalHandles) ||
        surbBatch.openAuthorities.length !== surbBatch.terminalHandles.length ||
        surbBatch.openAuthorities.length < 1
      ) {
        authentication()
      }
    } else if (surbBatch !== null) {
      invalid()
    }
    reassembler = new Reassembler({
      now: () => Number(state.monotonicNow()),
      epochExpiresAt: Number(owner.material.expiresAt),
      profile: replyMode === REPLY_MODE.SURB_REQUIRED ? SURB_REPLY_FRAGMENT_PROFILE : undefined
    })
    if (replyMode === REPLY_MODE.SURB_REQUIRED) {
      terminalAdmission = createSurbTerminalAdmission({
        now: () => state.monotonicNow(),
        wallNow: () => state.wallNow(),
        localDeadline: options.operationDeadlineMs
      })
      for (let i = 0; i < surbBatch.terminalHandles.length; i++) {
        registerSurbTerminalHandle(terminalAdmission, {
          terminalHandle: surbBatch.terminalHandles[i],
          openAuthority: surbBatch.openAuthorities[i],
          replyBinding: surbBatch.replyBindings[i],
          requestId: surbBatch.requestId || request.requestId,
          batchId: surbBatch.batchId || request.batchId
        })
      }
    }
    const operationState = {
      ...
      cancelDeadline: null,
      surbTerminalInbox: replyMode === REPLY_MODE.SURB_REQUIRED ? [] : null,
      surbTerminalWaiters: replyMode === REPLY_MODE.SURB_REQUIRED ? [] : null
    }
    if (operationState.surbTerminalInbox) {
      owner.surbTerminalInbox = operationState.surbTerminalInbox
      owner.surbTerminalWaiters = operationState.surbTerminalWaiters
    }
    ...
    const run = (async () => {
      ...
      while (operationState.live) {
        ...
        if (operationState.surbTerminalInbox && operationState.surbTerminalInbox.length > 0) {
          injectedTerminal = operationState.surbTerminalInbox.shift()
        } else if (operationState.surbTerminalInbox) {
          const receiveReservation = reserveM3RouteFrame(owner.binding.transport)
          operationState.receiveReservation = receiveReservation
          operationState.awaitingNetwork = true
          try {
            const raced = await new Promise((resolve, reject) => {
              let settled = false
              const finish = (value) => {
                if (settled) return
                settled = true
              ...
              receiveReservedM3RouteFrame(receiveReservation).then(
                (f) => finish({ frame: f }),
                fail
              )
              operationState.surbTerminalWaiters.push(() => {
                if (operationState.surbTerminalInbox.length > 0) {
                  finish({ injected: operationState.surbTerminalInbox.shift() })
                }
              })
            })
            if (raced.injected) {
              injectedTerminal = raced.injected
              // The transport reservation lost the race and is still armed; left
              // alone it would swallow the next reverse frame on this route (the
              // next correlated reply after a required one was lost exactly so).
              cancelM3RouteFrameReservation(receiveReservation)
            } else {
              frame = raced.frame
            }
          } catch (err) {
            ...
            operationState.surbTerminalWaiters.length = 0
          }
        } else {
          const receiveReservation = reserveM3RouteFrame(owner.binding.transport)
          operationState.receiveReservation = receiveReservation
        ...
        try {
          if (!operationState.live) throw PrivateRouteError.ERR_DESTROYED()
          if (injectedTerminal !== null) {
            // Last hop delivered a terminal cell outside RoutePayloadCodec (wire peel path).
            opened = {
              class: CELL_CLASS.DATAGRAM,
              payload: injectedTerminal
            }
          } else {
            // SURB hop/terminal packed frames: try unpack before RoutePayloadCodec.
            if (operationState.replyMode === REPLY_MODE.SURB_REQUIRED) {
              const {
                unpackSurbRouteFrame,
                tryDecodeSurbTerminalCell,
                isSurbHopFrame
              } = require('./surb-path')
              const unpacked = unpackSurbRouteFrame(frame)
              if (unpacked !== null) {
                const terminal = tryDecodeSurbTerminalCell(unpacked)
                if (terminal !== null) {
                  terminal.terminalHandle.fill(0)
                  terminal.payload.fill(0)
                  opened = { class: CELL_CLASS.DATAGRAM, payload: unpacked }
                } else {
                  unpacked.fill(0)
                }
              }
              // A hop cell the guard forwarded unpeeled is not a codec frame; the codec
              // would reject it and end this operation. Drop it and keep waiting.
              if (opened === null && isSurbHopFrame(frame)) continue
            }
            if (opened === null) {
          ...
          let encodedReply = null
          if (operationState.replyMode === REPLY_MODE.SURB_REQUIRED) {
            const terminal = tryDecodeSurbTerminalCell(opened.payload)
            if (terminal === null) {
              continue
            }
            let fragmentPlain = null
            try {
              fragmentPlain = admitSurbTerminalPayload(operationState.terminalAdmission, {
              ...
              if (terminal.terminalHandle) terminal.terminalHandle.fill(0)
              if (terminal.payload) terminal.payload.fill(0)
            }
            if (fragmentPlain === null) continue
            try {
              encodedReply = ownedReassembler.pushAuthenticated(fragmentPlain)
            } finally {
              if (fragmentPlain) fragmentPlain.fill(0)
            }
            if (encodedReply === null) {
              continue
            }
            // Strip zero-pad used to fill the committed SURB fragment count.
            if (encodedReply.byteLength >= 8) {
              const bodyLen = encodedReply.readUInt16BE(6)
              const total = 8 + bodyLen
              if (total > 0 && total <= encodedReply.byteLength) {
                const trimmed = b4a.from(encodedReply.subarray(0, total))
                encodedReply.fill(0)
                encodedReply = trimmed
              }
            }
          } else {
            encodedReply = ownedReassembler.pushAuthenticated(opened.payload)
    ...
    const promise = Promise.race([run, cancellation]).finally(() => {
      ...
      if (operationState.terminalAdmission) {
        try {
          revokeSurbTerminalAdmission(operationState.terminalAdmission)
        } catch {}
        operationState.terminalAdmission = null
      }
    ...
    if (reassembler) reassembler.destroy()
    if (terminalAdmission) revokeSurbTerminalAdmission(terminalAdmission)
    clearRoutedRequest(request)
...
function createRotatedLiveRouteAuthority(previousAuthority, routeManager, branchClass) {
  ...
  if (
    ...
    previous.suspended ||
    (previous.admissions && previous.admissions.size !== 0) ||
...
class LiveRouteAuthority {
  ...
  bootstrap(options) {
    const state = stateFor(this)
    options = exactDiscoveryOptions(options)
    const owner = ownerFor(state, options.branch)
    const hasAdmission = Object.prototype.hasOwnProperty.call(options, 'queryAdmission')
    if (hasAdmission) {
      readQueryAdmission(state, options.queryAdmission, options.branch, options.commandId)
      assertProductionDrain(state)
      readQueryAdmission(state, options.queryAdmission, options.branch, options.commandId)
    } else {
      assertProductionCurrent(state)
    ...
    if (state.suspended) throw PrivateRouteError.ERR_DESTROYED()
    return snapshotLiveOpaqueDestinations(owner.capability, options.limit)
  }
...
function createLiveRouteQueryAdmission(authority, branch, commandIds) {
  if (branch !== BRANCH_CLASS.LOOKUP && branch !== BRANCH_CLASS.ANNOUNCE) {
    invalid()
  }
  const state = stateFor(authority)
  if (!state.real || state.suspended || state.resourcesRevoked || state.destroyed) {
    authentication()
  }
  const owner = ownerFor(state, branch)
  if (!owner || owner.state !== state) {
    authentication()
  }
  if (!Array.isArray(commandIds)) {
    invalid()
  }
  const lengthDescriptor = Object.getOwnPropertyDescriptor(commandIds, 'length')
  ...
  for (let i = 0; i < len; i++) {
    const descriptor = Object.getOwnPropertyDescriptor(commandIds, String(i))
    if (!descriptor || !Object.prototype.hasOwnProperty.call(descriptor, 'value')) {
      invalid()
    }
    const id = descriptor.value
    if (!Number.isSafeInteger(id) || !branchAllowsCommand(branch, id)) {
      invalid()
    }
    allowed.add(id)
  }
  const expectedLease = state.lease
  ...
  if (
    ...
    state.resourcesRevoked ||
    state.destroyed ||
    state.lease !== expectedLease ||
    state.owners.get(branch) !== owner ||
    owner.state !== state ||
  ...
  QUERY_ADMISSION_TOKENS.set(token, record)
  state.admissions.add(token)
  return token
}

function revokeLiveRouteQueryAdmission(token) {
  if (token === null || typeof token !== 'object') return false
  const record = QUERY_ADMISSION_TOKENS.get(token)
  if (!record) return false
  QUERY_ADMISSION_TOKENS.delete(token)
  SPENT_QUERY_ADMISSIONS.add(token)
  if (record.state && record.state.admissions) {
    record.state.admissions.delete(token)
  }
  record.state = null
  record.lease = null
  record.owner = null
...
function deliverSurbTerminalToLiveRoute(authority, terminalCell) {
  if (!b4a.isBuffer(terminalCell)) invalid()
  const state = stateFor(authority)
  // Prefer LOOKUP owner inbox (SURB rides the lookup branch in Gate C).
  for (const branch of [BRANCH_CLASS.LOOKUP, BRANCH_CLASS.ANNOUNCE]) {
    let owner = null
    try {
      owner = ownerFor(state, branch)
    } catch {
      continue
    }
    if (!owner || !Array.isArray(owner.surbTerminalInbox)) continue
    owner.surbTerminalInbox.push(b4a.from(terminalCell))
    const waiters = owner.surbTerminalWaiters
