...
const M3_PROTOCOL_VERSION = 1

const BRANCH_CLASS = Object.freeze({ LOOKUP: 0, ANNOUNCE: 1 })

const M3_LINK_ROLE = Object.freeze({ CLIENT: 0, SAFETY_RELAY: 1, DHT_EXIT: 2 })

const RELAY_CAPABILITY = Object.freeze({
  CIRCUIT_RELAY_V1: 1,
  DHT_EXIT_V1: 2,
  PRIVATE_RECORDS_V1: 4
})

const CAPACITY_CLASS = Object.freeze({ SMALL: 0, MEDIUM: 1, LARGE: 2 })

const MUTATION_FLAG = Object.freeze({ READ_ONLY: 0, MUTATING: 1 })

const DESTINATION_VALIDATION_CLASS = Object.freeze({
  EXIT_LOCAL: 0,
  DHT_NODE_HANDLE: 1,
  SIGNED_CAPABILITY_HANDLE: 2
})

const CONTEXT_CLASS = Object.freeze({
...
const M3_MESSAGE_ID = Object.freeze({
...
const ROUTED_ERROR = Object.freeze({
  MALFORMED: 0x0180,
...
const M3_ID_REGISTRY = Object.freeze(
...
const TOPOLOGY_ROLE = Object.freeze({
  SOURCE: 0,
  SAFETY_GUARD: 1,
  SAFETY_FINAL: 2,
  PRIVATE_ENTRY: 3,
...
const LINK_OPERATION = Object.freeze({
  INITIATE: 1,
  ACCEPT: 2,
  KNOWN: 3
})
...
const BOOTSTRAP_TYPE = Object.freeze({
  LINK_CREATE: 0,
  LINK_CREATED: 1,
  LINK_REJECT: 2,
  LINK_CANCEL: 3
...
const BOOTSTRAP_REJECT_CODE = Object.freeze({
  UNAUTHORIZED: 0,
  CIRCUIT_LIMIT: 1,
  ROUTE_UNAVAILABLE: 2
})
...
const DOMAIN_VALUES = Object.freeze({
  ...
  TOPOLOGY_GRANT_V1: b4a.from('hyperdht-private-routes/topology-grant/v1'),
  ...
  DHT_EXIT_DHT_SEEDS: b4a.from('hyperdht-private-routes/m3/dht-exit-dht-seeds/v1'),
  DHT_EXIT_DHT_SEEDS_SET: b4a.from('hyperdht-private-routes/m3/dht-exit-dht-seeds/set/v1')
})
...
const DOMAIN = Object.freeze(Object.defineProperties({}, domainProperties))
...
function identityBufferLength(value) {
  try {
    if (objectHasOwnProperty.call(value, 'byteLength')) return -1
    return m3BufferLength(value)
  } catch {
    return -1
  }
}

function roleForIdentity(publicKey) {
  if (identityBufferLength(publicKey) !== 32) {
...
function m3ObjectDataProperty(value, name, required) {
  const descriptor = objectGetOwnPropertyDescriptor(value, name)

  if (descriptor === undefined) {
    if (required) invalidM3Object()
    return undefined
  }
  if (!objectHasOwnProperty.call(descriptor, 'value')) invalidM3Object()
...
function m3ObjectOptions(value) {
  try {
    ...
    const messageId = m3ObjectDataProperty(value, 'messageId', true)
    const body = m3ObjectDataProperty(value, 'body', true)
    const authSuffix = m3ObjectDataProperty(value, 'authSuffix', false)

    return {
      messageId,
      body,
      authSuffix: authSuffix === undefined ? b4a.alloc(0) : authSuffix
    }
  } catch {
    invalidM3Object()
  }
...
function decodeM3Object(encoded) {
  try {
