...
const MAX_ROUTE_LIFETIME_MS = 15_000n
...
function createRouteExtensionLimits(limits, now, deadline) {
  ...
  if (deadline <= current || current > MAX_UINT64 - MAX_ROUTE_LIFETIME_MS) {
  ...
  return Object.freeze({
    ...
    expiresAtMs: min(current + MAX_ROUTE_LIFETIME_MS, deadline)
  })
}
...
class RouteExtensionSession {
  constructor(request) {
    ...
    try {
      ...
      const operationDelta = min(MAX_EXTENSION_MS, material.absoluteDeadline - localStart)
      const routeDelta = min(
        MAX_ROUTE_LIFETIME_MS,
        material.signedExpiry - wallStart,
        decoded.expiresAtMs - wallStart
      )
      if (operationDelta <= 0n || routeDelta <= 0n) {
        throw PrivateRouteError.ERR_PRIVACY_UNAVAILABLE()
      }
      this._localExpiresAt = localStart + operationDelta
      this._wireExpiresAt = wallStart + routeDelta
