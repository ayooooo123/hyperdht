...
const MAX_COUNTER_BYTES = MAX_UNSIGNED_SIZE + SIGNATURE_SIZE
...
function decodeUnsignedNatPunchPlan(buffer) {
  return decodeValue(buffer, false)
}

function encodeNatPunchCounter(unsigned, responderSignature64) {
  if (
    !b4a.isBuffer(unsigned) ||
    unsigned.byteLength < MIN_UNSIGNED_SIZE ||
    unsigned.byteLength > MAX_UNSIGNED_SIZE
  ) {
    invalid()
  }
  if (!fixed(responderSignature64, SIGNATURE_SIZE)) invalid()
  // Validate structure and reject trailing ambiguity by decoding strictly.
  decodeValue(unsigned, false)
  return b4a.concat([b4a.from(unsigned), b4a.from(responderSignature64)])
}

function decodeNatPunchCounter(buffer) {
  if (!b4a.isBuffer(buffer)) invalid()
  if (
    buffer.byteLength < MIN_UNSIGNED_SIZE + SIGNATURE_SIZE ||
    buffer.byteLength > MAX_COUNTER_BYTES
  ) {
    invalid()
  }
  const unsigned = b4a.from(buffer.subarray(0, buffer.byteLength - SIGNATURE_SIZE))
  const responderSignature = b4a.from(buffer.subarray(buffer.byteLength - SIGNATURE_SIZE))
  const decoded = decodeValue(unsigned, false)
  return {
    version: decoded.version,
    format: decoded.format,
    planId32: decoded.planId32,
    topologyGrantDigest32: decoded.topologyGrantDigest32,
    epoch: decoded.epoch,
    runId32: decoded.runId32,
    ...
    punchProfile: decoded.punchProfile,
    unsigned,
    responderSignature
  }
}
...
function isAllZero(buffer) {
  if (!b4a.isBuffer(buffer)) return false
...
function samePublicSide(left, right) {
  return (
    left &&
    right &&
    b4a.equals(left.identity32, right.identity32) &&
...
// Counter unsigned may differ from the offer unsigned only in:
// responder.reflectionClaimDigest32 (zero -> non-zero),
// responder.nonce32 (zero -> non-zero),
// and expiresAt (never raised).
function unsignedNatPunchCounterMatchesOffer(offerUnsigned, counterUnsigned) {
  if (!b4a.isBuffer(offerUnsigned) || !b4a.isBuffer(counterUnsigned)) return false
  if (offerUnsigned.byteLength !== counterUnsigned.byteLength) return false
  let offer
  let counter
  try {
    offer = decodeValue(offerUnsigned, false)
    counter = decodeValue(counterUnsigned, false)
  } catch {
    return false
  }
  if (offer.version !== counter.version || offer.format !== counter.format) return false
  if (!b4a.equals(offer.planId32, counter.planId32)) return false
  if (!b4a.equals(offer.topologyGrantDigest32, counter.topologyGrantDigest32)) return false
  if (offer.epoch !== counter.epoch || !b4a.equals(offer.runId32, counter.runId32)) return false
  if (offer.notBefore !== counter.notBefore) return false
  if (offer.punchProfileId !== counter.punchProfileId) return false
  if (!samePublicSide(offer.initiator, counter.initiator)) return false
  if (
    !b4a.equals(
      offer.initiator.reflectionClaimDigest32,
      counter.initiator.reflectionClaimDigest32
    ) ||
    !b4a.equals(offer.initiator.nonce32, counter.initiator.nonce32)
  ) {
    return false
  }
  if (!samePublicSide(offer.responder, counter.responder)) return false
  if (!isAllZero(offer.responder.reflectionClaimDigest32) || !isAllZero(offer.responder.nonce32)) {
    return false
  }
  if (
    isAllZero(counter.responder.reflectionClaimDigest32) ||
    isAllZero(counter.responder.nonce32)
  ) {
    return false
  }
  if (counter.expiresAt > offer.expiresAt) return false
  return true
}
