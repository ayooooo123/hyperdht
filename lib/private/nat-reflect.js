...
function issueReflectedEndpointClaim(options) {
  ...
  const record = {
    ...
    // The reflectors whose reply reached this socket, so the caller can account
    // for each contact it made: answered here, silent otherwise.
    answeredReflectors: Array.from(byIdentity.keys(), (key) => b4a.from(key, 'hex')),
    live: true
  }
...
// Which configured reflectors answered the probe that produced this claim; copies,
// like every other view of a claim. A destroyed claim answers nobody.
function readReflectedEndpointClaimReflectors(claim) {
  const record = isObject(claim) ? CLAIMS.get(claim) : null
  if (!record || !record.live || DESTROYED_CLAIMS.has(claim)) unauthorized()
  return Object.freeze(record.answeredReflectors.map((identity) => b4a.from(identity)))
}
...
function destroyReflectedEndpointClaim(claim) {
  ...
  for (const identity of record.answeredReflectors) clear(identity)
  record.answeredReflectors = []
  return true
}
...
module.exports = Object.freeze({
  ...
  readReflectedEndpointClaimReflectors,
