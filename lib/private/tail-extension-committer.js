...
const { M3_CONTEXT_ENVELOPE_SIZE } = require('./m3-context')
...
function createTailExtensionCommitter(options = {}) {
...
function isTailExtensionCommitter(value) {
  return object(value) && COMMITTERS.has(value)
}

function destroyTailExtensionCommitter(committer) {
  const state = object(committer) ? COMMITTERS.get(committer) : null
...
function enqueueTailExtended(committer, envelope) {
...
function installTailExtension(committer, nextRuntime, expiresAt) {
  const state = stateFor(committer)
  if (
    state.phase !== 1 ||
    !object(nextRuntime) ||
  ...
  let published = false
  try {
    forwarding = state.install(nextRuntime, expiresAt)
    assertState(state, lifecycle)
