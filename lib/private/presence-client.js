...
//
// `replyMode` (decision D12): reads run in the controller's required SURB reply
// mode when the client is created with `replyMode: 'SURB_REQUIRED'`, so the
// resolving endpoint's reply never travels the correlated path. Publishing and
// revocation are puts, whose replies are correlated acknowledgements today.
...
function createPresenceClient(options) {
  ...
  const replyMode = options.replyMode === undefined ? 'CORRELATED' : options.replyMode
  if (replyMode !== 'CORRELATED' && replyMode !== 'SURB_REQUIRED') {
    throw PrivateRouteError.INVALID_KEY()
  }
  ...
  async function publishForPeriods({ type, revision, descriptor, scope, targets }) {
    ...
    if (type === RECORD_TYPE.TOMBSTONE && scope === TOMBSTONE_SCOPE.RECORD) {
      ...
      for (const period of periods) {
        ...
        if (!isRecordTombstoneTarget(target)) throw PrivateRouteError.INVALID_DESCRIPTOR()
  ...
  async function resolvePresence({ previous } = {}) {
    ...
    for (const period of periods) {
      ...
      const record = await controller.mutableGet(publicKey, { latest: true, replyMode })
