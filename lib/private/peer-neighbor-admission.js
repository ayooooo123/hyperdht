...
// An idle neighbor link pings after 500 ms (`link-control-session.js`), and
// each ping, pong and ACK spends one service cell: about 4 cells per second
// per side, measured. Default budgets use 5 per second so a neighbor lasts its
// whole grant. The node ledger stays finite (packet §3.4); a relay that renews
// grants starts a fresh owner, and so a fresh ledger, per renewal cycle.
const SERVICE_CELLS_PER_SECOND = 5n
const NEIGHBOR_SETUP_CELLS = 32n
const DISCOVERY_CELLS = 24n
const RECONNECT_ALLOWANCE = 8n
const MAX_U32 = 0xffff_ffffn

function cells(count) {
  const value = count > MAX_U32 ? MAX_U32 : count
  return { cells: Number(value), bytes: 1200n * value, commands: Number(value) }
}

// Budgets sized to the longest admitted grant: one neighbor reservation covers
// a full grant of keepalives, and the node ledger covers, per neighbor, one
// grant of spend plus a full reservation for a late reconnect, plus discovery
// for every attempt and reconnect.
function grantSizedBudgets(state, slots, maxAttempts) {
  const now = BigInt(state.wallNow())
  let longestMs = 0n
  for (const slot of slots) {
    if (slot.expiresAt - now > longestMs) longestMs = slot.expiresAt - now
  }
  const perNeighbor = ((longestMs + 999n) / 1000n) * SERVICE_CELLS_PER_SECOND + NEIGHBOR_SETUP_CELLS
  const discovery = DISCOVERY_CELLS * (BigInt(maxAttempts) + RECONNECT_ALLOWANCE)
  const node = BigInt(slots.length) * (2n * perNeighbor + discovery)
  return { neighborServiceReservation: cells(perNeighbor), nodeServiceBudget: cells(node) }
}
...
function createPeerNeighborAdmission(options) {
  ...
  try {
    ...
    const sized = grantSizedBudgets(state, Array.from(state.slots.values()), maxAttempts)
    state.pool = createPeerNativeNeighborPool({
      relayOwner,
      endpoint,
      maxNeighbors,
      nodeServiceBudget:
        nodeServiceBudget === undefined ? sized.nodeServiceBudget : nodeServiceBudget,
      neighborServiceReservation:
        neighborServiceReservation === undefined
          ? sized.neighborServiceReservation
          : neighborServiceReservation,
