...
function createPeerNeighborAdmission(options) {
  ...
  try {
    for (const encoding of grants) {
      const slot = admitGrant(state, encoding)
      if (state.slots.has(slot.peerKey)) throw PrivateRouteError.CIRCUIT_STATE()
      state.slots.set(slot.peerKey, slot)
    }
...
function startSlot(state, slot) {
  slot.running = runSlot(state, slot)
}
