...
function sealTailExtend(session, state, options) {
  ...
  try {
    ...
    if (
      !uint64(projectedWireExpiresAt) ||
      !uint64(requestedWireExpiresAt) ||
      requestedWireExpiresAt === 0n
    )
      invalid()
    if (
...
function takeFinalExitHandoff(session, state) {
  ...
  const material = state.finalExitMaterial
  if (!material) invalid()
...
function createTailControlResponderAuthority(session, responderToken, options) {
  ...
  try {
    ...
    if (terminalResponder) {
      sessionState.finalExitMaterial = createResponderFinalExitMaterial(session, sessionState)
      sessionState.finalExitStage = 'FINAL_EXIT_READY'
    }
    return authority
  } catch (err) {
    clearResponderAuthorityState(record)
...
function completeTailExtend(authority, completion) {
  ...
  try {
    ...
    forwarding = installTailExtension(extensionCommitter, runtime)
    runtime = null
