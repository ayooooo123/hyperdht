...
// True when a reverse route frame carries a SURB hop cell (packed, or raw on the
// in-process harness). A relay that cannot peel such a frame must drop it, and an
// endpoint must never hand it to the route codec: it is not a codec frame. The
// check reads the frame in place; it owns and copies nothing.
function isSurbHopFrame(frame) {
  if (!isBuffer(frame)) return false
  let offset = 0
  if (frame.byteLength === ROUTE_FRAME_SIZE) {
    const len = (frame[0] << 8) | frame[1]
    if (len >= 1 && len <= ROUTE_FRAME_SIZE - 2) offset = 2
  }
  const magic = SURB_HOP_MAGIC
  if (frame.byteLength - offset <= magic.byteLength) return false
  for (let i = 0; i < magic.byteLength; i++) {
    if (frame[offset + i] !== magic[i]) return false
  }
  return true
}
