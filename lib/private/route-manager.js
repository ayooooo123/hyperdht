...
let teardownOpenRouteMaterial = (material, teardownId) =>
  require('./live-route-authority').teardownOpenRouteTransport(material, teardownId)
const TEST_ONLY_ROUTE_MANAGER_FACTORY_ISSUER = Object.freeze({
  ...
  installTeardown(handler) {
    if (typeof handler !== 'function') invalid()
    const previous = teardownOpenRouteMaterial
    teardownOpenRouteMaterial = handler
    return () => {
      if (teardownOpenRouteMaterial !== handler) invalid()
      teardownOpenRouteMaterial = previous
    }
  }
})
...
function preflightCandidateDirectory(directory) {
  const scope = readRelayCandidateDirectoryScope(directory)
  scope.guardIdentity.fill(0)
  scope.guardEndpoint.fill(0)
  scope.guardAdvertisementDigest.fill(0)
}
...
function retireManagerState(state) {
  if (state.destroyed) return false
  state.destroyed = true
  MANAGERS.delete(state.manager)
  DESTROYED_MANAGERS.add(state.manager)
  return true
}

function destroyManagerRouteOwnership(state) {
  ...
  for (const retired of state.retiredBranches.values()) {
    ...
    safeDestroyOpenMaterial(retired.material)
  }
...
function destroyManagerGuardOwnership(state) {
  try {
...
function currentBranchStillLive(state, branchClass) {
  const current = state.branches.get(branchClass)
  const now = absoluteNow(state.extensionClock.wallNow)
  return !!(current && !current.lost && current.material.expiresAt > now)
}
...
class RouteManager {
  ...
  suspend() {
    ...
    const operation = Promise.resolve().then(async () => {
      const teardownIds = [null, null]
      let teardownOperations = []
      let firstError = null
      try {
        preflightCandidateDirectory(state.candidateDirectory)
        teardownIds[0] = randomId(state.randomBytes)
        teardownIds[1] = randomId(state.randomBytes)
        if (typeof teardownOpenRouteMaterial !== 'function') invalid()
        teardownOperations = [
          teardownOpenRouteMaterial(lookup.material, teardownIds[0]),
          teardownOpenRouteMaterial(announce.material, teardownIds[1])
        ].map((pending) =>
          Promise.resolve(pending).catch((err) => {
            if (firstError === null) {
              firstError = err
              safeDestroyOpenMaterial(lookup.material)
              safeDestroyOpenMaterial(announce.material)
            }
            throw err
          })
        )
        const settled = await Promise.allSettled(teardownOperations)
        if (firstError !== null) throw firstError
        const rejected = settled.find((result) => result.status === 'rejected')
        if (rejected) throw rejected.reason
        if (state.destroyed || MANAGERS.get(this) !== state) {
          throw PrivateRouteError.ERR_DESTROYED()
        }
        retireManagerState(state)
        destroyManagerRouteOwnership(state)
        ...
        if (!state.destroyed) retireManagerState(state)
        destroyManagerRouteOwnership(state)
        destroyManagerGuardOwnership(state)
        if (teardownOperations.length > 0) await Promise.allSettled(teardownOperations)
        throw err instanceof PrivateRouteError ? err : PrivateRouteError.ERR_PRIVACY_UNAVAILABLE()
      } finally {
        for (const teardownId of teardownIds) {
          if (b4a.isBuffer(teardownId)) teardownId.fill(0)
        }
      }
    })
...
function readOwnedLiveRoutePair(lease) {
  const record = isObject(lease) ? LIVE_PAIR_LEASES.get(lease) : null
  if (!record || record.spent || record.failed) {
  ...
  if (
    state.destroyed ||
    !state.ready ||
    (state.status !== 'READY' && state.status !== 'ROTATING') ||
    record.lookup.lost ||
    record.announce.lost
  ) {
    // Failed use burns admission, not the manager's ownership-transfer handle.
    record.failed = true
    SPENT_LIVE_PAIR_LEASES.add(lease)
    throw PrivateRouteError.ERR_DESTROYED()
  }
  const now = absoluteNow(state.extensionClock.wallNow)
  // Sampling the injected clock can revoke or transfer this exact lease.
  if (record.spent || LIVE_PAIR_LEASES.get(lease) !== record) {
    throw PrivateRouteError.ERR_REPLAY()
  }
  const lookupCurrent = state.branches.get(BRANCH_CLASS.LOOKUP) === record.lookup
  const announceCurrent = state.branches.get(BRANCH_CLASS.ANNOUNCE) === record.announce
  if (
    state.destroyed ||
    !state.ready ||
    (state.status !== 'READY' && state.status !== 'ROTATING') ||
    state.retiredBranches.size > 1 ||
    (!lookupCurrent && state.retiredBranches.get(BRANCH_CLASS.LOOKUP) !== record.lookup) ||
    (!announceCurrent && state.retiredBranches.get(BRANCH_CLASS.ANNOUNCE) !== record.announce) ||
    (!lookupCurrent && !announceCurrent) ||
    record.lookup.lost ||
    record.announce.lost ||
    record.lookup.material.expiresAt <= now ||
    record.announce.material.expiresAt <= now
  ) {
    record.failed = true
    SPENT_LIVE_PAIR_LEASES.add(lease)
    throw PrivateRouteError.ERR_DESTROYED()
...
function readLiveRoutePair(lease) {
  const record = readOwnedLiveRoutePair(lease)
  const state = record.state
  // Refuse new work while the old, still-owned pair drains. A readiness check
  // must not consume the lease that the rotation transaction still owns.
  if (
    state.status !== 'READY' ||
    state.rotationDrafts.size !== 0 ||
    state.rotationSeeds.size !== 0 ||
    state.retiredBranches.size !== 0 ||
    state.branches.get(BRANCH_CLASS.LOOKUP) !== record.lookup ||
    state.branches.get(BRANCH_CLASS.ANNOUNCE) !== record.announce
  ) {
    throw PrivateRouteError.ERR_PRIVATE_BRANCH_ROTATING()
  }
  return describeLiveRoutePair(record)
}

function describeLiveRoutePair(record) {
  const state = record.state
  return {
    lookup: {
...
function assertLiveRoutePairDrain(lease) {
  // Only an already-admitted operation may use this check. Transfer/revoke,
  // loss of exact manager ownership, and expiry still fail closed.
  readOwnedLiveRoutePair(lease)
}
...
function claimRotatedLiveRoutePair(manager, branchClass) {
  ...
  try {
    ...
    return Object.freeze({
      lease,
      // Transfer describes ownership only. Readiness and every admission still
      // validate liveness; a lost sibling can be replaced without authorizing it.
      pair: describeLiveRoutePair(LIVE_PAIR_LEASES.get(lease)),
      retired
    })
  } catch (err) {
...
function hasRouteManagerBranchLoss(manager, branchClass) {
  const state = readRouteManager(manager)
  if (branchClass !== BRANCH_CLASS.LOOKUP && branchClass !== BRANCH_CLASS.ANNOUNCE) invalid()
  const branch = state.branches.get(branchClass)
  return !!(branch && branch.lost)
}

// The lost flag belongs to the published branch object. A replacement discards
// it; restoring a wakeup never carries an old loss into a new generation.
function renewRouteManagerBranchLoss(manager, branchClass, sink) {
  const state = readRouteManager(manager)
  if (!isObject(sink) || !Object.isFrozen(sink) || Reflect.ownKeys(sink).length !== 0) {
    throw PrivateRouteError.UNAUTHORIZED()
  }
  if (!hasRouteManagerBranchLoss(manager, branchClass)) return false
  const key = branchSinkKey(branchClass, 'BranchLoss')
  if (!state.lifecycleRegistered || state.lifecycleSinks[key] !== null) return false
  state.lifecycleSinks[key] = sink
  return true
}

function redeliverRouteManagerBranchLosses(manager) {
  const state = readRouteManager(manager)
  if (!state.ready || state.status !== 'READY') return false
  let delivered = false
  for (const branchClass of [BRANCH_CLASS.LOOKUP, BRANCH_CLASS.ANNOUNCE]) {
    if (hasRouteManagerBranchLoss(manager, branchClass)) {
      delivered = issueBranchSink(state, branchClass, 'BranchLoss') || delivered
    }
  }
  return delivered
}
...
function createRouteManagerBranchLossRegistration(manager, branchClass) {
  ...
  BRANCH_PHYSICAL_LOSS_REGISTRATIONS.set(registration, {
    ...
    state,
    branchClass,
    generation: state.branches.get(branchClass).material.generation
  })
...
function issueRouteManagerBranchPhysicalLoss(registration) {
  ...
  return reportRouteManagerBranchLoss(record.manager, record.branchClass, record.generation)
}

function reportRouteManagerBranchLoss(manager, branchClass, generation) {
  const state = readRouteManager(manager)
  if (branchClass !== BRANCH_CLASS.LOOKUP && branchClass !== BRANCH_CLASS.ANNOUNCE) invalid()
  if (typeof generation !== 'bigint' || generation < 1n) invalid()
  if (!state.ready || (state.status !== 'READY' && state.status !== 'ROTATING')) return false
  let branch = state.branches.get(branchClass)
  if (!branch || branch.material.generation !== generation) {
    branch = state.retiredBranches.get(branchClass)
    if (!branch || branch.lost || branch.material.generation !== generation) return false
    branch.lost = true
    return true
  }
  if (branch.lost) return false
  branch.lost = true
  ...
  // A build already replaces this branch. Keep the fault on its old object,
  // but do not spend the replacement's freshly registered lifecycle sink.
  if (!state.rotationDrafts.has(branchClass) && !state.rotationSeeds.has(branchClass)) {
    issueBranchSink(state, branchClass, 'BranchLoss')
  }
  return true
}
...
function readBranchPath(manager, branchClass) {
  const state = readRouteManager(manager)
  if (branchClass !== BRANCH_CLASS.LOOKUP && branchClass !== BRANCH_CLASS.ANNOUNCE) invalid()
  const branch = state.branches.get(branchClass)
  if (!branch || !branch.path) throw PrivateRouteError.ERR_PRIVACY_UNAVAILABLE()
  return branch.path
}
