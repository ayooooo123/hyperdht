...
const PEER_FINAL_EXIT_MATERIAL_KEYS = Object.freeze([
  'clockIdentity',
  'expiresAt',
  'finalizeForwardKey',
...
function snapshotPeerFinalExitMaterial(material) {
  const facts = {}
  for (const name of PEER_FINAL_EXIT_MATERIAL_KEYS) facts[name] = material[name]
  return Object.freeze(facts)
}

function peerFinalExitMaterialMatches(record, material) {
  try {
    const facts = record.materialFacts
    const keys = Reflect.ownKeys(material)
    if (!facts || keys.length !== PEER_FINAL_EXIT_MATERIAL_KEYS.length) return false
    for (const name of PEER_FINAL_EXIT_MATERIAL_KEYS) {
      const descriptor = Object.getOwnPropertyDescriptor(material, name)
      if (!descriptor || !Object.hasOwn(descriptor, 'value') || descriptor.value !== facts[name]) {
        return false
      }
    }
    return (
      facts.tailControl === record.owner &&
      facts.clockIdentity === record.state.clockIdentity &&
      facts.initiator === record.state.initiator &&
      facts.expiresAt === record.state.wireExpiresAt &&
      facts.wireExpiresAt === record.state.wireExpiresAt &&
      facts.localDeadline === record.state.localDeadline
    )
  } catch {
    return false
  }
...
function createPeerFinalExitHandoff(session) {
  ...
  const materialFacts = snapshotPeerFinalExitMaterial(material)
...
function prepareTailControlFinalExitActivation(handoff, activationOwner) {
  ...
  if (
    ...
    state.destroyed ||
    state.finalExitStage !== 'FINAL_EXIT_HANDOFF' ||
    !peerFinalExitMaterialMatches(record, material)
  ) {
    invalid()
  }
  ...
  try {
    ...
    FINAL_EXIT_TRANSFERS.set(transfer, {
      ...
      materialFacts: record.materialFacts,
...
function commitTailControlFinalExitActivation(transfer, activationOwner) {
  ...
  if (
    ...
    state.destroyed ||
    state.finalExitStage !== 'FINAL_EXIT_HANDOFF' ||
    state.finalExitGeneration !== record.generation ||
    !peerFinalExitMaterialMatches(record, material)
  ) {
    invalid()
  }
