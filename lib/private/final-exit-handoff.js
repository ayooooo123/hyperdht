...
function validMaterial(owner, material) {
  try {
    ...
    return (
      ...
      typeof material.wireExpiresAt === 'bigint' &&
      material.wireExpiresAt > 0n &&
      typeof material.localDeadline === 'bigint' &&
      material.localDeadline > 0n &&
      object(material.clockIdentity) &&
...
function getFinalExitTailControl(owner) {
  const peerTailControl = require('./peer-tail-control')
  if (peerTailControl.isPeerTailControlOwner(owner)) return peerTailControl
  return require('./tail-control')
}

function getFinalExitHandoffTailControl(handoff) {
  const record = object(handoff) ? HANDOFFS.get(handoff) : null
  if (!record) {
    if (object(handoff) && SPENT_HANDOFFS.has(handoff)) replay()
    authentication()
  }
  return getFinalExitTailControl(record.owner)
}

function createFinalExitHandoff(owner, material) {
...
function consumeFinalExitHandoff(handoff) {
  const record = object(handoff) ? HANDOFFS.get(handoff) : null
  if (!record) {
    if (object(handoff) && SPENT_HANDOFFS.has(handoff)) replay()
    authentication()
  }
  const tailControl = getFinalExitTailControl(record.owner)
  if (
    tailControl &&
    typeof tailControl.rejectTailControlFinalExitHandoffConsume === 'function' &&
    tailControl.rejectTailControlFinalExitHandoffConsume(record.owner)
  ) {
    replay()
  }
  ...
  try {
    bridgeDestroyed =
      tailControl && typeof tailControl.destroyTailControlFinalExitHandoffOwner === 'function'
        ? tailControl.destroyTailControlFinalExitHandoffOwner(record.owner)
        : false
  } catch {}
  if (bridgeDestroyed) {
    ...
    destroyFinalExitHandoffMaterial(record.material)
    replay()
  }
  if (!HANDOFFS.has(handoff)) replay()
  HANDOFFS.delete(handoff)
  OWNER_HANDOFFS.delete(record.owner)
  SPENT_HANDOFFS.add(handoff)
...
function revokeFinalExitHandoff(owner) {
  ...
  SPENT_HANDOFFS.add(handoff)
  try {
    const { destroyTailControlFinalExitHandoffOwner } = getFinalExitTailControl(owner)
    destroyTailControlFinalExitHandoffOwner(owner)
  } catch {}
  if (record) destroyFinalExitHandoffMaterial(record.material)
  return true
}

function destroyFinalExitHandoffMaterial(material) {
...
module.exports = {
  getFinalExitTailControl,
  getFinalExitHandoffTailControl,
