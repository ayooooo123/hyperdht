...
const MAX_RESPONDER_STARTUP_MS = 1000n
...
function createPeerBootstrapResponder(relayOwner) {
  const facts = readPeerRelayOwner(relayOwner)
  const now = BigInt(facts.monotonicNow())
  const remaining = BigInt(facts.parsedAdvertisement.expiresAt) - BigInt(facts.wallNow())
  if (now < 0n || remaining <= 0n) throw PrivateRouteError.INVALID_ROUTE()
  const startupDelay = minBigInt(MAX_RESPONDER_STARTUP_MS, remaining)
  const startupDeadline = checkedU64Add(now, startupDelay)
  ...
  try {
    const timer = facts.setTimer(() => {
      if (!responderState.boundEndpoint) destroyPeerBootstrapResponder(handle)
    }, Number(startupDelay))
    if (responderState.destroyed) {
      try {
        facts.clearTimer(timer)
      } catch {}
      throw PrivateRouteError.ERR_PRIVACY_UNAVAILABLE()
    }
    responderState.startupTimer = timer
    if (timer === null || timer === undefined) throw PrivateRouteError.ERR_PRIVACY_UNAVAILABLE()
  } catch (err) {
    destroyPeerBootstrapResponder(handle)
    throw err
  }
  return handle
}
...
function takePeerBootstrapResponderBinding(responder, endpoint) {
  ...
  if (state.destroyed) throw PrivateRouteError.INVALID_ROUTE()
