...
// Experimental required mode covers reads and put acknowledgements.
...
// `now` is the endpoint's monotonic clock and bounds the operation (`localDeadline`
// lives on it); `wallNow` is the wall clock the hop capabilities were advertised
// on, which is what an open authority's `expiresAtMs` compares against.
function createSurbTerminalAdmission(options) {
  if (options === null || typeof options !== 'object' || Array.isArray(options)) invalid()
  const now = options.now
  const wallNow = options.wallNow
  const localDeadline = options.localDeadline
  if (typeof now !== 'function' || typeof wallNow !== 'function' || !isUint64(localDeadline)) {
    invalid()
  }
...
function admitSurbTerminalPayload(table, options) {
  ...
  // An unknown handle is not this table's business: no clock read, no state change.
  if (!entry) return null
  // The wall clock is sampled after the lookup and before the entry is spent: a
  // callback that throws or returns a non-time must leave the entry in the table so
  // revocation still reaches its open authority.
  let wall = null
  try {
    wall = state.wallNow()
  } catch {
