...
function admitSurbTerminalPayload(table, options) {
  ...
  // The wall clock is sampled before the entry is spent: a callback that throws or
  // returns a non-time must leave the entry in the table so revocation still reaches
  // its open authority.
  let wall = null
  try {
    wall = state.wallNow()
