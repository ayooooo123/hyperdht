...
function requestDhtExitImmutableGet(io, table, encodedRequest, options) {
  ...
  if (options && options.onSurbReplySettled !== undefined) {
    optionFields.push('onSurbReplySettled')
  }
  ...
  if (
    ...
    (options.onSurbReplySettled !== undefined && typeof options.onSurbReplySettled !== 'function')
  ) {
    invalid()
  }
    ...
    try {
      ...
      if (replyMode === REPLY_MODE.SURB_REQUIRED) {
        ...
        try {
          ...
          // The correlated path settles the service through onRoutedReply; a
          // required reply, sent or dropped, settles it here so the next request
          // on the route is admitted.
          if (options.onSurbReplySettled !== undefined) {
            try {
              options.onSurbReplySettled()
            } catch {}
          }
        }
        return true
    ...
    while (state.live && service.live) {
      ...
      try {
        ...
        try {
          ...
          requestOptions.onSurbReplySettled = () => {
            if (service.activeOperation === pending || service.activeOperation === operation) {
              service.activeOperation = null
            }
          }
