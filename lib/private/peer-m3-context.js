...
function encodePeerTailFinalizeAD(value) {
  ...
  if (!fixed(circuitId, 16) || !uint64(generation) || generation === 0n || !direction(selectedDirection) || !uint64(counter)) {
    invalid()
  }
...
function sealPeerContextFrame(options) {
  ...
  if (contextClass === PEER_TAIL_CONTROL_CONTEXT_CLASS) {
    ...
    ad = encodePeerTailControlAD({ branchId, circuitId, generation, direction: selectedDirection, counter: wireCounter })
    marker = CELL_CLASS_STREAM
  } else if (contextClass === PEER_TAIL_FINALIZE_CONTEXT_CLASS) {
    if (!validCounter(counter)) invalid()
    wireCounter = counter
    ad = encodePeerTailFinalizeAD({ circuitId, generation, direction: selectedDirection, counter: wireCounter })
  } else {
    if (!validCounter(counter)) invalid()
    wireCounter = counter
    const routeId = ownData(options, 'routeId')
    const selectedPurpose = ownData(options, 'purpose')
    ad = encodePeerRouteAD({ routeId, circuitId, generation, purpose: selectedPurpose, direction: selectedDirection, counter: wireCounter })
  }
