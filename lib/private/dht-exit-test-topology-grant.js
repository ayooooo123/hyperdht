...
const TEST_REFERRAL_GRANTS = new WeakMap()
const SPENT_TEST_REFERRAL_GRANTS = new WeakSet()
...
function createTestDhtExitReferralGrant(encoded, publicKey, expected) {
  const decoded = verifyTestIsolatedAddressGrant(encoded, publicKey, expected)
  const authority = Object.freeze({})
  try {
    TEST_REFERRAL_GRANTS.set(authority, {
      exitRole: decoded.exitRole,
      generation: decoded.generation,
      tupleDigest: fixed(decoded.tupleDigest, 32)
    })
    return authority
  } finally {
    decoded.runNonce.fill(0)
    decoded.tupleDigest.fill(0)
    decoded.signature.fill(0)
  }
}

function consumeTestDhtExitReferralGrant(authority, value) {
  const binding =
    authority !== null && typeof authority === 'object' ? TEST_REFERRAL_GRANTS.get(authority) : null
  if (!binding) {
    if (
      authority !== null &&
      typeof authority === 'object' &&
      SPENT_TEST_REFERRAL_GRANTS.has(authority)
    ) {
      throw PrivateRouteError.ERR_REPLAY()
    }
    authentication()
  }
  TEST_REFERRAL_GRANTS.delete(authority)
  SPENT_TEST_REFERRAL_GRANTS.add(authority)
  let digest = null
  try {
    exactOwnData(value, TUPLE_FIELDS)
    digest = digestTestIsolatedAddressTuple(value)
    if (
      binding.exitRole !== value.exitRole ||
      binding.generation !== value.generation ||
      !same(binding.tupleDigest, digest)
    ) {
      authentication()
    }
    return true
  } finally {
    binding.tupleDigest.fill(0)
    if (digest !== null) digest.fill(0)
  }
}
...
module.exports = Object.freeze({
  ...
  TEST_ONLY_DHT_EXIT_TOPOLOGY_ISSUER: Object.freeze({
    ...
    referralGrant: createTestDhtExitReferralGrant
  }),
