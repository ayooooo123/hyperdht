...
function createExtensionLinkCompletion(material, destroy) {
...
function takeExtensionLinkCompletion(capability) {
...
function destroyTakenExtensionLinkCompletion(material) {
...
function destroyExtensionLinkCompletion(capability) {
  const state = object(capability) ? COMPLETIONS.get(capability) : null
  if (!state) return false
  COMPLETIONS.delete(capability)
  SPENT.add(capability)
