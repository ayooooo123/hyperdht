...
// Format 1 appends authority32 to each endpoint.
const V1_ENDPOINT_EXTRA_SIZE = 32
...
function copyEndpoint(value) {
  return {
    ...
    ...(value.authority32 ? { authority32: b4a.from(value.authority32) } : {})
  }
}
...
// configured; the peer signature records the peer's consent. Link setup keys
// come from the peer's verified advertisement, as in the routed-DHT stack.
...
function normalizeEndpointV1(value) {
  if (!exactKeys(value, ['identity32', 'role', 'host', 'port', 'operations', 'authority32'])) {
    invalidRoute()
  }
  if (!nonZero32(value.authority32)) invalidRoute()
