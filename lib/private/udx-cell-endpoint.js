...
function createReconnectTransport(material, materialState) {
  ...
  const template = materialState.freshOwnerTemplate
  let owner = null
  try {
    if (!template) throw unavailable()
    if (template.testConstruction) {
      owner = new UdxCellEndpoint(template.options, {
    ...
    if (owner) void owner.close().catch(() => {})
    throw err instanceof PrivateRouteError ? err : unavailable()
  }
  const transportState = { active: true, owner, ready: null }
  transportState.ready = owner.bind().catch(async (err) => {
