...
function destroyRecordM3CellTransfers(record) {
  ...
  for (const state of Array.from(record.m3CellTransfers)) destroyM3CellLinkTransferState(state)
}
    ...
    const session = Object.freeze({
      async open() {
        ...
        TEST_BOOTSTRAP_ESTABLISHED.set(established, {
          ...
          admission: admitted,
          session,
          expiresAt: (encoded ? encoded.options : sessionOptions).setup.expiresAt,
          live: true
        })
