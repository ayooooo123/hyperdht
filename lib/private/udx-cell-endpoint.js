...
class UdxCellEndpoint {
  ...
  openLink(linkHandle, sessionOptions) {
    ...
    // translates in between, and rejects every link on a host behind a NAT. The
    // ownership test is unchanged in substance: the advertised pair is fixed when the
    // endpoint is constructed and is never taken from the handle.
    if (
