...
const PUNCH_TAG_ROLE = b4a.from('pr-role-punch/1\n')
const PUNCH_TAG_MESH = b4a.from('pr-mesh-punch/1\n')
const KNOWN_PUNCH_TAGS = [PUNCH_TAG, PUNCH_TAG_ROLE, PUNCH_TAG_MESH]
const DEFAULT_PUNCH_ROUNDS = 6
const DEFAULT_PUNCH_INTERVAL_MS = 500

function isPunchPacket(packet) {
  if (!b4a.isBuffer(packet) || packet.byteLength < PUNCH_TAG.byteLength) return false
  for (const tag of KNOWN_PUNCH_TAGS) {
    if (
      packet.byteLength >= tag.byteLength &&
      b4a.equals(packet.subarray(0, tag.byteLength), tag)
    ) {
      return true
    }
  }
...
function receive(state, packet, from) {
  ...
  if (packet.byteLength !== BOOTSTRAP_SIZE) {
    if (isPunchPacket(packet)) {
      if (state.onPunch) {
        try {
          state.onPunch(packet, from)
        } catch {}
      }
    }
    return
...
class UdxCellEndpoint {
  constructor(options = {}, construction = null) {
    ...
    const onPunch = options.onPunch !== undefined ? options.onPunch : null
    if (
      !adapter ||
      ...
      (onPunch !== null && typeof onPunch !== 'function')
    ) {
      invalid()
    }
  ...
  async punch(target, options = {}) {
    const state = ENDPOINTS.get(this)
    if (!state || state.closing || !state.bound) throw stateError()
    if (!isObject(target) || !isObject(options)) invalid()

    let host
    let port

    if (typeof target.host === 'string' && target.port !== undefined) {
      if (
        !numericHost(target.host) ||
      ...
      host = target.host
      port = target.port
    } else if (SEND_HANDLES.has(target)) {
      const record = SEND_HANDLES.get(target)
      if (!validateRecord(state, record)) throw PrivateRouteError.UNAUTHORIZED()
      host = record.peer.host
      port = record.peer.port
    } else {
      let link
      try {
        link = readLinkHandle(target)
      } catch {
        throw PrivateRouteError.UNAUTHORIZED()
      }
      if (
        !link ||
        !link.peerAddress ||
        !numericHost(link.peerAddress.host) ||
        !Number.isInteger(link.peerAddress.port) ||
        link.peerAddress.port < 1 ||
        link.peerAddress.port > 0xffff
      ) {
        throw PrivateRouteError.UNAUTHORIZED()
      }
    ...
    const rounds = options.rounds !== undefined ? options.rounds : DEFAULT_PUNCH_ROUNDS
    if (!Number.isSafeInteger(rounds) || rounds < 1 || rounds > 64) invalid()

    const interval = options.interval !== undefined ? options.interval : DEFAULT_PUNCH_INTERVAL_MS
    if (!Number.isSafeInteger(interval) || interval < 0) invalid()
    ...
    const tag = b4a.isBuffer(options.tag) ? options.tag : PUNCH_TAG
    const payload = b4a.isBuffer(options.payload)
      ? options.payload
      : b4a.concat([tag, b4a.from([1, 1])])
    ...
    for (let round = 1; round <= rounds; round++) {
      if (state.closing || (signal && signal.aborted)) break
      try {
...
function createBootstrapUdxGuardSessionOptions(authority, admission, reservation, options) {
  ...
  if (
    ...
    (options.punch !== undefined && typeof options.punch !== 'boolean') ||
  ...
  BOOTSTRAP_GUARD_SESSION_OPTIONS.set(capability, {
    ...
    options: (() => {
      ...
      if (options.punch !== undefined) opts.punch = options.punch
      if (options.punchRounds !== undefined) opts.punchRounds = options.punchRounds
      if (options.punchInterval !== undefined) opts.punchInterval = options.punchInterval
      return opts
    })()
  })
  return capability
...
function openBootstrapUdxGuard(authority, admission, linkHandle, sessionOptions) {
  ...
  if (encoded) {
    ...
    actualOptions = {
      ...
      ...(values.punch !== undefined ? { punch: values.punch } : {}),
      ...(values.punchRounds !== undefined ? { punchRounds: values.punchRounds } : {}),
      ...(values.punchInterval !== undefined ? { punchInterval: values.punchInterval } : {})
    }
  }
  const endpointState = ENDPOINTS.get(state.endpoint)
