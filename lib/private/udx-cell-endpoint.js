      ...
      if (messageId === 0x0301 || messageId === 0x0304) {
        ...
        if (reg && !reg.destroyed) {
          ...
          const sendReply = (replyPacket) => {
            ...
            return sendEndpointDirect(state.endpoint, sourceAddr.host, sourceAddr.port, replyPacket)
          }
          try {
...
function takePeerDirectRequesterTransport(transport) {
  ...
  const baseCapability = {
    ...
    send(packet1200) {
      ...
      return sendEndpointDirect(state.endpoint, state.targetHost, state.targetPort, packet1200)
    },
    onPacket(callback) {
...
function closePeerNeighborSession(session) {
  ...
  markPeerNeighborUnavailable(record)
  record.neighborClosePromise = Promise.resolve().then(async () => {
    try {
      await session.close()
    } catch {}
    await Promise.allSettled(Array.from(record.neighborNativeWaits))
    return true
  })
...
async function destroyPeerEstablishedLinkTeardown(established, reason = 0, options = {}) {
  if (!isObject(established)) return false
  if (typeof reason !== 'number' || !Number.isInteger(reason) || reason < 0 || reason > 3) {
    throw PrivateRouteError.INVALID_ROUTE()
  }
  const record = ESTABLISHED_SEND_HANDLES.get(established)
  if (!record || !record.endpoint) return false
  const session = record.session
  markPeerNeighborUnavailable(record)
  if (record.linkControl && record.phase === 'OPEN') {
    try {
      const destroyPromise = Promise.resolve(
        record.endpoint[UDX_LINK_DESTROY_CIRCUIT](record.handle, reason)
      )
      destroyPromise.catch(() => {})
      const schedule = options.schedule || (record.linkControl && record.linkControl.schedule)
      const cancel = options.cancel || (record.linkControl && record.linkControl.cancel)
      if (typeof schedule === 'function') {
        let timer = null
        await Promise.race([
          destroyPromise,
          new Promise((resolve) => {
            timer = schedule(resolve, 250)
          })
        ])
        if (timer !== null && typeof cancel === 'function') cancel(timer)
      } else {
        await Promise.race([destroyPromise, new Promise((resolve) => setTimeout(resolve, 250))])
      }
    } catch {}
  }
