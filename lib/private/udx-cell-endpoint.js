...
function destroyM3CellLinkTransferState(state) {
  ...
  if (record && record.m3CellTransfer === state) record.m3CellTransfer = null
  for (const waiter of (state.waiters || []).splice(0)) waiter.reject(PrivateRouteError.ERR_DESTROYED())
  ...
  state.binding = null
  if (state.transfer && endpointState && record) invalidateRecord(endpointState, record)
  state.endpoint = null
...
function invalidateRecord(state, record) {
  ...
  record.phase = 'CLOSED'
  destroyM3CellLinkTransferState(record.m3CellTransfer)
...
function receive(state, packet, from) {
  ...
  if (
    ...
    receiveM3CellLinkTransfer(authority.m3CellTransfer, owned)
  ) {
    return
  }
...
function openBootstrapUdxGuard(authority, admission, linkHandle, sessionOptions) {
  ...
  if (encoded) {
    ...
    actualOptions = {
      ...
      linkSetup: createLinkSetupAuthority({ now: values.now, randomBytes: values.randomBytes }),
      setup: {
  ...
  const session = state.endpoint.openLink(linkHandle, actualOptions)
  const endpointState = ENDPOINTS.get(state.endpoint)
...
function createReconnectTransport(material, materialState) {
  ...
  const transportState = { active: true, owner: null, ready: null }
  materialState.reconnectTransport = transportState
  transportState.ready = closeOriginalGuardOwner(materialState)
    .then(async () => {
      if (!transportState.active || GUARD_LEASE_MATERIALS.get(material) !== materialState)
        throw PrivateRouteError.UNAUTHORIZED()
      const owner = createFreshReconnectOwner(materialState.freshOwnerTemplate)
      transportState.owner = owner
      materialState.reconnectOwner = owner
      await owner.bind()
      if (!transportState.active || GUARD_LEASE_MATERIALS.get(material) !== materialState)
        throw PrivateRouteError.UNAUTHORIZED()
      return owner
  ...
  return Object.freeze({
    send(host, port, packet) {
      ...
      return transportState.ready
        .then((owner) => {
          return sendEndpointDirect(owner, materialState.host, materialState.port, packet, () => {
            return transportState.active && GUARD_LEASE_MATERIALS.get(material) === materialState
          })
        })
        .catch(async (err) => {
          transportState.active = false
...
function createM3CellLinkTransferIssuer(endpoint, established) {
  const state = isObject(endpoint) ? ENDPOINTS.get(endpoint) : null
  const record = isObject(established) ? ESTABLISHED_SEND_HANDLES.get(established) : null
  if (
    !state ||
...
function registerM3CellLinkTransfer(issuer, authenticatedBinding) {
  ...
  const transfer = Object.freeze({
    ...
    receive() {
      ...
      return new Promise((resolve, reject) => current.waiters.push({ resolve, reject }))
    },
    destroy() {
      const current = M3_CELL_LINK_TRANSFERS.get(transfer)
      return destroyM3CellLinkTransferState(current)
  ...
  try {
    ...
    record.m3CellTransfer = transferState
