...
const DHT_EXIT_READY = Object.freeze({
  domain: b4a.from('hyperdht-private-routes/m3/dht-exit-ready/v1'),
...
const DHT_EXIT_READY_SIGNERS = new WeakMap()
...
function createDhtExitReadySigner(identityOwner) {
  return createChild(identityOwner, DHT_EXIT_READY_SIGNERS, { dhtExitReady: null })
}
...
function signDhtExitReady(dhtExitReadySigner, body, expectedIdentityValue) {
  const state = signerState(dhtExitReadySigner, DHT_EXIT_READY_SIGNERS)
  return transact(state, (authority) =>
    signPhase(state, authority, 'dhtExitReady', DHT_EXIT_READY, body, expectedIdentityValue)
  )
}
...
function destroyDhtExitReadySigner(dhtExitReadySigner) {
  return destroyChild(dhtExitReadySigner, DHT_EXIT_READY_SIGNERS)
}
