...
class ActiveChallengeResponderAuthority {
  ...
  _expireBindings(now) {
    for (const [cacheKey, state] of this._cache) {
  ...
  admitCapsRetry(value) {
    ...
    try {
      ...
      const state = { query, advertisementDigest, binding, cacheKey, used: false }
  ...
  respond(binding, challenge, options) {
    ...
    try {
      ...
      signature = this._crypto.sign(input, identitySecret)
...
function projectionFor(state, record) {
...
class RelayCapabilityVerifier {
  ...
  accept(encoded, options) {
    ...
    try {
      ...
      if (history) {
        ...
        if (advertisement.epoch === history.epoch) {
          if (equal(advertisementDigest, history.digest)) {
            ...
            const projection = projectionFor(state, source)
      ...
      const projection = projectionFor(state, record)
  ...
  async beginChallenge(advertisement, sendChallenge) {
    ...
    if (authorityState.state !== state || authorityState.record !== record) {
