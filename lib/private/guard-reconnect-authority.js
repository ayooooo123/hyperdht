...
const reconnectRequests = new WeakMap()
...
function exactLegacyTransport(transport) {
  exactObject(transport, ['send', 'destroy'])
  if (
    typeof own(transport, 'send') !== 'function' ||
    typeof own(transport, 'destroy') !== 'function'
  )
    invalid()
  return transport
}

function exactReconnectTransport(transport) {
  exactObject(transport, ['reconnect', 'destroy'])
  if (
    !Object.isFrozen(transport) ||
    typeof own(transport, 'reconnect') !== 'function' ||
    typeof own(transport, 'destroy') !== 'function'
  )
    invalid()
  return transport
...
function consumeGuardReconnectRequest(request) {
  const record =
    request !== null && typeof request === 'object' ? reconnectRequests.get(request) : null
  if (!record || record.consumed) replay()
  const operation = record.operation
  if (!operation || operation.status !== 'ACTIVE') destroyed()
  record.consumed = true
  reconnectRequests.delete(request)
  return objectFreeze({
    guardIdentity: copy(operation.guardIdentity, 32),
    guardEndpoint: copy(operation.guardEndpoint, 19),
    advertisement: copy(operation.advertisement),
    advertisementDigest: copy(operation.advertisementDigest, 32),
    epoch: operation.epoch,
    expiresAt: operation.expiresAt,
    localIdentity: copy(operation.localIdentity, 32),
    localSecretKey: copy(operation.localSecretKey, 64),
    wallNow: operation.wallNow,
    monotonicNow: operation.monotonicNow,
    setTimer: operation.setTimer,
    clearTimer: operation.clearTimer,
    deadlineAt: operation.deadlineAt
  })
}

async function reconnectPinnedGuard(authority, operation) {
  const request = objectFreeze({})
  reconnectRequests.set(request, { operation, consumed: false })
  try {
    const transfer = await Promise.race([
      operation.transport.reconnect(request),
      operation.deadlinePromise
    ])
    if (reconnectRequests.has(request)) unauthorized()
    deadlineCheck(operation)
...
function createGuardReconnectAuthority(options) {
  ...
  try {
    ...
    if (typeof reconnectTransport !== 'function') {
      try {
        exactReconnectTransport(reconnectTransport)
      } catch {
        exactLegacyTransport(reconnectTransport)
      }
    }
    if (
      !u64(epoch) ||
      ...
      reconnect: function reconnectOperation() {
        ...
        try {
          ...
          if (transport === null) {
            ...
            try {
              exactReconnectTransport(transport)
            } catch {
              unavailable()
            }
          }
          ...
          return typeof operation.transport.reconnect === 'function'
            ? reconnectPinnedGuard(authority, operation)
            : reconnect(authority, operation)
        } catch (err) {
...
module.exports = {
  ...
  [GUARD_RECONNECT_REQUEST_CONSUMER]: consumeGuardReconnectRequest
}
