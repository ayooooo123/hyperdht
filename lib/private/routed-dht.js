...
function validateRoutedReplyStructure(encoded, request, requestBytes) {
  ...
  if (
    responseBytes > MAX_IMMUTABLE_RESPONSE_BYTES ||
...
function validateRoutedReplyForRequest(encoded, options) {
  ...
  try {
    ...
    validateRoutedReplyStructure(encoded, request, requestBytes)
    ...
    validateRoutedReplyStructure(ownedEncoded, request, requestBytes)
