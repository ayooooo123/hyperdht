...
// V2 envelope: replyMode(1)+batchId(16)+surbCount(1)+reserved(2)+requestBytes(2)+surbBytes(2)=24
const ROUTED_REQUEST_V2_ENVELOPE_SIZE = 24
const MAX_ROUTED_REQUEST_V2_BODY_SIZE =
  ROUTED_REQUEST_V2_ENVELOPE_SIZE +
  MAX_ROUTED_REQUEST_BODY_SIZE +
  M3_HEADER_SIZE +
  MAX_SURB_BATCH * SURB_DESCRIPTOR_SIZE
const MAX_ROUTED_REPLY_BODY_SIZE = MAX_ROUTED_REPLY_BYTES - M3_HEADER_SIZE
// V2 request envelopes can exceed the reply ceiling. Keep the allocator large
// enough for either, while each encoder/decoder enforces its own message bound.
const MAX_ENCODED_MESSAGE_SIZE =
  M3_HEADER_SIZE +
  (MAX_ROUTED_REQUEST_V2_BODY_SIZE > MAX_ROUTED_REPLY_BODY_SIZE
    ? MAX_ROUTED_REQUEST_V2_BODY_SIZE
    : MAX_ROUTED_REPLY_BODY_SIZE)
const MAX_ROUTED_REPLY_AMPLIFICATION = 4445
const MAX_IMMUTABLE_RESPONSE_BYTES = 1026
const MAX_MUTABLE_GET_RESPONSE_BYTES = 1023
const MAX_PUT_RESPONSE_BYTES = 1
const PUT_SUCCESS_ACK = 0x00
...
const REPLY_PROFILE = objectFreeze({
  [M3_MESSAGE_ID.IMMUTABLE_GET_V1]: objectFreeze({
    classes: objectFreeze([BRANCH_CLASS.LOOKUP, BRANCH_CLASS.ANNOUNCE]),
    maxResponse: MAX_IMMUTABLE_RESPONSE_BYTES,
    token: '0or32',
    closers: true
  }),
  [M3_MESSAGE_ID.MUTABLE_GET_V1]: objectFreeze({
    classes: objectFreeze([BRANCH_CLASS.LOOKUP, BRANCH_CLASS.ANNOUNCE]),
    maxResponse: MAX_MUTABLE_GET_RESPONSE_BYTES,
    token: '0or32',
    closers: true
  }),
  [M3_MESSAGE_ID.IMMUTABLE_PUT_V1]: objectFreeze({
    classes: objectFreeze([BRANCH_CLASS.ANNOUNCE]),
    maxResponse: MAX_PUT_RESPONSE_BYTES,
    token: 'zero',
    closers: false
  }),
  [M3_MESSAGE_ID.MUTABLE_PUT_V1]: objectFreeze({
    classes: objectFreeze([BRANCH_CLASS.ANNOUNCE]),
    maxResponse: MAX_PUT_RESPONSE_BYTES,
    token: 'zero',
    closers: false
...
function replyProfile(commandId) {
  const profile = REPLY_PROFILE[commandId]
  if (!profile) invalid()
  return profile
}

function profileAllowsClass(profile, operationClass) {
  for (let index = 0; index < profile.classes.length; index++) {
    if (profile.classes[index] === operationClass) return true
  }
  return false
}

function targetOf(commandId, encodedBody) {
  const bodyBytes = bufferLength(encodedBody)
  if (commandId === M3_MESSAGE_ID.IMMUTABLE_GET_V1) {
    if (bodyBytes !== 32) invalid()
    return copy(subarray(encodedBody, 0, 32))
  }
  if (commandId === M3_MESSAGE_ID.MUTABLE_GET_V1) {
    if (bodyBytes !== 40) invalid()
    return copy(subarray(encodedBody, 0, 32))
  }
  if (commandId === M3_MESSAGE_ID.IMMUTABLE_PUT_V1) {
    if (bodyBytes < 67 || bodyBytes > 1090) invalid()
    return copy(subarray(encodedBody, 32, 64))
  }
  if (commandId === M3_MESSAGE_ID.MUTABLE_PUT_V1) {
    if (bodyBytes < 171 || bodyBytes > 1066) invalid()
    return copy(subarray(encodedBody, 32, 64))
  }
  invalid()
}

function validateReplyPayload(profile, errorCode, tokenBytes, closerCount, encodedResponse) {
  const responseBytes = bufferLength(encodedResponse)
  if (responseBytes > profile.maxResponse) invalid()
  if (profile.token === 'zero' && tokenBytes !== 0) invalid()
  if (profile.token === '0or32' && tokenBytes !== 0 && tokenBytes !== 32) invalid()
  if (profile.closers === false && (tokenBytes !== 0 || closerCount !== 0)) invalid()
  if (errorCode === 0 && profile.maxResponse === MAX_PUT_RESPONSE_BYTES) {
    if (responseBytes !== 1 || encodedResponse[0] !== PUT_SUCCESS_ACK) invalid()
  }
}
...
function exactData(value, names) {
  ...
  const result = reflectApply(objectCreate, null, [null])
...
function allocate(size) {
  let output = null
  try {
    if (!numberIsSafeInteger(size) || size < 0 || size > MAX_ENCODED_MESSAGE_SIZE) {
      invalid()
    }
...
function encodeM3(messageId, body) {
  ...
  } else if (messageId === M3_MESSAGE_ID.ROUTED_REQUEST_V2) {
    minimum = ROUTED_REQUEST_V2_ENVELOPE_SIZE
    maximum = MAX_ROUTED_REQUEST_V2_BODY_SIZE
...
function encodeRoutedRequest(value) {
  ...
  try {
    ...
    // KI-15: this slot carries a RELATIVE budget in milliseconds, never an absolute deadline.
    // A monotonic clock is host-local, so an absolute value minted on one host would be
    // compared against an unrelated clock on another and every routed request refused. Each
    // side derives its own absolute deadline by adding this budget to its own clock.
    const operationBudgetMs = dataProperty(value, 'operationBudgetMs')
    ...
    if (!fixed(callerRequestId, 16) || !uint64(operationBudgetMs) || encodedBodyBytes < 0) {
      invalid()
    }
    ...
    writeUint64(body, operationBudgetMs, 39)
...
function decodeRoutedRequest(encoded) {
  ...
  try {
    ...
    result = objectFreeze({
      ...
      operationBudgetMs: readUint64(body, 39),
      destination,
      destinationEncoded,
      encodedBody
    })
...
function encodeRoutedRequestV2(value) {
  let batchId = null
  ...
  try {
    value = object(value)
    const replyMode = dataProperty(value, 'replyMode')
    const callerBatchId = dataProperty(value, 'batchId')
    const callerRequest = dataProperty(value, 'request')
    const surbDescriptors = dataProperty(value, 'surbDescriptors')

    if (replyMode !== REPLY_MODE.CORRELATED && replyMode !== REPLY_MODE.SURB_REQUIRED) invalid()
    if (!fixed(callerBatchId, 16)) invalid()
    requestBytes = copy(callerRequest)
    const requestLen = bufferLength(requestBytes)
    if (requestLen < M3_HEADER_SIZE + ROUTED_REQUEST_FIXED_BODY_SIZE) invalid()
    // Nested request must be a valid V1 routed request.
    decodeRoutedRequest(requestBytes)

    let surbCount = 0
    let surbBytes = 0
    if (replyMode === REPLY_MODE.CORRELATED) {
      if (surbDescriptors !== undefined && surbDescriptors !== null) {
        if (!isArray(surbDescriptors) || surbDescriptors.length !== 0) invalid()
      }
    } else {
      if (
        !isArray(surbDescriptors) ||
        surbDescriptors.length < 1 ||
        surbDescriptors.length > MAX_SURB_BATCH
      ) {
        invalid()
      }
      surbCount = surbDescriptors.length
      surbBytes = surbCount * SURB_DESCRIPTOR_SIZE
      descriptors = allocate(surbBytes)
      for (let i = 0; i < surbCount; i++) {
        let encoded = null
        try {
          encoded = encodeSurbDescriptor(surbDescriptors[i])
          set(descriptors, encoded, i * SURB_DESCRIPTOR_SIZE)
        } finally {
          clear(encoded)
        }
      }
    ...
    batchId = copy(callerBatchId)
    body = allocate(ROUTED_REQUEST_V2_ENVELOPE_SIZE + requestLen + surbBytes)
    body[0] = replyMode
    set(body, batchId, 1)
    body[17] = surbCount
    writeUint16(body, 0, 18)
    writeUint16(body, requestLen, 20)
    writeUint16(body, surbBytes, 22)
    set(body, requestBytes, ROUTED_REQUEST_V2_ENVELOPE_SIZE)
    if (surbBytes > 0) set(body, descriptors, ROUTED_REQUEST_V2_ENVELOPE_SIZE + requestLen)
    return encodeM3(M3_MESSAGE_ID.ROUTED_REQUEST_V2, body)
  } finally {
    clear(batchId)
    clear(requestBytes)
    clear(descriptors)
...
function decodeRoutedRequestV2(encoded) {
  let body = null
  let batchId = null
  let requestBytes = null
  let nested = null
  ...
  try {
    body = decodeM3(
      encoded,
      M3_MESSAGE_ID.ROUTED_REQUEST_V2,
      ROUTED_REQUEST_V2_ENVELOPE_SIZE,
      MAX_ROUTED_REQUEST_V2_BODY_SIZE
    )
    const replyMode = body[0]
    if (replyMode !== REPLY_MODE.CORRELATED && replyMode !== REPLY_MODE.SURB_REQUIRED) invalid()
    batchId = copy(subarray(body, 1, 17))
    const surbCount = body[17]
    if (readUint16(body, 18) !== 0) invalid()
    const requestLen = readUint16(body, 20)
    const surbBytes = readUint16(body, 22)
    if (bufferLength(body) !== ROUTED_REQUEST_V2_ENVELOPE_SIZE + requestLen + surbBytes) invalid()

    if (replyMode === REPLY_MODE.CORRELATED) {
      if (surbCount !== 0 || surbBytes !== 0) invalid()
    } else {
      if (surbCount < 1 || surbCount > MAX_SURB_BATCH) invalid()
      if (surbBytes !== surbCount * SURB_DESCRIPTOR_SIZE) invalid()
    }

    requestBytes = copy(
      subarray(body, ROUTED_REQUEST_V2_ENVELOPE_SIZE, ROUTED_REQUEST_V2_ENVELOPE_SIZE + requestLen)
    )
    nested = decodeRoutedRequest(requestBytes)

    const surbDescriptors = []
    if (surbCount > 0) {
      const start = ROUTED_REQUEST_V2_ENVELOPE_SIZE + requestLen
      for (let i = 0; i < surbCount; i++) {
        const offset = start + i * SURB_DESCRIPTOR_SIZE
        surbDescriptors[i] = decodeSurbDescriptor(
          subarray(body, offset, offset + SURB_DESCRIPTOR_SIZE)
        )
      }
    }
    ...
    result = objectFreeze({
      ...
      request: nested,
      requestBytes,
      surbDescriptors: objectFreeze(surbDescriptors),
      // Promote nested V1 fields for exit dispatch convenience.
      requestId: nested.requestId,
      operationClass: nested.operationClass,
      commandId: nested.commandId,
      commandVersion: nested.commandVersion,
      mutationFlag: nested.mutationFlag,
      destinationValidationClass: nested.destinationValidationClass,
      maxResponseBytes: nested.maxResponseBytes,
      maxAmplificationBytes: nested.maxAmplificationBytes,
      requestCost: nested.requestCost,
      responseCost: nested.responseCost,
      operationBudgetMs: nested.operationBudgetMs,
      destination: nested.destination,
      destinationEncoded: nested.destinationEncoded,
      encodedBody: nested.encodedBody
    })
    batchId = null
    requestBytes = null
    nested = null
    ...
    clear(requestBytes)
    if (!complete) {
      if (nested) clearRoutedRequest(nested)
      if (result) clearRoutedRequestV2(result)
    }
  }
}

function peekRoutedRequestMessageId(encoded) {
  const encodedBytes = bufferLength(encoded)
  if (encodedBytes < M3_HEADER_SIZE) invalid()
  if (readUint32(encoded, 0) !== 1) invalid()
  return readUint16(encoded, 4)
}

function decodeAnyRoutedRequest(encoded) {
  const messageId = peekRoutedRequestMessageId(encoded)
  if (messageId === M3_MESSAGE_ID.ROUTED_REQUEST_V1) {
    ...
    return objectFreeze({
      ...
      surbCount: 0,
      request: request,
      requestBytes: copy(encoded),
      surbDescriptors: objectFreeze([])
    })
  }
  if (messageId === M3_MESSAGE_ID.ROUTED_REQUEST_V2) return decodeRoutedRequestV2(encoded)
  invalid()
}

function clearRoutedRequestV2(request) {
  if (!request) return
  clear(clearDataProperty(request, 'batchId'))
  clear(clearDataProperty(request, 'requestBytes'))
  const nested = clearDataProperty(request, 'request')
  if (nested && nested !== request) clearRoutedRequest(nested)
  // Nested fields may be shared with promoted aliases; clearRoutedRequest handles owned buffers
  // only once when nested is cleared. Promoted aliases are the same buffers.
  const descriptors = clearDataProperty(request, 'surbDescriptors')
  if (descriptors) {
...
function encodeRoutedReply(value) {
  ...
  try {
    ...
    const profile = replyProfile(fields.commandId)
    if (!profileAllowsClass(profile, fields.operationClass)) invalid()
...
function decodeRoutedReply(encoded) {
  ...
  try {
    ...
    if (commandVersion !== 1) invalid()
    const profile = replyProfile(commandId)
    if (!profileAllowsClass(profile, operationClass)) invalid()
    ...
    validateReplyPayload(profile, errorCode, tokenBytes, closerCount, encodedResponse)
...
function validateRoutedReplyForRequest(encoded, options) {
  ...
  try {
    ...
    // V2 requests carry the nested V1 body; validation binds against that body.
    {
      const any = decodeAnyRoutedRequest(fields.encodedRequest)
    ...
    if (
      !fixed(target, 32) ||
      !fixed(circuitId, 16) ||
      (fields.branch !== BRANCH_CLASS.LOOKUP && fields.branch !== BRANCH_CLASS.ANNOUNCE) ||
      !uint64(fields.generation) ||
      !uint64(fields.now)
    ) {
      invalid()
    }
    const profile = replyProfile(request.commandId)
    let requestTarget = null
    try {
      requestTarget = targetOf(request.commandId, request.encodedBody)
    } catch {
      policyMismatch()
    }
    if (
    ...
    if (
      ...
      stagedStructure.responseBytes > profile.maxResponse
    ) {
      invalid()
    }
    ...
    if (
      ...
      bufferLength(reply.encodedResponse) > profile.maxResponse
    ) {
      invalid()
    }
    validateReplyPayload(
      profile,
      reply.errorCode,
      bufferLength(reply.token),
      reply.closerNodes.length,
...
function callbackDestination(request) {
  ...
  try {
    ...
    result = objectFreeze({
      ...
      operationBudgetMs: request.operationBudgetMs,
...
function validateRoutedRequestForExit(encoded, options) {
  ...
  try {
    ...
    request = decodeAnyRoutedRequest(encoded)
    // KI-15 left `current` UNREAD by the admission predicate below, which now compares two
    // durations. The sample is retained on purpose and MUST NOT be deleted as dead code. It
    // does two things nothing else on this path does: it validates that the exit's injected
    // clock returns a sane uint64 before a request is admitted, and it gives the reentrancy
    // checkpoint on the next line something to catch - a hostile `now` that reenters this
    // function. Remove the call and that checkpoint becomes vacuous, silently, with no test
    // failing.
    let current
    try {
      current = now()
    ...
    if (
      ...
      request.operationBudgetMs > bigIntConstructor(policy.timeoutMs)
    ) {
      policyMismatch()
    }
...
function clearRoutedRequest(request) {
  ...
  if (nested && nested !== request) {
    clearRoutedRequest(nested)
    return
  }
