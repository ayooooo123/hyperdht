...
function validateRoutedReplyForRequest(encoded, options) {
  ...
  try {
    const encodedBytes = bufferLength(encoded)
    reply = decodeRoutedReply(encoded)
    ...
    if (
      ...
      encodedBytes - bufferLength(fields.encodedRequest) > MAX_ROUTED_REPLY_AMPLIFICATION
    ) {
      invalid()
    }
    verifyRoutedReplyReferralAuthority(authority, {
      fromEncoded: reply.fromEncoded,
      target,
    ...
    for (let index = 0; index < reply.closerNodes.length; index++) {
      if (
        index > 0 &&
        compareCloser(reply.closerNodes[index - 1], reply.closerNodes[index], target) >= 0
      ) {
        invalid()
      }
    }
    for (let index = 0; index < reply.closerNodes.length; index++) {
      stageRoutedReplyReferral(authority, {
        encoded: reply.closerNodeEncoded[index],
        decoded: reply.closerNodes[index]
      })
    }
    if (bufferLength(reply.encodedResponse) > MAX_IMMUTABLE_RESPONSE_BYTES) invalid()
