...
function entriesArray(value) {
  try {
    ...
    if (length !== EXIT_ORIGIN_SERVICE_POLICY.length) invalid()
    return value
  } catch (err) {
    if (err instanceof PrivateRouteError) throw err
    invalid()
...
function encodeExitOriginServicePolicy(entries = EXIT_ORIGIN_SERVICE_POLICY) {
  ...
  try {
    ...
    output = allocate(POLICY_BYTES)
    writeUint16(output, EXIT_ORIGIN_SERVICE_POLICY.length, 0)
    let offset = 2
    for (let index = 0; index < EXIT_ORIGIN_SERVICE_POLICY.length; index++) {
...
function decodeExitOriginServicePolicy(encoded) {
  try {
    if (bufferLength(encoded) !== POLICY_BYTES || readUint16(encoded, 0) !== 9) invalid()
    const entries = []
    let offset = 2
    for (let index = 0; index < EXIT_ORIGIN_SERVICE_POLICY.length; index++) {
