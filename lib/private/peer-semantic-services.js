  ...
  try {
    for (const s of service.streams.values()) {
      while (s.controls.length && !service.destroyed) {
        ...
        if (next.afterHandshake && (s.queue.length || !acknowledged(s, s.handshakeTicket))) break
...
function admit(service, meta, nested) {
  ...
  try {
    ...
    if (decoded.messageId === ID.PEER_DATA_V2) {
      check(s.transportOpen && s.initialCredit && s.kind !== 'registration' && !s.remoteEnded)
...
function grant(s) {
  if (!s.transportOpen || s.closed || s.creditQueued || s.incoming.length || s.dataAdmissions.size)
    return
  ...
  // A queued grant authorizes nothing until the reliable lane issues it.
  s.creditQueued = true
  control(
    s,
    ID.PEER_CREDIT_V2,
    ...
    (ticket) => {
      if (s.closed) return
      s.localCredit = value
      s.creditQueued = false
      if (epoch === 1) s.initialCredit = ticket
    }
  )
}
...
function receiveData(s, meta, f) {
  ...
  check(
    s.transportOpen &&
      s.initialCredit &&
      admission &&
      equal(admission.digest, meta.nestedDigest) &&
      !s.remoteEnded
  )
  check(
    f.common.position === s.receivedBytes &&
...
function completeFin(s) {
  if (s.closed || !acknowledged(s, s.finTicket)) return
  s.finWait.resolve()
  if (s.remoteEnded) closeSession(s, 0, null, false, true)
}
...
function closeSession(s, code = 6, error = null, send = true, graceful = false) {
  ...
  if (graceful) {
    ...
    s.finWait.reject(failure)
    s.finishWait.reject(failure)
  }
