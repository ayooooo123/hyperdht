...
const TEST_ONLY_LINK_CIRCUIT_TEARDOWN_OBSERVER = Symbol('test-only-link-circuit-teardown-observer')
...
class LinkCircuitTeardown {
  ...
  add(capability) {
    ...
    state.records.add({ ...record, timer: null, status: 'OPEN' })
    return true
  }
  fail(link, direction) {
    const state = TEARDOWNS.get(this)
    ...
    for (const record of state.records) {
      ...
      try {
        timer = state.schedule(() => {
          ...
          if (record.status !== 'FAILING' || record.timer !== timer) return
          record.timer = null
          try {
            record.session.close('TRANSPORT_LOST')
          } finally {
            releaseCircuitRecord(state, record, false)
          }
      ...
      if (synchronous || timer === null || timer === undefined || state.closed) {
        const session = record.session
        record.timer = timer
        releaseCircuitRecord(state, record, true)
        session.close('TRANSPORT_LOST')
        throw unavailable()
      }
      record.timer = timer
  ...
  close() {
    ...
    state.closed = true
    for (const record of Array.from(state.records)) {
      const session = record.session
      releaseCircuitRecord(state, record, true)
      try {
        session.close()
  ...
  [TEST_ONLY_LINK_CIRCUIT_TEARDOWN_OBSERVER]() {
    const state = TEARDOWNS.get(this)
    let timers = 0
    for (const record of state.records) if (record.timer !== null) timers++
    return Object.freeze({
      records: state.records.size,
      timers,
      retainedCallbacks:
        Number(state.now !== null) +
        Number(state.schedule !== null) +
        Number(state.cancel !== null),
      closed: state.closed
    })
  }
}

function releaseCircuitRecord(state, record, cancel) {
  if (!record || record.status === 'CLOSED') return false
  record.status = 'CLOSED'
  const timer = record.timer
  record.timer = null
  try {
    if (cancel && timer !== null && state.cancel) state.cancel(timer)
  } catch {
  } finally {
