...
const { consumeTestDhtExitTopologyAuthority } = require('./dht-exit-test-topology-grant')
...
function referralCandidate(value) {
  exactOwnData(value, ['id', 'host', 'port'])
  const candidate = configuredTuple({ host: value.host, port: value.port })
...
function reserveProbeSettlement(state, kind, candidate, deadline, issuerEntry = null) {
  ...
  const result = createSettlement(
    ...
    candidate,
    deadline,
    { command: 0, internal: true, target: null, token: null },
    issuerEntry,
    probeKey
  )
...
function settleExitDhtReservation(settlementAuthority, correlatedReplyAuthority) {
  ...
  if (settlement.kind === 'ORDINARY_DHT_REQUEST') {
    if (correlated.message.command !== 9 || correlated.message.internal !== false) authentication()
    if (correlated.reply.error !== 0) return null
...
function consumeDhtExitImmutableGetCompletionForIO(completion) {
  const state = takeCompletion(completion)
  const candidates = []
  for (const candidate of state.reply.closerNodes.slice(0, MAX_REPLY_PROBES)) {
    if (!globallyRoutableIpv4(candidate.host)) continue
    candidates.push(
      Object.freeze({
...
function reserveReferralProbe(table, referralReplyAuthority, candidateValue, absoluteDeadline) {
  ...
  const candidate = referralCandidate(candidateValue)
...
function verifyDhtExitRoutedDestination(table, value) {
  const state = tableState(table)
  exactOwnData(value, [
    'destination',
    'destinationEncoded',
  ...
  const deadline = uint64(value.absoluteDeadlineMs)
...
function reserveOrdinaryDhtRequest(table, destinationRef, requestSpec, absoluteDeadline) {
  ...
  if (requestSpec.command !== 9) invalid()
  return createSettlement(
    ...
    deadline,
    {
      ...
      token: copyNullable32(requestSpec.token)
    },
    entry
  )
}
