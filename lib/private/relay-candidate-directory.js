...
// Every supported count accepts more than half of its one- or two-byte range.
// 128 consecutive honest rejections are therefore below 2^-128, while this bound
// keeps a hostile entropy capability from retaining directory ownership forever.
const MAX_RANDOM_DRAWS = 128
...
const SOURCE_FIELDS = Object.freeze(['wallNow', 'monotonicNow', 'randomBytes'])
...
function clockOwnerIsLive(state) {
  ...
  return (
    ...
    typeof state.randomBytes === 'function'
  )
}
...
function sampleDirectoryIndex(directory, state, suspended, count) {
  if (!Number.isSafeInteger(count) || count < 1 || count > 0x1_0000) {
    invalidate(state)
    incompatible()
  }
  if (count === 1) return 0
  const bytes = count <= 0x100 ? 1 : 2
  const range = bytes === 1 ? 0x100 : 0x1_0000
  const limit = range - (range % count)
  for (let attempt = 0; attempt < MAX_RANDOM_DRAWS; attempt++) {
    let source = null
    try {
      if (state.sampling || typeof state.randomBytes !== 'function') {
        invalidate(state)
        incompatible()
      }
      state.sampling = true
      try {
        source = state.randomBytes(bytes)
      } catch {
        invalidate(state)
        incompatible()
      } finally {
      ...
      if (bufferLength(source) !== bytes) {
        invalidate(state)
        incompatible()
      }
      requireDirectoryState(directory, state, suspended)
      const value = bytes === 1 ? source[0] : source[0] * 0x100 + source[1]
...
function recordsConflict(left, right) {
  return equal(left.identity, right.identity) || endpointSubnetEqual(left.endpoint, right.endpoint)
}

function recordDiverseFrom(record, excluded) {
  for (const other of excluded) {
    if (recordsConflict(record, other)) return false
  }
  return true
}

function pairDiverse(middle, exit, excluded) {
  return (
    recordDiverseFrom(middle, excluded) &&
    recordDiverseFrom(exit, excluded) &&
    !recordsConflict(middle, exit)
  )
}

function pairsDiverse(firstMiddle, firstExit, secondMiddle, secondExit, excluded) {
  return (
    pairDiverse(firstMiddle, firstExit, excluded) &&
    pairDiverse(secondMiddle, secondExit, excluded) &&
    !recordsConflict(firstMiddle, secondMiddle) &&
    !recordsConflict(firstMiddle, secondExit) &&
    !recordsConflict(firstExit, secondMiddle) &&
    !recordsConflict(firstExit, secondExit)
  )
}

function searchPairs(directory, state, middles, exits, excluded) {
  let count = 0
    ...
    for (const firstExit of exits) {
      for (const secondMiddle of middles) {
        for (const secondExit of exits) {
          if (pairsDiverse(firstMiddle, firstExit, secondMiddle, secondExit, excluded)) count++
        }
      }
    }
  }
  if (count === 0) return null
  let selectedIndex = sampleDirectoryIndex(directory, state, state.suspended, count)
  for (const firstMiddle of middles) {
    for (const firstExit of exits) {
      for (const secondMiddle of middles) {
        for (const secondExit of exits) {
          if (!pairsDiverse(firstMiddle, firstExit, secondMiddle, secondExit, excluded)) continue
...
function choosePairs(directory, state, now, excluded = []) {
  ...
  let chosen = searchPairs(directory, state, middles.preferred, exits.preferred, excluded)
  if (chosen === null && demotionApplies(middles, exits)) {
    chosen = searchPairs(directory, state, middles.all, exits.all, excluded)
  }
  if (chosen === null) incompatible()
  return chosen
}

function searchPair(directory, state, middles, exits, excluded) {
  let count = 0
  for (const middle of middles) {
    for (const exit of exits) {
      if (pairDiverse(middle, exit, excluded)) count++
    }
  }
  if (count === 0) return null
  let selectedIndex = sampleDirectoryIndex(directory, state, state.suspended, count)
  for (const middle of middles) {
    for (const exit of exits) {
      if (!pairDiverse(middle, exit, excluded)) continue
      if (selectedIndex-- === 0) return { middle, exit }
    }
  }
  invalidate(state)
  incompatible()
...
function chooseReplacementPair(directory, state, now, excluded) {
  requireLiveGuard(state, now)
  pruneInvalidRecords(state, now)
  const middles = validCandidates(state, ROLE.SAFETY, RELAY_CAPABILITY.CIRCUIT_RELAY_V1)
  const exits = validCandidates(
  ...
  let chosen = searchPair(directory, state, middles.preferred, exits.preferred, excluded)
  if (chosen === null && demotionApplies(middles, exits)) {
    chosen = searchPair(directory, state, middles.all, exits.all, excluded)
  }
  if (chosen === null) incompatible()
  return chosen
}
...
class RelayCandidateDirectory {
  ...
  reserveInitialPair(options) {
    ...
    const pairs = choosePairs(this, state, now)
  ...
  reserveReplacement(options) {
    ...
    const selected = chooseReplacementPair(this, state, now, excluded)
    now = sampleDirectoryWall(this, state, false)
    revalidateSelection(state, now, [
      oppositePair.middle,
      oppositePair.exit,
  ...
  [kInspectRelayCandidateDirectory]() {
    ...
    return objectFreeze({
      ...
      callbackCount:
        ...
        Number(typeof state.randomBytes === 'function'),
...
function consumeSealedRelayCandidateDirectory(token) {
  ...
  try {
    ...
    const state = {
      ...
      randomBytes: sealed.randomBytes,
