...
function dynamicBytesField(name, lengthFrom, minimum, maximum, options = {}) {
  return Object.assign(
    { name, kind: 'bytes', size: null, lengthFrom, minimum, maximum },
    options
  )
}
...
function bodySize(currentSchema, values) {
  ...
  if (!Number.isSafeInteger(size) || size < currentSchema.minimumBodyBytes || size > currentSchema.maximumBodyBytes) {
    invalid()
  }
  return size
}
...
function decodeDescriptor(body, field, values, offset) {
  if (field.kind === 'common') return { value: decodeCommonObject(body, offset, field.stable), size: 40 }
...
function decodeFieldsBody(body, currentSchema) {
  ...
  if (
    length < currentSchema.minimumBodyBytes ||
    length > currentSchema.maximumBodyBytes
  ) {
    invalid()
  }
  const fields = {}
...
function validateRouteClose(value) {
  if (!nonzeroBuffer(value.routeId) || value.generation === 0n || !nonzeroBuffer(value.closeNonce)) {
    invalid()
  }
}
...
const SCHEMAS = new Map([
  ...
  [
    ...
    schema(
      ...
      72,
      0,
      [bytesField('queryNonce', 32), scalarField('cookieExpiresAt', 'u64'), bytesField('returnCookie', 32)],
...
function decodePeerTransport(wire) {
  ...
  try {
    ...
    if (bufferLength(decoded.body) < 0 || bufferLength(decoded.authSuffix) !== current.suffixBytes) {
      invalid()
    }
