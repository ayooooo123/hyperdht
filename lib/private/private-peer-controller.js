...
class PrivatePeerController {
  ...
  async _publishNow(server) {
    ...
    const seq = await this._nextDescriptorSequence(server, publisherRelayPublicKey)
    ...
    let publisher = null
    try {
      attachment = await this._connectRelay(entryRelayPublicKey, server._keyPair)
  ...
  async _nextDescriptorSequence(server, publisherRelayPublicKey) {
    const stream = await this._connectRelay(publisherRelayPublicKey)
    try {
      const request = b4a.allocUnsafe(33)
      request[0] = OP_SEQUENCE
      server._keyPair.publicKey.copy(request, 1)
      await writeAll(stream, request)
      const response = await readExactly(stream, 9)
      if (response[0] !== STATUS_OK) throw unavailable()
      const current = readU64(response, 1)
      const next = current > server._seq ? current + 1n : server._seq + 1n
      if (next > 0xffffffffffffffffn) throw unavailable()
      server._seq = next
      return next
    } finally {
      stream.end()
  ...
  async _handleRelayConnection(stream) {
    ...
    if (opcode === OP_SEQUENCE) return this._handleSequence(stream)
    throw invalid()
  }

  async _handleSequence(stream) {
    const destinationPublicKey = await readExactly(stream, 32)
    let seq = 0n
    try {
      seq = (await this.descriptors.get(destinationPublicKey)).seq
    } catch {}
    const response = b4a.alloc(9)
    response[0] = STATUS_OK
    writeU64(response, seq, 1)
