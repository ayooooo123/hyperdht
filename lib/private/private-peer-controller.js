...
function observe(controller, event) {
  ...
  try {
    controller.observer(Object.freeze(event))
  } catch {}
}
...
class CircuitLink extends EventEmitter {
  constructor(controller, rawStream, context, initiator, label) {
    ...
    if (buffered) this._receive(buffered)
  }

  send(payload) {
    if (this.destroyed) return Promise.reject(unavailable())
    ...
    sending.catch((error) => this.destroy(error))
    return sending
  }
...
function transformBridge(controller, left, right) {
  let closed = false
  ...
  left.on('payload', (payload) => {
    ...
    right.send(payload).catch(close)
  })
  right.on('payload', (payload) => {
    observe(controller, {
      type: 'route-transform',
      from: right.label,
      to: left.label,
...
class LogicalRouteStream extends Duplex {
  ...
  _write(data, callback) {
    ...
    const send = async () => {
      while (offset < data.byteLength) {
        ...
        this.bytes += length
        if (this.bytes > MAX_STREAM_BYTES) throw unavailable()
  ...
  _destroy(callback) {
    if (!this.localEnded && !this.link.destroyed) {
  ...
  _drain() {
    while (this.queue.length > 0) {
      if (!this.push(this.queue.shift())) return
    }
    if (this.remoteEnded) this.push(null)
  }

  remoteReset() {
    if (!this.destroyed) this.destroy(unavailable())
  }
}
...
class DestinationCircuit {
  ...
  _open(streamId) {
    if (this.streams.has(streamId) || this.streams.size >= MAX_ROUTE_STREAMS) throw invalid()
    const raw = new LogicalRouteStream(this.link, streamId, () => this.streams.delete(streamId))
...
class EntryCircuit {
  ...
  _fromSource(record, payload) {
    try {
      ...
      record.bytes += message.data.byteLength
      if (record.bytes > MAX_STREAM_BYTES) throw unavailable()
  ...
  _receive(payload) {
    try {
      ...
      if (!record || message.flags === LOGICAL_OPEN) throw invalid()
...
class PrivatePeerController {
  ...
  async _nextDescriptorSequence(server) {
    const id = key(server._keyPair.publicKey)
    let previous = this.lastSequences.get(id) || server._seq
    try {
      const remote = await this.descriptors.get(server._keyPair.publicKey)
      if (remote.seq > previous) previous = remote.seq
    } catch {}
  ...
  async _publishNow(server) {
    ...
    const seq = await this._nextDescriptorSequence(server)
    ...
    let connection = null
    try {
      connection = await this._connectRelay(destinationGuardPublicKey, server._keyPair)
      const request = b4a.concat([
        b4a.from([OP_BUILD_DESTINATION]),
        descriptor,
  ...
  async _openSession(destinationPublicKey) {
    ...
    for (const safetyRelayPublicKey of relays) {
      ...
      try {
        const descriptor = await this._resolveThrough(safetyRelayPublicKey, destinationPublicKey)
        if (
          same(safetyRelayPublicKey, descriptor.entryRelayPublicKey) ||
          same(safetyRelayPublicKey, descriptor.destinationGuardPublicKey)
        ) {
          continue
        }
        connection = await this._connectRelay(safetyRelayPublicKey)
        localContext = createRouteContext()
        downstreamContext = createRouteContext()
        const request = b4a.concat([
  ...
  _onRelayConnection(stream) {
    ...
    this._handleRelayConnection(stream).catch(async () => {
      try {
        if (!stream.destroyed) await status(stream, STATUS_UNAVAILABLE)
      } catch {}
      stream.destroy()
    })
  }
  ...
  async _handleBuildDestination(stream) {
    ...
    if (!same(stream.remotePublicKey, descriptor.destinationPublicKey)) throw invalid()
    const localContext = decodeRouteContext(await readExactly(stream, ROUTE_CONTEXT_BYTES))
    const downstreamContext = decodeRouteContext(await readExactly(stream, ROUTE_CONTEXT_BYTES))
  ...
  async _handleEntryRegister(stream) {
    ...
    const context = decodeRouteContext(await readExactly(stream, ROUTE_CONTEXT_BYTES))
    const token = key(descriptor.routeToken)
    const current = this.entryCircuits.get(token)
    if (current) current.destroy(unavailable())
  ...
  async _handleBuildSource(stream) {
    ...
    try {
      ...
      if ((await readExactly(next, 1))[0] !== STATUS_OK) throw unavailable()
      await status(stream)
      const left = new CircuitLink(this, stream, localContext, false, 'guard-source')
      const right = new CircuitLink(this, next, downstreamContext, true, 'guard-entry-source')
      const bridge = transformBridge(this, left, right)
      this.bridges.add(bridge)
    } catch (error) {
      clearRouteContext(localContext)
