...
class DestinationCircuit {
  ...
  _open(streamId) {
    if (
      ...
      this.streams.size >= MAX_ROUTE_STREAMS
    ) {
      throw invalid()
    }
...
class PrivatePeerController {
  ...
  async _publishNow(server) {
    ...
    try {
      ...
      await writeAll(
        connection,
        b4a.concat([b4a.from([OP_BUILD_DESTINATION]), server._keyPair.publicKey])
      )
      ...
      const seq = this._nextDescriptorSequence(server, previous)
  ...
  async _handleBuildDestination(stream) {
    const destinationPublicKey = await readExactly(stream, 32)
    if (!same(stream.remotePublicKey, destinationPublicKey)) throw invalid()
    ...
    try {
      ...
      const recovery = b4a.allocUnsafe(2 + (previous ? DESCRIPTOR_BYTES : 0))
