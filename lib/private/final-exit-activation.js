...
function claimFinalExitActivation(handoff, claim) {
  ...
  let owner = null
  let complete = false
  let tailControlModule = null
  try {
    const { getFinalExitHandoffTailControl } = require('./final-exit-handoff')
    tailControlModule = getFinalExitHandoffTailControl(handoff)
    owner = Object.freeze({})
    OWNERS.set(owner, {
      tailControlModule,
      material: null,
    ...
    const { prepareTailControlFinalExitActivation, commitTailControlFinalExitActivation } =
      tailControlModule
    ...
    if (!complete) {
      if (prepared) {
        const { revokeTailControlFinalExitActivation } = tailControlModule
...
function destroyFinalExitActivationOwner(owner) {
  ...
  if (material && object(material.tailControl)) {
    try {
      const { destroyTailControlFinalExitActivation } = record.tailControlModule
