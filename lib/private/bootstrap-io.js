...
const TEST_TIMER_FIELDS = Object.freeze(['setTimeout', 'clearTimeout'])
...
async function send(state, endpoint, message) {
  ...
  if (!transport || typeof transport.send !== 'function' || deadline === null) destroyed()
  const response = await Promise.race([
    Promise.resolve().then(() => transport.send(endpoint.host, endpoint.port, message)),
    deadline
  ])
  checkBudget(state)
  return response
...
class BootstrapIO {
  ...
  [TEST_ONLY_BOOTSTRAP_IO_OBSERVER](testTimers) {
    ...
    if (testTimers !== undefined) {
      ...
      exactObject(testTimers, TEST_TIMER_FIELDS)
      const testSetTimer = own(testTimers, 'setTimeout')
      const testClearTimer = own(testTimers, 'clearTimeout')
      if (typeof testSetTimer !== 'function' || typeof testClearTimer !== 'function') invalid()
