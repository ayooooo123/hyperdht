...
const { BOOTSTRAP_SIZE } = require('./bootstrap-envelope')
const { cryptoSuite } = require('./crypto-suite')
const { PrivateRouteError } = require('./errors')
const { RELAY_CAPABILITY, ROLE, roleForIdentity } = require('./protocol')
const {
...
const RECONNECT_BOOTSTRAP_IO_FACTORY = Symbol.for(
  'hyperdht-private-routes/reconnect-bootstrap-io-factory'
)
...
const PRE_GUARD_CONTACT_FIELDS = Object.freeze(['host', 'port', 'outcome'])
...
function unavailable(cause) {
  const error = PrivateRouteError.ERR_PRIVATE_GUARD_UNAVAILABLE()
...
function checkBudget(state) {
  ...
  if (now < state.startedAt || now >= state.deadlineAt) unavailable()
...
async function send(state, target, message) {
  checkBudget(state)
  ...
  if (
    ...
    !isBootstrapUdxAuthority(transport) ||
    deadline === null ||
    state.deadlinePromise !== deadline ||
    !state.deadlineArmed ||
    ...
    const response = await Promise.race([
      defer(() => {
        ...
        return target.kind === 'configured'
          ? requestConfigured(transport, target.index, message)
          : requestProspectiveGuard(transport, target.admission, message)
      }),
      deadline
    ])
    checkBudget(state)
...
function armDeadline(state, operationGeneration) {
  ...
  try {
    timer = scheduleTimer(expired, Number(state.deadlineAt - state.startedAt))
  } catch {
    arming = false
    stop(state)
    unavailable()
...
function begin(io, state, operationGeneration) {
  ...
  if (state.deadlineAt === null) state.deadlineAt = state.startedAt + state.budget
  if (state.deadlineAt <= state.startedAt || state.deadlineAt > BigInt(Number.MAX_SAFE_INTEGER)) {
    unavailable()
  }
  try {
    bindBootstrapUdxOperation(
      state.datagrams,
      Number(state.deadlineAt),
      operationGeneration,
      state.monotonicNow,
      state.startedAt
    )
...
async function queryCaps(state, target) {
  ...
  try {
    ...
    const response = await send(state, target, capsQueryRequest(randomTarget, queryNonce))
...
async function challengeProjection(state, target, verifier, projection, session) {
  await send(state, target, capsRetryRequest(session, projection.canonicalBytes)).then(
    exactEmptyResponse
  )
  const authority = createActiveChallengeSendAuthority({
    ...
    async send(challenge) {
      const response = exactResponse(
        await send(state, target, bytesRequest('active-challenge', challenge)),
        'activeChallenge'
      )
...
function expose(state, phase, category, endpoint, outcome, attempt = true) {
  let digest = null
  let input = null
  try {
...
function stop(state) {
  ...
  try {
    if (datagrams) destroyBootstrapUdxAuthority(datagrams)
  } catch {}
...
function clearPinnedGuard(pinnedGuard) {
  ...
  clear(pinnedGuard.advertisement)
...
function consumeBootstrapGuardPin(transfer) {
  ...
  try {
    ...
    advertisement = copy(state.pinnedGuard.advertisement)
...
async function authenticateProspectiveGuard(state, record, endpoint, admission) {
  let session = null
  let verifier = null
  state.challengeInFlight++
  if (state.challengeInFlight !== 1) unavailable()
  try {
    expose(state, 'challenge', 'prospective-guard', endpoint, 'pending')
    const target = { kind: 'prospective', admission }
    session = await queryCaps(state, target)
    ...
    const validated = await challengeProjection(state, target, verifier, projection, session)
    if (!sameRecordProjection(record, validated)) unavailable()
    expose(state, 'challenge', 'prospective-guard', endpoint, 'authenticated', false)
    return true
  } catch (err) {
    expose(state, 'challenge', 'prospective-guard', endpoint, 'rejected', false)
    throw err
  } finally {
...
async function establishGuardLink(state, record, endpoint, admission) {
  let circuitId = null
  let initiatorLocalId = null
  let responderLocalId = null
  let session = null
  try {
    const wallNow = checkBudget(state)
    const monotonicNow = sample(state, 'monotonicNow')
    if (monotonicNow > BigInt(Number.MAX_SAFE_INTEGER)) unavailable()
    const remaining = record.expiresAt - wallNow
    if (remaining <= 0n) unavailable()
    const signedExpiry = monotonicNow + remaining
    if (signedExpiry > BigInt(Number.MAX_SAFE_INTEGER)) unavailable()
    circuitId = random(state, 16)
    initiatorLocalId = random(state, 16)
    responderLocalId = random(state, 16)
    const linkResponse = exactResponse(
      await send(
        state,
        { kind: 'prospective', admission },
        bytesRequest('link', b4a.alloc(BOOTSTRAP_SIZE))
      ),
      'link'
    )
    const reservation = own(linkResponse, 'reservation')
    const absoluteDeadline = state.deadlineAt
    const establishedMonotonicNow = state.monotonicNow
    const establishedRandomBytes = state.randomBytes
    const sessionRandomBytes = (size) => {
      let source = null
      try {
        source = establishedRandomBytes(size)
        if (!fixed(source, size)) unavailable()
        return copy(source, size)
      } catch {
        unavailable()
      } finally {
    ...
    const sessionNow = () => {
      let value
      try {
        value = establishedMonotonicNow()
      } catch {
        unavailable()
      }
      if (!u64(value) || value > BigInt(Number.MAX_SAFE_INTEGER)) unavailable()
      return Number(value)
    }
    const sessionOptions = createBootstrapUdxGuardSessionOptions(
      ...
      reservation,
      {
        ...
        responderStaticKey: record.routePublicKey,
        now: sessionNow,
        handleNow: state.monotonicNow,
        wallNow: () => {
          const value = sample(state, 'wallNow')
          if (value > BigInt(Number.MAX_SAFE_INTEGER)) unavailable()
          return Number(value)
        },
        schedule: state.deadlineSetTimer,
        cancel: state.deadlineClearTimer,
        randomBytes: sessionRandomBytes,
        absoluteDeadline: Number(absoluteDeadline),
        signedExpiry: Number(signedExpiry)
      }
    )
    session = openBootstrapUdxGuard(state.datagrams, admission, reservation, sessionOptions)
    ...
    clear(circuitId)
    clear(initiatorLocalId)
    clear(responderLocalId)
  }
}
...
async function run(io, state, operationGeneration) {
  ...
  let pinnedAdvertisement = null
  let pinnedDigest = null
  let pinnedGuard = null
  let lastRejection = null
  try {
    for (let configuredIndex = 0; configuredIndex < state.endpoints.length; configuredIndex++) {
      const endpoint = state.endpoints[configuredIndex]
      if (state.challengeCount >= MAX_GUARD_CHALLENGES) break
      let session = null
      let records = null
      try {
        checkBudget(state)
        // Retain attempted exposure even if cancellation erases bootstrap keys
        // before the response settles. Completion does not count a second attempt.
        expose(state, 'bootstrap', 'configured-bootstrap', endpoint, 'pending')
        session = await queryCaps(state, { kind: 'configured', index: configuredIndex })
        records = collectVerifiedRecords(state, session)
        const candidates = retainVerifiedRecords(state, records)
        expose(state, 'bootstrap', 'configured-bootstrap', endpoint, 'verified', false)
        for (let pass = 0; pass < 2 && established === null; pass++) {
          for (const candidate of candidates) {
            ...
            let admission = null
            try {
              admission = admitBootstrapUdxGuard(state.datagrams, {
                identity: candidate.identity,
                host: guardEndpoint.host,
                port: guardEndpoint.port
              })
              await authenticateProspectiveGuard(state, candidate, guardEndpoint, admission)
              attemptEstablished = await establishGuardLink(
                state,
                candidate,
                guardEndpoint,
                admission
              )
              chosen = { record: candidate, endpoint: guardEndpoint, admission }
              ...
              // The authority owns and revokes any unpinned established session.
            }
          }
        }
        ...
        lastRejection = err
        try {
          expose(state, 'bootstrap', 'configured-bootstrap', endpoint, 'rejected', false)
        } catch {}
      } finally {
        clearCapsSession(session)
    ...
    if (!chosen || !established) unavailable(lastRejection)
    ...
    pinnedAdvertisement = null
    pinnedIdentity = null
    pinnedEndpoint = null
    pinnedDigest = null
    lease = pinBootstrapUdxGuard(state.datagrams, chosen.admission, established)
    established = null
...
class BootstrapIO {
  constructor(options) {
    ...
    try {
      ...
      if (
        !isBootstrapUdxAuthority(datagrams) ||
        ...
        invalid()
      let rawExposureKey = null
      try {
        rawExposureKey = randomBytes(32)
        exposureKey = copy(rawExposureKey, 32)
      } finally {
        clear(rawExposureKey)
      }
      states.set(this, {
        owner: this,
        endpoints,
  ...
  exposureReport() {
    return copyReport(states.get(this).exposure)
  }

  recordPreGuardContact(options) {
    const state = states.get(this)
    if (!state || state.destroyed) return
    exactObject(options, PRE_GUARD_CONTACT_FIELDS)
    const outcome = own(options, 'outcome')
    if (outcome !== 'observed' && outcome !== 'silent' && outcome !== 'rejected') {
      invalid()
    }
    const host = own(options, 'host')
    const port = own(options, 'port')
    const endpoint = ownEndpoint({ host, port })
    try {
      expose(state, 'reflect', 'reflector', endpoint, outcome)
    } finally {
      clear(endpoint.canonical)
    }
  }
...
function createReconnectBootstrapIO(options, absoluteDeadline) {
  if (!u64(absoluteDeadline) || absoluteDeadline > BigInt(Number.MAX_SAFE_INTEGER)) invalid()
  const io = new BootstrapIO(options)
  states.get(io).deadlineAt = absoluteDeadline
  return io
}
...
module.exports = {
  ...
  [RECONNECT_BOOTSTRAP_IO_FACTORY]: createReconnectBootstrapIO
}
