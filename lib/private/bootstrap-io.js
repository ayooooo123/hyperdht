...
function consumeBootstrapGuardPin(transfer) {
  ...
  try {
    if (!leaseMaterials.has(lease)) replay()
  ...
  try {
    ...
    for (const endpoint of state.endpoints) {
      ...
      try {
        ...
        let record = null
        for (let pass = 0; pass < 2 && record === null; pass++) {
          for (const candidate of records) {
            ...
            const key = b4a.toString(candidate.identity, 'hex')
        ...
        if (record === null || state.challengeCount > MAX_GUARD_CHALLENGES) unavailable()
        const guardEndpoint = endpointForRecord(record)
        if (guardEndpoint === null) unavailable()
        await authenticateProspectiveGuard(state, record, guardEndpoint)
        attemptEstablished = await establishGuardLink(state, record, guardEndpoint)
        for (const candidate of records) {
          const key = b4a.toString(candidate.identity, 'hex')
          state.recordKeys.set(key, candidate)
          state.records.push(candidate)
        }
        records = null
        chosen = { record, endpoint: guardEndpoint }
