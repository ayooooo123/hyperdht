    ...
    const response = await Promise.race([
      defer(() => {
        ...
        return target.kind === 'configured'
          ? sendConfigured(transport, target.index, message)
          : sendProspectiveGuard(transport, target.admission, message)
      }),
      deadline
    ])
    checkBudget(state)
...
function begin(io, state, operationGeneration) {
  ...
  const absoluteDeadline = state.startedAt + BOOTSTRAP_BUDGET
  if (absoluteDeadline > BigInt(Number.MAX_SAFE_INTEGER)) unavailable()
  try {
    bindBootstrapUdxOperation(state.datagrams, Number(absoluteDeadline), operationGeneration)
  } catch {
    stop(state)
    unavailable()
  }
...
async function establishGuardLink(state, record, endpoint, admission) {
  ...
  try {
    ...
    const absoluteDeadline = state.startedAt + BOOTSTRAP_BUDGET
    const sessionNow = () => {
      const value = sample(state, 'monotonicNow')
