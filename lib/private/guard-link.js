...
function finalizeRelayDialAuthority(state, terminal) {
  ...
  if (
    state.factoryState &&
    state.factoryState !== TERMINAL_EXTENSION_ADJACENT_BINDING
  ) {
...
function dialRelayAdvertisement(authority, options) {
  ...
  try {
    exactOwnData(options, RELAY_DIAL_OPTIONS)
  } catch (err) {
    finalizeRelayDialAuthority(state, 'SPENT')
    throw err
  }
...
function cleanupExtensionAdjacentFactoryInputs(values) {
  ...
  const ownsAuthority =
    authorityState &&
    authorityState.phase === 'UNUSED' &&
    authorityState.factoryState === null
...
function snapshotExtensionResponderOptions(options) {
  ...
  try {
    ...
    const signerDescriptor = getOwnPropertyDescriptorIntrinsic(
      options,
      'extensionResponderSigner'
    )
