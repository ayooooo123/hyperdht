...
function minU64(first, ...rest) {
  let value = first
  for (const candidate of rest) if (candidate < value) value = candidate
  return value
}
...
// Peer-minted wall timestamps are checked for authenticated ordering only: an
// offer whose deadline has passed on this host is refused, but no upper bound is
// measured against this host's clock (KI-4: that comparison rejected honest
// offers from a peer whose clock ran ahead). What bounds the deadline locally is
// `requestedLimits.expiresAtMs >= offerDeadlineMs` together with
// `limitsWithinAdvertisement`, which caps the requested expiry at 300 s from this
// host's clock. The replay reservation therefore covers the whole window in which
// this host would accept the offer, and the extension responder clamps the
// admitted lifetime it signs to MAX_ADJACENT_LINK_MS on its own clock.
  ...
  responder = Object.freeze({
    accept() {
      ...
      try {
        ...
        clear(randomScratch)
        randomScratch = null
        // The admitted lifetime is bounded on this host's clock, not the peer's:
        // a slower responder shortens the link rather than refusing it.
        admittedLimits = encodeLimits({
          ...offer.requestedLimits,
          expiresAtMs: minU64(
            offer.requestedLimits.expiresAtMs,
            offer.offerDeadlineMs,
            current + MAX_ADJACENT_LINK_MS
          )
        })
        body = b4a.allocUnsafeSlow(LINK_ACCEPT_BODY_SIZE)
...
function completeExtensionLink(pending, options = {}) {
  ...
  try {
    ...
    // Order peer timestamps against authenticated bounds, not this host's clock.
    if (
      !validSignature ||
      ...
      accept.acceptedAtMs >= accept.admittedLimits.expiresAtMs ||
