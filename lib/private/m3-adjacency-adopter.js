...
function createM3ResponderAdopter(adopt, destroy) {
...
function isM3ResponderAdopter(value) {
  return object(value) && ADOPTERS.has(value)
}

function adoptM3ResponderLink(adopter, established) {
...
function takeM3ResponderLink(adoption) {
...
function destroyTakenM3ResponderLink(adopter, value) {
  const owner = object(adopter) ? ADOPTERS.get(adopter) : null
  if (!owner) authentication()
  if (!object(value)) invalid()
  try {
...
function destroyM3ResponderLink(adoption) {
  const state = object(adoption) ? ADOPTIONS.get(adoption) : null
