...
class PrivateRoutingController {
  ...
  async immutableGet(target, opts = undefined) {
    ...
    // passed AND controller was created with experimentalSurbReplies: true.
    const replyMode = resolveReplyMode(state, opts)
    if (replyMode === REPLY_MODE.SURB_REQUIRED) {
      state.routedDHTIO.setReplyMode(REPLY_MODE.SURB_REQUIRED)
    }
  ...
  async mutableGet(publicKey, opts = {}) {
    ...
    if (opts && Object.prototype.hasOwnProperty.call(opts, 'replyMode')) {
      const mode = resolveReplyMode(state, { replyMode: opts.replyMode })
      void mode
    }
