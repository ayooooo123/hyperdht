      ...
      try {
        authority = transition(
          ...
          () => {
            ...
            // `testManualBuild` means the test owns branch construction. It has
            // always covered the initial build, `beginRouteBuildWithTransfer`; a
            // replacement build ran regardless, so no test could drive a rotation
            // without a live guard, middle and exit to answer it.
            return state.testManualBuild
              ? null
              : state.routeManager.claimReplacementBuild(branchClass)
          }
        )
      } catch (err) {
