  ...
  switch (signal.kind) {
    ...
    case 'announceBranchExpiry': {
      ...
      if (state.state !== PRIVATE_ROUTING_STATE.READY || !state.routeManager) {
        // Refusing outside READY is correct; losing the trigger with it is not.
        // An expiry is a standing condition, so give the manager a fresh sink for
        // the slot this delivery emptied and let its own clock redeliver. A branch
        // loss is a past event with no timer behind it: that class keeps its
        // expiry timer as a backstop and the gap is KI-10's, not this path's.
