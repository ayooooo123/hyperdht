...
function assertQueryReady(state) {
  ...
  if (state.state !== PRIVATE_ROUTING_STATE.READY || !state.transportDHT || !state.queryContexts) {
    unavailable()
  }
}
...
function enterUnavailable(state) {
  ...
  state.endpointResources = null
  state.unavailablePromise = Promise.resolve().then(() => cleanupUnavailable(state, endpointResources))
