...
class PrivateRoutingController {
  ...
  async immutableGet(target, opts = undefined) {
    ...
    // SURB_REQUIRED only when explicitly passed AND the controller was created
    // with experimentalSurbReplies: true; the hold covers the query's retry too,
    // so a required get never downgrades.
    ...
    let query = null
    try {
      query = state.transportDHT.query(
