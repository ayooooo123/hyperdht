...
const {
  ...
  renewRouteManagerBranchLoss,
  redeliverRouteManagerBranchLosses,
...
function assertQueryReady(state) {
...
function admitQueryGeneration(state, commandName, branchName, commitCommandName, replyMode) {
  assertQueryReady(state)
  const dht = state.transportDHT
  const io = state.routedDHTIO
  const contexts = state.queryContexts
  if (!dht || !io || !contexts) unavailable()
  const primaryContext = contexts[commandName] && contexts[commandName][branchName]
  const commitContext =
    commitCommandName !== null && contexts[commitCommandName]
      ? contexts[commitCommandName][branchName]
      : null
  if (!primaryContext || (commitCommandName !== null && !commitContext)) unavailable()

  const releaseReplyMode =
    replyMode === REPLY_MODE.SURB_REQUIRED ? io.holdRequiredReplyMode() : null

  let admission = null
  try {
    admission = io.admitQuery(primaryContext, commitContext)
  } catch (err) {
    if (releaseReplyMode !== null) releaseReplyMode()
    throw err
  }
  ...
  const releaseAll = () => {
    ...
    state.attemptHolds.delete(record)
    try {
      if (releaseReplyMode !== null) releaseReplyMode()
    } finally {
      try {
  ...
  record.releaseAll = releaseAll
  state.attemptHolds.add(record)

  return {
    dht,
    io,
    context: admission.context,
    commitContext: admission.commitContext,
    record,
    releaseAll
  }
}

function isRotationClassError(err) {
  if (!err || typeof err !== 'object') return false
  return (
    err.code === 'ERR_PRIVATE_BRANCH_ROTATING' ||
    err.code === 'ERR_DESTROYED' ||
...
function mapCommitError(err) {
  ...
  if (err.code === 'ERR_RECORD_CONFLICT') throw err
  if (err.error === ROUTED_ERROR.RECORD_CONFLICT || err.code === ROUTED_ERROR.RECORD_CONFLICT) {
    const conflict = new Error('Record conflict')
    conflict.code = 'ERR_RECORD_CONFLICT'
    throw conflict
  }
  throw err
}
...
function exactOptions(options) {
  ...
  let experimentalSurbReplies = false
  for (const key of keys) {
    if (key === 'endpointBootstrapAuthority') {
      const descriptor = Object.getOwnPropertyDescriptor(options, 'endpointBootstrapAuthority')
      if (!descriptor || !Object.hasOwn(descriptor, 'value')) invalid()
      sawAuthority = true
      continue
    }
    if (key === 'experimentalSurbReplies') {
      const descriptor = Object.getOwnPropertyDescriptor(options, 'experimentalSurbReplies')
      if (!descriptor || !Object.hasOwn(descriptor, 'value')) invalid()
      if (descriptor.value !== true && descriptor.value !== false) invalid()
      experimentalSurbReplies = descriptor.value === true
      continue
    }
    invalid()
  }
  if (!sawAuthority) invalid()
  return { experimentalSurbReplies }
}

function resolveReplyMode(state, opts) {
  if (opts === undefined || opts === null) return REPLY_MODE.CORRELATED
  if (!isObject(opts) || Object.getPrototypeOf(opts) !== Object.prototype) invalid()
  const keys = Reflect.ownKeys(opts)
  if (keys.length === 0) return REPLY_MODE.CORRELATED
  if (keys.length !== 1 || keys[0] !== 'replyMode') invalid()
  const descriptor = Object.getOwnPropertyDescriptor(opts, 'replyMode')
  if (!descriptor || !Object.hasOwn(descriptor, 'value')) invalid()
  const mode = descriptor.value
  if (mode === REPLY_MODE.CORRELATED || mode === 'CORRELATED') return REPLY_MODE.CORRELATED
  if (mode === REPLY_MODE.SURB_REQUIRED || mode === 'SURB_REQUIRED') {
    if (!state.experimentalSurbReplies) unavailable()
    return REPLY_MODE.SURB_REQUIRED
  }
  invalid()
}
...
// KI-9. A branch expiry signal is one-shot and the manager drops its timer before
// issuing it, so a delivery refused outside READY used to destroy the branch's only
// rotation trigger: that class never rotated again for the life of the controller.
// Mint a replacement sink for the same kind at the current signal generation and
// let the manager re-arm. A stale-generation signal never reaches here, so a
// renewal cannot resurrect a trigger for a superseded generation. Whether the
// rotation in flight then succeeds or fails is irrelevant: on the failure path the
// manager is destroyed with the controller and the retry stops with it, which is
// why this lives on the manager's clock rather than in a queue keyed on the next
// READY that an UNAVAILABLE controller would never reach.
function renewRefusedBranchExpiry(state, kind, branchClass) {
  if (!state.routeManager) return false
  const sink = createSignal(state, kind)
  try {
    return renewRouteManagerBranchExpiry(state.routeManager, branchClass, sink)
  } catch {
    revokeSignal(state, sink)
    return false
  }
}

function beginBranchRotation(state, branchClass, expected) {
  const authority = transition(state, expected, PRIVATE_ROUTING_STATE.ROTATING, () => {
    ...
    return state.testManualBuild ? null : state.routeManager.claimReplacementBuild(branchClass)
  })
  if (authority !== null) {
...
function pendingBranchLoss(manager) {
  if (hasRouteManagerBranchLoss(manager, BRANCH_CLASS.LOOKUP)) return BRANCH_CLASS.LOOKUP
  if (hasRouteManagerBranchLoss(manager, BRANCH_CLASS.ANNOUNCE)) return BRANCH_CLASS.ANNOUNCE
  return null
}
...
async function handleQueuedSignal(signal) {
  ...
  switch (signal.kind) {
    ...
    case 'announceBranchExpiry': {
      ...
      const loss = signal.kind === 'lookupBranchLoss' || signal.kind === 'announceBranchLoss'
      // Loss signals wake the controller; only the current manager-owned branch
      // can establish the fault. A queued wakeup cannot lose a replacement.
      if (
        loss &&
        (!state.routeManager || !hasRouteManagerBranchLoss(state.routeManager, branchClass))
      ) {
        return false
      }
      if (state.state !== PRIVATE_ROUTING_STATE.READY || !state.routeManager) {
        if (loss) {
          const sink = createSignal(state, signal.kind)
          if (!renewRouteManagerBranchLoss(state.routeManager, branchClass, sink)) {
            revokeSignal(state, sink)
          }
        }
        // Expiry retries use the manager clock. Loss stays on the branch until
        // replacement or teardown and is redelivered at the next READY.
        if (signal.kind === 'lookupBranchExpiry' || signal.kind === 'announceBranchExpiry') {
          renewRefusedBranchExpiry(state, signal.kind, branchClass)
        }
        return false
      }
      try {
        beginBranchRotation(state, branchClass, PRIVATE_ROUTING_STATE.READY)
      } catch (err) {
    ...
    case 'pairReady':
      ...
      if (state.state === PRIVATE_ROUTING_STATE.READY && state.routeManager) {
        redeliverRouteManagerBranchLosses(state.routeManager)
      }
      return true
...
function issuePrivateRoutingControllerSignal(sink) {
  ...
  Promise.resolve().then(async () => {
    try {
      ...
      if (!state.destroying && state.state === PRIVATE_ROUTING_STATE.BUILDING) {
...
const TEST_ONLY_PRIVATE_ROUTING_CONTROLLER_ISSUER = Object.freeze({
  ...
  installReadyForSuspend(controller, options) {
    const state = stateFor(controller)
    if (
      state.state !== PRIVATE_ROUTING_STATE.OFF ||
      !isObject(options) ||
      Reflect.ownKeys(options).length !== 4 ||
      !Object.hasOwn(options, 'routeManager') ||
      !Object.hasOwn(options, 'routedDHTIO') ||
      !Object.hasOwn(options, 'liveRouteAuthority') ||
      !Object.hasOwn(options, 'guardLease') ||
      !isRouteManager(options.routeManager) ||
      !isObject(options.routedDHTIO) ||
      typeof options.routedDHTIO.suspend !== 'function' ||
      typeof options.routedDHTIO.destroy !== 'function' ||
      !isObject(options.liveRouteAuthority) ||
      typeof options.liveRouteAuthority.destroy !== 'function'
    ) {
      invalid()
    }
    state.routeManager = options.routeManager
    state.routedDHTIO = options.routedDHTIO
    state.liveRouteAuthority = options.liveRouteAuthority
    state.guardLease = options.guardLease
    state.queryContexts = Object.freeze({})
    transition(state, PRIVATE_ROUTING_STATE.OFF, PRIVATE_ROUTING_STATE.READY)
    return true
  },
  ...
  bindSurbReturnPath(controller, branch, hops) {
    const state = stateFor(controller)
    if (!state.liveRouteAuthority) throw PrivateRouteError.UNAUTHORIZED()
    return bindSurbReturnPath(state.liveRouteAuthority, branch, hops)
  },
  liveRouteReturnPath(controller, branch) {
    const state = stateFor(controller)
    if (!state.liveRouteAuthority) return null
    return liveRouteReturnPath(state.liveRouteAuthority, branch)
  },
  deliverSurbTerminal(controller, terminalCell) {
    const state = stateFor(controller)
    if (!state.liveRouteAuthority) invalid()
    return deliverSurbTerminalToLiveRoute(state.liveRouteAuthority, terminalCell)
  },
  holdQueryAttempt(controller) {
    return admitQueryGeneration(
      stateFor(controller),
      'immutableGet',
      'lookup',
      null,
  ...
  swapTransportDHT(controller, dht) {
    const state = stateFor(controller)
    const previous = state.transportDHT
...
function beginRouteBuildWithTransfer(state, resources, transfer, generations = null) {
  ...
  try {
    if (transfer.exposureReport) state.exposureReport = transfer.exposureReport
    revokeSignals(state)
...
async function destroyTransportGeneration(state) {
  ...
  for (const record of state.attemptHolds) {
    if (dht && record.dht !== dht) continue
    record.releaseAll()
  }
  const queries = []
  for (const [query, record] of state.activeQueries) {
    if (record.dht !== dht) continue
...
function enterUnavailable(state) {
  if (state.unavailablePromise !== null) return state.unavailablePromise
  if (
    !state.destroying &&
    state.state !== PRIVATE_ROUTING_STATE.UNAVAILABLE &&
    state.state !== PRIVATE_ROUTING_STATE.DESTROYED
  ...
  state.rotationBranch = null
  if (state.endpointResources) {
    state.exposureReport = state.endpointResources.bootstrapIO.exposureReport()
  }
  const endpointResources = state.endpointResources
  state.endpointResources = null
  state.unavailablePromise = Promise.resolve().then(() =>
    cleanupUnavailable(state, endpointResources)
  )
  return state.unavailablePromise
}

async function cleanupUnavailable(state, endpointResources) {
  ...
  if (state.readyReject) {
    state.readyReject(state.branchBuildError || PrivateRouteError.ERR_PRIVACY_UNAVAILABLE())
    state.readyResolve = null
    state.readyReject = null
  }
}

function hopFromBranchCandidate(candidate) {
  if (!candidate || typeof candidate !== 'object') invalid()
  const pp = candidate.payloadParameters
  if (!pp || typeof pp !== 'object') invalid()
  if (
    !b4a.isBuffer(candidate.identity) ||
    candidate.identity.byteLength !== 32 ||
    !b4a.isBuffer(pp.routeEncryptionPublicKey) ||
    pp.routeEncryptionPublicKey.byteLength !== 32 ||
    typeof pp.epoch !== 'bigint' ||
    typeof pp.issuedAtMs !== 'bigint' ||
    typeof pp.expiresAtMs !== 'bigint'
  ) {
    invalid()
  }
  return Object.freeze({
    id: b4a.from(candidate.identity),
    routeKey: b4a.from(pp.routeEncryptionPublicKey),
    capabilityEpoch: pp.epoch,
    issuedAtMs: pp.issuedAtMs,
    expiresAtMs: pp.expiresAtMs
  })
}

function bindProductionSurbReturnPaths(state) {
  if (!state.liveRouteAuthority || !state.routeManager || !state.guardLease) return false
  const { readBranchPath } = require('./route-manager')
  const guardHop = readGuardSurbHop(state.guardLease, state.wallNow)
  for (const branchClass of [BRANCH_CLASS.LOOKUP, BRANCH_CLASS.ANNOUNCE]) {
    let path = null
    try {
      path = readBranchPath(state.routeManager, branchClass)
    } catch {
      continue
    }
    if (!path || !path.middle) continue
    const middleHop = hopFromBranchCandidate(path.middle)
    // Reverse SURB onion: exit peels toward middle, then guard, then endpoint.
    bindSurbReturnPath(state.liveRouteAuthority, branchClass, [middleHop, guardHop])
  }
  return true
}

function assertGenerationInstall(state, expected, manager, previousDHT) {
  if (state.destroying || state.state === PRIVATE_ROUTING_STATE.DESTROYED) destroyed()
  if (
    state.state !== expected ||
    state.routeManager !== manager ||
    state.transportDHT !== previousDHT
  ) {
    unavailable()
  }
}
...
async function installReadyGeneration(state) {
  ...
  const manager = state.routeManager
  ...
  try {
    if (expected === PRIVATE_ROUTING_STATE.ROTATING && oldDht) {
      const draining = []
      for (const record of state.attemptHolds) {
        if (record.dht !== oldDht) continue
        draining.push(record.done)
      }
      await Promise.all(draining)
    }
    assertGenerationInstall(state, expected, manager, oldDht)
    authority =
    ...
    let lostBranch = pendingBranchLoss(manager)
    if (lostBranch === null) {
      try {
        await routed.ready()
      } catch (err) {
        assertGenerationInstall(state, expected, manager, oldDht)
        lostBranch = pendingBranchLoss(manager)
        if (lostBranch === null) throw err
      }
    }
    assertGenerationInstall(state, expected, manager, oldDht)
    lostBranch = pendingBranchLoss(manager)
    if (lostBranch !== null) {
      // Ownership may transfer through a lost sibling, but query authority may
      // not. Retain only the staged owner while replacing the next failed branch.
      ...
      assertGenerationInstall(state, expected, manager, null)
      redeliverRouteManagerBranchLosses(manager)
      beginBranchRotation(state, lostBranch, expected)
      return false
    }
    dht = new DHT({
    ...
    state.lookupGeneration = committedGenerations.lookupGeneration
    state.announceGeneration = committedGenerations.announceGeneration
    // Bind SURB return hops from the published branch path + guard pin (production).
    bindProductionSurbReturnPaths(state)
    ...
    if (
      ...
      state.state === expected &&
      state.routeManager === manager &&
      state.transportDHT === (staged ? null : oldDht) &&
      (!staged || state.liveRouteAuthority === authority)
    ) {
      state.branchBuildError = err
      await enterUnavailable(state)
    }
...
class PrivateRoutingController {
  async ready() {
    ...
    if (
      ...
      unavailable()
    if (state.state === PRIVATE_ROUTING_STATE.OFF) await this.start()
    else if (state.readyPromise !== null) await state.readyPromise
    assertQueryReady(state)
  }

  exposureReport() {
    const state = stateFor(this)
    return state.endpointResources
      ? state.endpointResources.bootstrapIO.exposureReport()
      : state.exposureReport
  }
    ...
    state.startPromise = Promise.resolve()
      ...
      .catch(async (err) => {
        ...
        if (state.state !== PRIVATE_ROUTING_STATE.DESTROYED && !state.destroying) {
          ...
          if (owned) state.exposureReport = owned.bootstrapIO.exposureReport()
          state.endpointResources = null
  ...
  async immutableGet(target, opts = undefined) {
    const state = stateFor(this)
    assertQueryReady(state)
    if (!b4a.isBuffer(target) || target.byteLength !== 32) {
      unavailable()
    }
    const replyMode = resolveReplyMode(state, opts)
    const session = admitQueryGeneration(state, 'immutableGet', 'lookup', null, replyMode)
    let query = null
    try {
      query = session.dht.query(
        { target, command: COMMANDS.IMMUTABLE_GET, value: null },
        {
          transportContext: session.context,
          concurrency: 1,
          retries: 1
        }
      )
      state.activeQueries.set(query, session.record)
  ...
  async immutablePut(value, opts = {}) {
    const state = stateFor(this)
    assertQueryReady(state)
    if (!b4a.isBuffer(value) || value.byteLength > IMMUTABLE_GET_MAX_VALUE) {
    ...
    while (attempt < 2) {
      ...
      let query = null
      try {
        session = admitQueryGeneration(state, 'immutableGet', 'announce', 'immutablePut', replyMode)
            ...
            commit(reply, dht) {
              return dht
                .request(
                  ...
                  reply.from,
                  { transportContext: commitContext }
                )
                .then((response) => {
                  if (response && response.error) mapCommitError(response)
                  return response
                })
            }
          }
        ...
        await query.finished()
        return Object.freeze({
          hash: b4a.from(target),
          closestNodes: query.closestNodes
        })
      } catch (err) {
        if (isRotationClassError(err) && attempt < 2) continue
        if (isRotationClassError(err)) unavailable()
        throw err
      } finally {
        try {
          if (query !== null) {
          ...
          if (session !== null) session.releaseAll()
        }
      }
    }
  }
  ...
  async mutableGet(publicKey, opts = {}) {
    const state = stateFor(this)
    assertQueryReady(state)
    if (!b4a.isBuffer(publicKey) || publicKey.byteLength !== 32) unavailable()
    const refresh = opts && opts.refresh ? opts.refresh : null
    const userSeq = opts && opts.seq ? opts.seq : 0
    const latest = !opts || opts.latest !== false
    // SURB_REQUIRED only when explicit and experimentalSurbReplies is on.
    // Refresh commits are still restricted to correlated mode.
    const replyMode =
      opts && Object.prototype.hasOwnProperty.call(opts, 'replyMode')
        ? resolveReplyMode(state, { replyMode: opts.replyMode })
        : REPLY_MODE.CORRELATED
    if (replyMode === REPLY_MODE.SURB_REQUIRED && refresh) unavailable()
    const target = cryptoSuite.hash([publicKey])
    const branchName = refresh ? 'announce' : 'lookup'
    const commitName = refresh ? 'mutablePut' : null
    const session = admitQueryGeneration(state, 'mutableGet', branchName, commitName, replyMode)
    let signed = null
    let result = null
    let query = null
    try {
      const commitContextAdmitted = session.commitContext
      query = session.dht.query(
            ...
            ? (reply, dht) => {
                ...
                return signed
                  ? dht.request(
                      ...
                      reply.from,
                      { transportContext: commitContextAdmitted }
                    )
      ...
      for await (const node of query) {
        ...
        if (!latest) {
          return Object.freeze({
            ...
            signature: b4a.from(node.signature),
            from: node.from,
            token: node.token === null || node.token === undefined ? null : b4a.from(node.token)
          })
        }
      ...
      return Object.freeze({
        seq: result.seq,
        value: b4a.from(result.value),
        signature: b4a.from(result.signature),
        from: result.from,
        token: result.token === null || result.token === undefined ? null : b4a.from(result.token)
      })
    } finally {
      try {
        if (query !== null) {
  ...
  async mutablePut(keyPair, value, opts = {}) {
    ...
    if (value.byteLength > MUTABLE_PUT_MAX_VALUE) throw unsupportedCommand()
    const replyMode =
      opts && Object.prototype.hasOwnProperty.call(opts, 'replyMode')
        ? resolveReplyMode(state, { replyMode: opts.replyMode })
        : REPLY_MODE.CORRELATED
    const signMutable = (opts && opts.signMutable) || Persistent.signMutable
    const seq = (opts && opts.seq) || 0
    const target = cryptoSuite.hash([keyPair.publicKey])
    ...
    while (attempt < 2) {
      ...
      let query = null
      try {
        session = admitQueryGeneration(state, 'mutableGet', 'announce', 'mutablePut', replyMode)
        const commitContext = session.commitContext
        query = session.dht.query(
        ...
        if (err && err.code === 'ERR_RECORD_CONFLICT') throw err
        if (isRotationClassError(err) && attempt < 2) continue
        if (isRotationClassError(err)) unavailable()
        mapCommitError(err)
      } finally {
  ...
  suspend() {
    ...
    const operation = (async () => {
      let reconnect = null
      try {
        if (state.routedDHTIO) await state.routedDHTIO.suspend()
        reconnect = await state.routeManager.suspend()
        await destroyTransportGeneration(state)
        clearBranchConnections(state)
  ...
  async resume() {
    ...
    try {
      ...
      transfer = Object.freeze({
        ...
        exposureReport: moved.exposureReport
      })
      directory.resume()
      state.suspendedDirectory = null
      ...
      state.resumeGenerations = null
      if (state.readyReject) {
        // Settle the generation before cleanup can reenter readiness or destroy.
        state.readyReject(err)
        state.readyResolve = null
        state.readyReject = null
      }
  ...
  async networkChanged() {
    ...
    if (state.suspendPromise !== null) {
      const suspension = state.suspendPromise
      if (state.routeManager) {
        try {
          state.routeManager.destroy()
        } catch {}
      ...
      if (state.state === PRIVATE_ROUTING_STATE.UNAVAILABLE) return true
    }
    if (state.reconnect) {
      const reconnect = state.reconnect
      state.reconnect = null
  ...
  destroy() {
    ...
    state.destroyPromise = Promise.resolve().then(async () => {
      const suspension = state.suspendPromise
      if (suspension !== null && state.routeManager) {
        try {
          state.routeManager.destroy()
        } catch {}
      }
      ...
      if (state.unavailablePromise !== null) {
        try {
          await state.unavailablePromise
        } catch {}
      }
      let resources = state.endpointResources
      if (resources) state.exposureReport = resources.bootstrapIO.exposureReport()
      state.endpointResources = null
      if (!resources) {
        try {
      ...
      if (state.readyReject) {
        state.readyReject(PrivateRouteError.ERR_DESTROYED())
        state.readyResolve = null
        state.readyReject = null
      }
      await destroyTransportGeneration(state)
