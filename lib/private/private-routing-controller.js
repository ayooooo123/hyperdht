...
class PrivateRoutingController {
  ...
  async immutableGet(target, opts = undefined) {
    ...
    // Puts stay correlated in this slice. SURB_REQUIRED only when explicitly
    // passed AND the controller was created with experimentalSurbReplies: true;
    // the hold covers the query's retry too, so a required get never downgrades.
    const replyMode = resolveReplyMode(state, opts)
    const releaseReplyMode =
      replyMode === REPLY_MODE.SURB_REQUIRED ? state.routedDHTIO.holdRequiredReplyMode() : null
  ...
  async immutablePut(value) {
    ...
    while (attempt < 2) {
      ...
      const query = state.transportDHT.query(
        { target, command: COMMANDS.IMMUTABLE_GET, value: null },
        {
          transportContext: state.queryContexts.immutableGet.announce,
          concurrency: 1,
          retries: 1,
          map: mapImmutable,
          commit(reply, dht) {
            return dht
              .request(
                ...
                reply.from,
                { transportContext: state.queryContexts.immutablePut.announce }
              )
              .then((response) => {
  ...
  async mutableGet(publicKey, opts = {}) {
    ...
    // SURB_REQUIRED only when explicit and experimentalSurbReplies is on. A
    // refresh commit stays correlated (puts are correlated in this slice), so a
    // required mode is refused together with `refresh`.
    const replyMode =
    ...
    const branchContext = refresh
      ? state.queryContexts.mutableGet.announce
      : state.queryContexts.mutableGet.lookup
    let signed = null
    let result = null
    const query = state.transportDHT.query(
      { target, command: COMMANDS.MUTABLE_GET, value: c.encode(c.uint, userSeq) },
      {
        transportContext: branchContext,
        concurrency: 1,
        retries: 1,
        map: mapMutable,
        commit: refresh
          ? (reply, dht) => {
              ...
              return signed
                ? dht.request(
                    ...
                    reply.from,
                    { transportContext: state.queryContexts.mutablePut.announce }
                  )
  ...
  async mutablePut(keyPair, value, opts = {}) {
    ...
    while (attempt < 2) {
      ...
      const query = state.transportDHT.query(
        ...
        {
          transportContext: state.queryContexts.mutableGet.announce,
          concurrency: 1,
          retries: 1,
          map: mapMutable,
          commit(reply, dht) {
