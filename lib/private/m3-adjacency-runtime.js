...
const { isSurbHopFrame } = require('./surb-path')
...
const BRANCH_TEARDOWN_BODY_BYTES = 58
const BRANCH_TEARDOWN_PACKET_BYTES = 8 + BRANCH_TEARDOWN_BODY_BYTES
const BRANCH_DESTROY_REASON_DOWNSTREAM_PHYSICAL_LOSS = 1
const BRANCH_TEARDOWN_REASON_INTENTIONAL_SUSPEND = 2
...
const V2_RECEIVE_RESERVATIONS = new WeakMap()

function getTakePeerM3RouteCarrier() {
  try {
    const peerRuntime = require('./peer-m3-adjacency-runtime')
    if (typeof peerRuntime.takePeerM3RouteCarrier === 'function') {
      return peerRuntime.takePeerM3RouteCarrier
    }
  } catch {}
  return null
}
function validatePeerCarrier(carrier) {
  if (!safeObject(carrier) || !Object.isFrozen(carrier)) return false
  const methods = [
    'sendFrame',
    'sendFinalizeFrame',
    'activate',
  ...
  if (Reflect.ownKeys(carrier).length !== methods.length + 1) return false
  ...
  const descriptor = Object.getOwnPropertyDescriptor(carrier, 'clock')
  if (!descriptor || !safeObject(descriptor.value) || !Object.isFrozen(descriptor.value))
    return false
  const clock = descriptor.value
  return (
    Reflect.ownKeys(clock).length === 5 &&
    safeObject(clock.clockIdentity) &&
    typeof clock.wireExpiresAt === 'bigint' &&
    typeof clock.localDeadline === 'bigint' &&
    typeof clock.wallNow === 'function' &&
...
function teardownMessageId(payload) {
  if (!fixed(payload, BRANCH_TEARDOWN_PACKET_BYTES)) return null
  if (payload[0] !== 0 || payload[1] !== 0 || payload[2] !== 0 || payload[3] !== 1) return null
  return (payload[4] << 8) | payload[5]
}

function encodeBranchTeardown(state, teardownId, messageId = M3_MESSAGE_ID.BRANCH_TEARDOWN_V1) {
  if (
    (messageId !== M3_MESSAGE_ID.BRANCH_TEARDOWN_V1 &&
      messageId !== M3_MESSAGE_ID.BRANCH_TEARDOWN_ACK_V1) ||
  ...
  const body = b4a.allocUnsafeSlow(BRANCH_TEARDOWN_BODY_BYTES)
  try {
    body[0] = state.branchClass
    setIntrinsic.call(body, state.branchId, 1)
    setIntrinsic.call(body, state.circuitId, 17)
    writeU64(body, state.generation, 33)
    body[41] = BRANCH_TEARDOWN_REASON_INTENTIONAL_SUSPEND
    setIntrinsic.call(body, teardownId, 42)
    return encodeM3Object({ messageId, body })
  } finally {
    clear(body)
  }
}

function encodeBranchTeardownBody(body, messageId) {
  if (
    !fixed(body, BRANCH_TEARDOWN_BODY_BYTES) ||
    (messageId !== M3_MESSAGE_ID.BRANCH_TEARDOWN_V1 &&
      messageId !== M3_MESSAGE_ID.BRANCH_TEARDOWN_ACK_V1)
  ) {
    invalid()
...
function decodeBranchTeardown(payload, state, messageId) {
  const observed = teardownMessageId(payload)
  if (observed !== messageId) return null
  let decoded = null
  try {
    decoded = decodeM3Object(payload)
    const body = decoded.body
    if (
      ...
      readU64(body, 33) !== state.generation ||
      body[41] !== BRANCH_TEARDOWN_REASON_INTENTIONAL_SUSPEND ||
      !nonzero(body.subarray(42, 58))
    ) {
      authentication()
    }
    return {
      body: copy(body, BRANCH_TEARDOWN_BODY_BYTES),
      teardownId: copy(body.subarray(42, 58), 16)
    }
  } finally {
    if (decoded) {
      clear(decoded.body)
...
function clearDecodedBranchTeardown(decoded) {
  if (!decoded) return
  clear(decoded.body)
  clear(decoded.teardownId)
  decoded.body = null
  decoded.teardownId = null
}
...
function takeM3RouteTransport(owner) {
  const taker = getTakePeerM3RouteCarrier()
  if (taker) {
    let carrier = null
    try {
      carrier = taker(owner)
    ...
    if (carrier !== null && carrier !== undefined) {
      if (!validatePeerCarrier(carrier)) invalid()
...
function sendM3RuntimePayload(state, cellClass, payload) {
  const channel = state.physicalChannel
  if (!safeObject(channel) || typeof channel.send !== 'function') invalid()
  let packet = null
  try {
    packet = state.runtime.sealTail({ class: cellClass, payload })
...
function sendM3RouteFrame(transport, frame) {
  ...
  if (record.v2) {
    return record.carrier.sendFrame(frame)
  }
  if (record.endpointTeardown !== null || record.exitTeardown !== null) destroyed()
  return sendM3RuntimePayload(record.runtimeState, CELL_CLASS.DATAGRAM, frame)
}

function sendM3FinalizeFrame(transport, frame) {
  const record = routeTransportState(transport)
  if (!record.v2 || !fixed(frame, 1100)) invalid()
  return record.carrier.sendFinalizeFrame(frame)
}

function activateM3RouteTransport(transport) {
  const record = routeTransportState(transport)
  if (!record.v2) invalid()
  return record.carrier.activate()
}

function readM3RouteTransportClock(transport) {
  const record = routeTransportState(transport)
  if (!record.v2) invalid()
  return record.carrier.clock
}
...
function scheduleM3RouteTransportTimer(transport, delayMs, callback) {
  ...
  if (record.v2) {
    return record.carrier.schedule(delayMs, callback)
  }
...
function cancelM3TeardownTimer(operation) {
  if (!operation || typeof operation.cancelTimer !== 'function') return false
  const cancel = operation.cancelTimer
...
function rejectEndpointTeardown(record, error) {
  const operation = record && record.endpointTeardown
  if (!operation || operation.settled) return false
  operation.settled = true
  operation.status = 'FAILED'
  cancelM3TeardownTimer(operation)
  clear(operation.teardownId)
  operation.teardownId = null
...
function failM3RouteTeardown(transport, record, error) {
  rejectEndpointTeardown(record, error)
  destroyM3RouteTransport(transport)
}

function armEndpointTeardownTimer(transport, record, operation) {
  if (
    M3_ROUTE_TRANSPORTS.get(transport) !== record ||
    record.endpointTeardown !== operation ||
    operation.settled
  ) {
    return false
  }
  const owner = AUTHORITY_STATES.get(record.runtimeState.authority)
  if (!owner) {
    failM3RouteTeardown(transport, record, PrivateRouteError.ERR_PRIVACY_UNAVAILABLE())
    return false
  }
  let current
  try {
    current = sampleMonotonicNow(owner)
  } catch {
    failM3RouteTeardown(transport, record, PrivateRouteError.ERR_PRIVACY_UNAVAILABLE())
    return false
  }
  ...
  const remaining = operation.deadline - current
  const delay = Number(remaining < BigInt(LINK_PING_AFTER) ? remaining : BigInt(LINK_PING_AFTER))
  let arming = true
  let firedSynchronously = false
  ...
  const onTimer = () => {
    ...
    let sampled
    try {
      sampled = sampleMonotonicNow(owner)
    } catch {
      failM3RouteTeardown(transport, record, PrivateRouteError.ERR_PRIVACY_UNAVAILABLE())
      return
    }
    if (sampled >= operation.deadline) {
      failM3RouteTeardown(transport, record, PrivateRouteError.ERR_PRIVACY_UNAVAILABLE())
      return
    }
    sendEndpointTeardown(transport, record, operation)
    armEndpointTeardownTimer(transport, record, operation)
  }
  try {
    cancelTimer = scheduleM3RouteTransportTimer(transport, delay, onTimer)
  } catch {
    arming = false
    failM3RouteTeardown(transport, record, PrivateRouteError.ERR_PRIVACY_UNAVAILABLE())
    return false
  ...
  operation.cancelTimer = cancelTimer
  return true
}

function sendEndpointTeardown(transport, record, operation) {
  if (
    M3_ROUTE_TRANSPORTS.get(transport) !== record ||
    record.endpointTeardown !== operation ||
    operation.settled ||
  ...
  let payload = null
  try {
    payload = encodeBranchTeardown(record.runtimeState, operation.teardownId)
    const sent = sendM3RuntimePayload(record.runtimeState, CELL_CLASS.DATAGRAM, payload)
...
function beginM3RouteTeardown(transport, teardownId) {
  let record
  try {
    record = routeTransportState(transport)
  } catch (err) {
  ...
  if (!fixed(teardownId, 16) || !nonzero(teardownId)) {
    return Promise.reject(PrivateRouteError.INVALID_ROUTE())
  }
  if (record.endpointTeardown !== null || record.teardownHandler !== null) {
    return Promise.reject(PrivateRouteError.ERR_REPLAY())
  }
  const owner = AUTHORITY_STATES.get(record.runtimeState.authority)
  if (!owner) return Promise.reject(PrivateRouteError.ERR_PRIVACY_UNAVAILABLE())
  let current
  try {
    current = sampleMonotonicNow(owner)
  } catch {
  ...
  const absolute = current + BigInt(LINK_CIRCUIT_TEARDOWN_TIMEOUT)
  const deadline =
    absolute < record.runtimeState.localDeadline ? absolute : record.runtimeState.localDeadline
  if (deadline <= current) return Promise.reject(PrivateRouteError.ERR_PRIVACY_UNAVAILABLE())
  let resolve
  let reject
  ...
  record.endpointTeardown = operation
  if (armEndpointTeardownTimer(transport, record, operation)) {
    sendEndpointTeardown(transport, record, operation)
  }
  return promise
}
...
function consumeEndpointTeardownAck(record, payload) {
  const decoded = decodeBranchTeardown(
    payload,
    record.runtimeState,
  ...
  try {
    const operation = record.endpointTeardown
    if (operation === null) authentication()
    if (operation.settled) replay()
    if (!same(decoded.teardownId, operation.teardownId)) authentication()
    operation.settled = true
    ...
    clearDecodedBranchTeardown(decoded)
  }
}

function clearExitTeardown(operation) {
  if (!operation) return
  cancelM3TeardownTimer(operation)
  clear(operation.body)
  clear(operation.teardownId)
  operation.body = null
  operation.teardownId = null
  operation.sending = false
...
function armExitTeardownDeadline(transport, record, operation) {
  const owner = AUTHORITY_STATES.get(record.runtimeState.authority)
  ...
  operation.deadline =
    absolute < record.runtimeState.localDeadline ? absolute : record.runtimeState.localDeadline
  if (operation.deadline <= current) unavailable()
  const remaining = operation.deadline - current
  operation.cancelTimer = scheduleM3RouteTransportTimer(transport, timerDelay(remaining), () => {
    operation.cancelTimer = null
    if (M3_ROUTE_TRANSPORTS.get(transport) === record && record.exitTeardown === operation) {
      destroyM3RouteTransport(transport)
    }
  })
}

function sendExitTeardownAck(transport, record, operation) {
  if (
    M3_ROUTE_TRANSPORTS.get(transport) !== record ||
    record.exitTeardown !== operation ||
    operation.status !== 'ACK_CACHED'
  ) {
    return false
  }
  if (operation.sending) {
    operation.resend = true
    return true
  }
  operation.sending = true
  let payload = null
  try {
    payload = encodeBranchTeardownBody(operation.body, M3_MESSAGE_ID.BRANCH_TEARDOWN_ACK_V1)
    Promise.resolve(sendM3RuntimePayload(record.runtimeState, CELL_CLASS.DATAGRAM, payload)).then(
      () => {
        if (M3_ROUTE_TRANSPORTS.get(transport) !== record || record.exitTeardown !== operation) {
          return
        }
        ...
        if (operation.resend) {
          operation.resend = false
          sendExitTeardownAck(transport, record, operation)
        }
      },
      () => {
        operation.sending = false
...
function beginExitTeardown(transport, record, decoded) {
  const handler = record.teardownHandler
  if (typeof handler !== 'function') authentication()
  const operation = {
    status: 'DRAINING',
    body: decoded.body,
    teardownId: decoded.teardownId,
    cancelTimer: null,
    deadline: 0n,
    sending: false,
    resend: false
  ...
  decoded.teardownId = null
  record.exitTeardown = operation
  record.teardownHandler = null
  try {
    armExitTeardownDeadline(transport, record, operation)
  } catch {
    destroyM3RouteTransport(transport)
    return false
  }
...
function consumeExitTeardownRequest(transport, record, payload) {
  const decoded = decodeBranchTeardown(
    payload,
    record.runtimeState,
    M3_MESSAGE_ID.BRANCH_TEARDOWN_V1
  )
  if (decoded === null) return false
  try {
    if (record.endpointTeardown !== null) authentication()
    const operation = record.exitTeardown
    if (operation === null) return beginExitTeardown(transport, record, decoded)
    if (!same(decoded.teardownId, operation.teardownId)) replay()
    if (operation.status === 'ACK_CACHED') sendExitTeardownAck(transport, record, operation)
    return true
  } finally {
    clearDecodedBranchTeardown(decoded)
  }
...
function consumeM3RouteTeardownControl(transport, record, payload) {
  const messageId = teardownMessageId(payload)
  if (messageId === M3_MESSAGE_ID.BRANCH_TEARDOWN_ACK_V1) {
    if (record.teardownHandler !== null || record.exitTeardown !== null) authentication()
    return consumeEndpointTeardownAck(record, payload)
  }
  if (messageId === M3_MESSAGE_ID.BRANCH_TEARDOWN_V1) {
    return consumeExitTeardownRequest(transport, record, payload)
  }
  return false
}

function registerM3RouteTeardownHandler(transport, handler) {
  const record = routeTransportState(transport)
  if (
    record.v2 ||
    typeof handler !== 'function' ||
    record.teardownHandler !== null ||
    record.endpointTeardown !== null ||
    record.exitTeardown !== null
  ) {
    invalid()
  }
  record.teardownHandler = handler
  return true
}
...
function consumeM3RouteBranchDestroy(transport, record, payload) {
  ...
  rejectEndpointTeardown(record, PrivateRouteError.ERR_PRIVACY_UNAVAILABLE())
  destroyM3RouteTransport(transport)
  ...
  Promise.resolve()
    .then(function receiveNextRoutePacket() {
      ...
      return Promise.resolve(channel.receive()).then((packet) => {
        ...
        try {
          ...
          if (consumeM3RouteTeardownControl(transport, record, opened[0])) {
    ...
    .then(
      ...
      (error) => {
        ...
        rejectEndpointTeardown(
          record,
          error instanceof PrivateRouteError ? error : PrivateRouteError.ERR_PRIVACY_UNAVAILABLE()
        )
        if (M3_ROUTE_TRANSPORTS.get(transport) === record) destroyM3RouteTransport(transport)
      }
    )
}
...
function reserveM3RouteFrame(transport) {
  const record = routeTransportState(transport)
  if (record.v2) {
    const carrierRes = record.carrier.reserveReceive()
    const reservation = Object.freeze({})
    const resState = { reservation, carrier: record.carrier, carrierRes, record }
    V2_RECEIVE_RESERVATIONS.set(reservation, resState)
    record.reservations.add(resState)
    return reservation
  }
  const reservation = Object.freeze({})
  const waiter = {
...
function takeV2RouteReceiveReservation(reservation) {
  const state = safeObject(reservation) ? V2_RECEIVE_RESERVATIONS.get(reservation) : null
  if (!state) return null
  V2_RECEIVE_RESERVATIONS.delete(reservation)
  state.record.reservations.delete(state)
  SPENT_M3_ROUTE_RECEIVE_RESERVATIONS.add(reservation)
  return state
}

function receiveReservedM3RouteEnvelope(reservation) {
  const state = takeV2RouteReceiveReservation(reservation)
  if (!state) {
  ...
  return state.carrier.receiveEnvelope(state.carrierRes)
}

function receiveReservedM3RouteFrame(reservation) {
  const v2State = takeV2RouteReceiveReservation(reservation)
  if (v2State) {
    return v2State.carrier.receiveFrame(v2State.carrierRes)
  }
...
function cancelM3RouteFrameReservation(reservation) {
  const v2State = safeObject(reservation) ? V2_RECEIVE_RESERVATIONS.get(reservation) : null
  if (v2State) {
    V2_RECEIVE_RESERVATIONS.delete(reservation)
    if (v2State.record && v2State.record.reservations) {
      v2State.record.reservations.delete(v2State)
    }
    return v2State.carrier.cancelReceive(v2State.carrierRes)
  }
  const state = safeObject(reservation) ? M3_ROUTE_RECEIVE_RESERVATIONS.get(reservation) : null
...
function readM3RouteTransportDiagnostics(transport) {
  ...
  if (record.v2) {
    return record.carrier.diagnostics()
  }
  return Object.freeze({
    ...
    endpointTeardown: record.endpointTeardown === null ? null : record.endpointTeardown.status,
    exitTeardown: record.exitTeardown === null ? null : record.exitTeardown.status,
    cachedTeardownAckBytes:
      record.exitTeardown && record.exitTeardown.body ? record.exitTeardown.body.byteLength : 0,
    teardownTimer: !!(
      (record.endpointTeardown && record.endpointTeardown.cancelTimer) ||
      (record.exitTeardown && record.exitTeardown.cancelTimer)
    ),
    teardownCallback: record.teardownHandler !== null
  })
}
...
function destroyM3RouteTransport(transport) {
  ...
  if (record.v2) {
    if (record.reservations) {
      for (const resState of Array.from(record.reservations)) {
        V2_RECEIVE_RESERVATIONS.delete(resState.reservation)
        try {
          resState.carrier.cancelReceive(resState.carrierRes)
        } catch {}
      }
      record.reservations.clear()
    }
    try {
      record.carrier.destroy()
    } catch {}
    return true
  }
  rejectEndpointTeardown(record, PrivateRouteError.ERR_DESTROYED())
  record.endpointTeardown = null
  clearExitTeardown(record.exitTeardown)
  record.exitTeardown = null
  record.teardownHandler = null
...
function createM3RelayForwardingFacade(previousRuntime, nextRuntime, options) {
  ...
  if (!previous || !next || previous === next || !safeObject(options)) invalid()
  const expected = [
    'releaseDownstream',
    'releaseUpstream',
    'monotonicNow',
  ...
  for (const key of keys) {
    if (!expected.includes(key) && !optional.includes(key)) invalid()
  }
  if (expected.some((key) => !keys.includes(key))) invalid()
  const capabilities = {}
  ...
  // Optional: peel one SURB hop layer on reverse DATAGRAM. Absent => forward payload bytes as-is
  // (non-SURB reverse stays byte-identical; terminal cells also pass through opaque).
  if (keys.includes('surbHopPeel')) {
    const descriptor = getOwnPropertyDescriptor(options, 'surbHopPeel')
    if (
      !descriptor ||
      !Object.prototype.hasOwnProperty.call(descriptor, 'value') ||
      typeof descriptor.value !== 'function'
    ...
    capabilities.surbHopPeel = descriptor.value
  }
  ...
  let timer = null
  let teardown = null
  let surbHopCellsDropped = 0
  const releases = {
    downstream: { callback: capabilities.releaseDownstream, state: 'READY' },
    upstream: { callback: capabilities.releaseUpstream, state: 'READY' }
  }
  ...
  const cancelTimer = () => {
    ...
    timer = null
    try {
      capabilities.cancelScheduled(handle)
    } catch {}
    return true
  }
  ...
  const clearTeardown = () => {
    ...
    clear(teardown.body)
    clear(teardown.teardownId)
    teardown.body = null
    teardown.teardownId = null
    teardown = null
  }

  const invokeRelease = (name) => {
    const record = releases[name]
    if (record.state !== 'READY') return Promise.reject(PrivateRouteError.ERR_REPLAY())
  ...
  const releaseRemaining = () => {
    const pending = []
    for (const name of ['downstream', 'upstream']) {
      if (releases[name].state === 'READY') pending.push(invokeRelease(name))
    }
    return Promise.allSettled(pending)
  }

  const finish = (err = null, release = true) => {
    ...
    cancelTimer()
    clearTeardown()
    const released = release ? releaseRemaining() : Promise.resolve()
    void released.finally(releaseTerminalForwarding)
    if (!release) releaseTerminalForwarding()
    return true
  }
  ...
  const armTeardownDeadline = () => {
    const current = nowValue(capabilities.monotonicNow)
    let deadline = current + BigInt(LINK_CIRCUIT_TEARDOWN_TIMEOUT)
    if (previous.localDeadline < deadline) deadline = previous.localDeadline
    if (next.localDeadline < deadline) deadline = next.localDeadline
    if (deadline <= current) unavailable()
    teardown.deadline = deadline
    let arming = true
    let fired = false
    const onTimeout = () => {
      if (arming) {
      ...
      timer = null
      finish(PrivateRouteError.ERR_PRIVACY_UNAVAILABLE())
    }
    let handle = null
    try {
      handle = capabilities.schedule(onTimeout, timerDelay(deadline - current))
    } catch {
      unavailable()
    }
    arming = false
    if (handle === null || handle === undefined || fired || !live || teardown === null) {
      if (handle !== null && handle !== undefined) {
        try {
          capabilities.cancelScheduled(handle)
        } catch {}
  ...
  const handleTeardownRequest = async (source, cellClass, payloads, payload, reverse) => {
    const decoded = decodeBranchTeardown(payload, source, M3_MESSAGE_ID.BRANCH_TEARDOWN_V1)
    if (decoded === null) return false
    try {
      if (reverse || cellClass !== CELL_CLASS.DATAGRAM || payloads.length !== 1) authentication()
      if (teardown === null) {
        teardown = {
          status: 'DRAINING_DOWNSTREAM',
          body: decoded.body,
          teardownId: decoded.teardownId,
          deadline: 0n
        }
        ...
        decoded.teardownId = null
        armTeardownDeadline()
      } else if (!same(decoded.teardownId, teardown.teardownId)) {
        replay()
      }
      if (teardown.status === 'ACK_CACHED') {
        let ack = null
        try {
          ack = encodeBranchTeardownBody(teardown.body, M3_MESSAGE_ID.BRANCH_TEARDOWN_ACK_V1)
          await sendPayload(previous, CELL_CLASS.DATAGRAM, ack)
        } finally {
          clear(ack)
        }
      } else if (teardown.status === 'DRAINING_DOWNSTREAM') {
        await sendPayload(next, CELL_CLASS.DATAGRAM, payload)
      } else if (teardown.status !== 'RELEASING_DOWNSTREAM') {
        authentication()
      }
      return true
    } finally {
  ...
  const handleTeardownAck = async (source, cellClass, payloads, payload, reverse) => {
    const decoded = decodeBranchTeardown(payload, source, M3_MESSAGE_ID.BRANCH_TEARDOWN_ACK_V1)
    if (decoded === null) return false
    try {
      if (!reverse || cellClass !== CELL_CLASS.DATAGRAM || payloads.length !== 1) authentication()
      if (teardown === null || teardown.status !== 'DRAINING_DOWNSTREAM') authentication()
      if (!same(decoded.teardownId, teardown.teardownId)) authentication()
      teardown.status = 'RELEASING_DOWNSTREAM'
      try {
        await invokeRelease('downstream')
      } catch (err) {
        teardown.status = 'DOWNSTREAM_RELEASE_FAILED'
        throw err
      }
      teardown.status = 'ACK_CACHED'
      try {
        await sendPayload(previous, CELL_CLASS.DATAGRAM, payload)
      } catch (err) {
        teardown.status = 'ACK_SEND_FAILED'
        throw err
      }
      return true
    } finally {
  ...
  const stopAfterFailure = async (err, downstreamFailure) => {
    ...
    if (
      teardown !== null &&
      downstreamFailure &&
      (teardown.status === 'RELEASING_DOWNSTREAM' || teardown.status === 'ACK_CACHED')
    ) {
      return
    }
    if (teardown !== null) {
    ...
    if (downstreamFailure) {
      ...
      try {
        ...
        failure =
          sendError instanceof PrivateRouteError ? sendError.code : 'ERR_PRIVACY_UNAVAILABLE'
      }
    }
    finish(err)
  }
    ...
    while (live && !stopping) {
      ...
      try {
        ...
        for (const payload of payloads) {
          if (!live || stopping) break
          if (await handleTeardownRequest(source, cellClass, payloads, payload, reverse)) continue
          if (await handleTeardownAck(source, cellClass, payloads, payload, reverse)) continue
          if (teardown !== null) authentication()
          ...
          // Reverse SURB hop peel is opt-in. Without surbHopPeel, payload bytes are unchanged.
          // With it, a hop cell this relay cannot peel (bad MAC, replay, expired
          // capability) is dropped here: forwarding it raw would hand the next hop a
          // frame it cannot open either and let one bad cell end the endpoint's
          // operation.
          if (
            reverse &&
            cellClass === CELL_CLASS.DATAGRAM &&
            typeof capabilities.surbHopPeel === 'function'
          ) {
            let peeled = null
            try {
              peeled = capabilities.surbHopPeel(payload)
            } catch {
              peeled = null
            }
            if (peeled !== null && peeled !== undefined) {
              if (!b4a.isBuffer(peeled)) invalid()
              try {
                await sendPayload(target, cellClass, peeled)
              } finally {
                if (peeled !== payload) clear(peeled)
              }
              continue
            }
            if (isSurbHopFrame(payload)) {
              surbHopCellsDropped++
              continue
            }
          }
  ...
  const facade = Object.freeze({
    diagnostics() {
      return Object.freeze({
        state: teardown === null ? (live ? 'FORWARDING' : 'DESTROYED') : teardown.status,
        failure,
        cachedTeardownAckBytes:
          teardown !== null && teardown.status === 'ACK_CACHED' ? BRANCH_TEARDOWN_BODY_BYTES : 0,
        teardownTimer: timer !== null,
        releaseDownstream: releases.downstream.state,
        releaseUpstream: releases.upstream.state,
        surbHopCellsDropped
      })
    },
    destroy() {
...
module.exports = {
  ...
  BRANCH_TEARDOWN_REASON_INTENTIONAL_SUSPEND,
