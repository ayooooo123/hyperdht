...
const SURB_DESCRIPTOR_SIZE = ID_BYTES + KEY_BYTES + RHO + MAC_BYTES + KEY_BYTES // 436
const SURB_ID_BYTES = 16
const BATCH_ID_BYTES = 16
const REQUEST_ID_BYTES = 16
const MESSAGE_ID_BYTES = 16
const MAX_SURB_BATCH = 8
const SURB_REPLY_DATA_BYTES = 492
const MAX_SURB_REPLY_APPLICATION_BYTES = MAX_SURB_BATCH * SURB_REPLY_DATA_BYTES // 3936
const REPLY_BINDING_SIZE =
  SURB_ID_BYTES + BATCH_ID_BYTES + REQUEST_ID_BYTES + MESSAGE_ID_BYTES + 2 + 2 // 68
const SURB_HOP_HEADER_SIZE = KEY_BYTES + RHO + MAC_BYTES // 372
...
function admitNullifier(state, nullifier) {
  ...
  if (state.seen.size >= state.maxReplayEntries) quotaExceeded()
...
// One authority per advertised route key. It owns the nullifier store, so a relay
// serving many circuits with one key admits each header once, not once per facade
// (two admissions of one header wrap under the same key and nonce). `wallNow` is
// read at every use, so the capability expires with the advertisement instead of
// holding the clock value it was created under.
function createSurbCapabilityAuthority(options) {
  ...
  let maxReplayEntries = options.maxReplayEntries
  if (maxReplayEntries === undefined) maxReplayEntries = 1 << 16
  ...
  if (typeof wallNow !== 'function') invalid()
  if (!Number.isInteger(maxReplayEntries) || maxReplayEntries <= 0) invalid()
  const now = wallNow()
  if (!isUint64(now)) invalid()
...
function destroySurbCapabilityAuthority(authority) {
  const state =
    authority !== null && (typeof authority === 'object' || typeof authority === 'function')
  ...
  capabilityAuthorityRecords.delete(authority)
  weakSetAdd.call(spentCapabilityAuthorities, authority)
  clearCapabilityState(state)
  return true
}
...
function liveCapability(authority) {
  ...
  if (!isUint64(now) || state.expiresAtMs <= now) invalid()
  return state
}
...
function processSurbHop(options) {
  ...
  try {
    ...
    // Atomic nullifier admission BEFORE payload transformation, in the store this
    // route key owns.
    admitNullifier(cap, k.nullifier)
...
function encodeSurbDescriptor(descriptor) {
  if (!descriptor || typeof descriptor !== 'object') invalid()
  if (
    !isBuffer(descriptor.firstHop, ID_BYTES) ||
    !isCanonicalPublicKey(descriptor.ephem) ||
    !isBuffer(descriptor.header, RHO) ||
    !isBuffer(descriptor.mac, MAC_BYTES) ||
    !isCanonicalPublicKey(descriptor.replyPubKey)
  ) {
    invalid()
  }
  const out = b4a.allocUnsafeSlow(SURB_DESCRIPTOR_SIZE)
  out.set(descriptor.firstHop, 0)
  out.set(descriptor.ephem, ID_BYTES)
  out.set(descriptor.header, ID_BYTES + KEY_BYTES)
  out.set(descriptor.mac, ID_BYTES + KEY_BYTES + RHO)
  out.set(descriptor.replyPubKey, ID_BYTES + KEY_BYTES + RHO + MAC_BYTES)
  return out
}

function decodeSurbDescriptor(encoded) {
  if (!isBuffer(encoded, SURB_DESCRIPTOR_SIZE)) invalid()
  const firstHop = b4a.from(encoded.subarray(0, ID_BYTES))
  const ephem = b4a.from(encoded.subarray(ID_BYTES, ID_BYTES + KEY_BYTES))
  const header = b4a.from(encoded.subarray(ID_BYTES + KEY_BYTES, ID_BYTES + KEY_BYTES + RHO))
  const mac = b4a.from(
    encoded.subarray(ID_BYTES + KEY_BYTES + RHO, ID_BYTES + KEY_BYTES + RHO + MAC_BYTES)
  )
  const replyPubKey = b4a.from(encoded.subarray(ID_BYTES + KEY_BYTES + RHO + MAC_BYTES))
  if (!isCanonicalPublicKey(ephem) || !isCanonicalPublicKey(replyPubKey)) invalid()
  return objectFreeze({ firstHop, ephem, header, mac, replyPubKey })
}

function encodeSurbReplyBinding(value) {
  if (value === null || typeof value !== 'object' || Array.isArray(value)) invalid()
  const surbId = value.surbId
  const batchId = value.batchId
  const requestId = value.requestId
  const messageId = value.messageId
  const fragmentIndex = value.fragmentIndex
  const fragmentCount = value.fragmentCount
  if (
    !isBuffer(surbId, SURB_ID_BYTES) ||
    !isBuffer(batchId, BATCH_ID_BYTES) ||
    !isBuffer(requestId, REQUEST_ID_BYTES) ||
    !isBuffer(messageId, MESSAGE_ID_BYTES)
  ) {
    invalid()
  }
  if (
    ...
    fragmentCount < 1 ||
    fragmentCount > MAX_SURB_BATCH ||
    fragmentIndex >= fragmentCount
  ) {
    invalid()
  }
  const out = b4a.allocUnsafeSlow(REPLY_BINDING_SIZE)
  out.set(surbId, 0)
  out.set(batchId, SURB_ID_BYTES)
  out.set(requestId, SURB_ID_BYTES + BATCH_ID_BYTES)
  out.set(messageId, SURB_ID_BYTES + BATCH_ID_BYTES + REQUEST_ID_BYTES)
  writeUint16BE(
    out,
    fragmentIndex,
    SURB_ID_BYTES + BATCH_ID_BYTES + REQUEST_ID_BYTES + MESSAGE_ID_BYTES
  )
  writeUint16BE(
    out,
    fragmentCount,
    SURB_ID_BYTES + BATCH_ID_BYTES + REQUEST_ID_BYTES + MESSAGE_ID_BYTES + 2
  )
  return out
}

function encodeSurbHopMessage(message) {
  if (!message || typeof message !== 'object') invalid()
  if (message.ephem === undefined) {
    if (!isBuffer(message.payload)) invalid()
    return b4a.from(message.payload)
  }
  if (
    !isCanonicalPublicKey(message.ephem) ||
    !isBuffer(message.header, RHO) ||
    !isBuffer(message.mac, MAC_BYTES) ||
    !isBuffer(message.payload)
  ) {
    invalid()
  }
  const out = b4a.allocUnsafeSlow(SURB_HOP_HEADER_SIZE + message.payload.byteLength)
  out.set(message.ephem, 0)
  out.set(message.header, KEY_BYTES)
  out.set(message.mac, KEY_BYTES + RHO)
  out.set(message.payload, SURB_HOP_HEADER_SIZE)
  return out
}

function decodeSurbHopMessage(encoded) {
  if (!isBuffer(encoded)) invalid()
  if (encoded.byteLength < SURB_HOP_HEADER_SIZE + REPLY_OVERHEAD) {
    // terminal payload-only form
    if (encoded.byteLength < REPLY_OVERHEAD) invalid()
    return objectFreeze({ payload: b4a.from(encoded) })
  }
  const ephem = b4a.from(encoded.subarray(0, KEY_BYTES))
  const header = b4a.from(encoded.subarray(KEY_BYTES, KEY_BYTES + RHO))
  const mac = b4a.from(encoded.subarray(KEY_BYTES + RHO, SURB_HOP_HEADER_SIZE))
  const payload = b4a.from(encoded.subarray(SURB_HOP_HEADER_SIZE))
  if (!isCanonicalPublicKey(ephem)) invalid()
  return objectFreeze({ ephem, header, mac, payload })
}
