...
// wrap tags = pt + 468. At this cap the worst SURB message is ~980 B, leaving ~220 B under
// the 1,200-byte cell for Noise/UDX/cell framing — which is NOT yet measured. Re-derive
// against real framing overhead when the DATAGRAM reply path is wired; treat as provisional.
...
function nullifierOf(sharedSecret) {
  if (!isBuffer(sharedSecret, KEY_BYTES)) invalid()
  const out = b4a.allocUnsafeSlow(32)
...
function buildSurb(hops, terminalId, opts) {
  ...
  for (let i = 0; i < n; i++) {
    const h = hops[i]
    if (!h || !isBuffer(h.routeKey, KEY_BYTES) || !isBuffer(h.id, ID_BYTES)) invalid()
  ...
  for (let i = 0; i < n; i++) {
    sodium.sodium_memzero(rhoKeys[i])
    sodium.sodium_memzero(muKeys[i])
  }
...
function processSurbHop(message, routeSecretKey) {
  ...
  let shared
  try {
    shared = cryptoSuite.keyAgreement(routeSecretKey, message.ephem)
  } catch {
    invalid()
  }
  const k = hopKeys(shared)
  ...
  const macValid = sodium.sodium_memcmp(expected, message.mac)
  if (!macValid) {
    sodium.sodium_memzero(k.rhoKey)
    sodium.sodium_memzero(k.muKey)
    sodium.sodium_memzero(k.wrapKey)
    invalid()
  }
...
function openSurbPayload(payload, openKeys) {
  if (
    !isBuffer(payload) ||
    !openKeys ||
    !isBuffer(openKeys.replyPubKey, KEY_BYTES) ||
    !isBuffer(openKeys.replySecretKey, KEY_BYTES) ||
    !Array.isArray(openKeys.wrapKeys)
  ) {
    invalid()
  }
  ...
  for (let i = openKeys.wrapKeys.length - 1; i >= 0; i--) {
    ...
    if (!w || !isBuffer(w.wrapKey, KEY_BYTES) || !isBuffer(w.wrapNoncePrefix, 16)) invalid()
