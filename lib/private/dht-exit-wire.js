...
const MAX_MUTABLE_GET_SEQ_BYTES = 9
...
function validateMessage(message, expectedTid) {
  ...
  if (message.command === PING) {
    if (message.internal !== true || token !== null || target !== null || message.value !== null) {
      invalid()
    }
  } else if (message.command === IMMUTABLE_GET) {
    if (message.internal !== false || target === null || message.value !== null) {
      invalid()
    }
  } else if (message.command === MUTABLE_GET) {
    if (
      message.internal !== false ||
      token !== null ||
      target === null ||
      !b4a.isBuffer(message.value) ||
      message.value.byteLength < 1 ||
      message.value.byteLength > MAX_MUTABLE_GET_SEQ_BYTES
    ) {
      invalid()
    }
    value = b4a.from(message.value)
  } else if (message.command === IMMUTABLE_PUT || message.command === MUTABLE_PUT) {
    if (
      message.internal !== false ||
      token === null ||
      target === null ||
      !b4a.isBuffer(message.value) ||
      message.value.byteLength > MAX_IMMUTABLE_VALUE_BYTES
    ) {
      invalid()
    }
    value = b4a.from(message.value)
...
function encodeDhtExitRequest(reservation, message) {
  ...
  if (value) state.end += encodedSize({ preencode: (s) => c.buffer.preencode(s, value) })
...
function decodeDhtReplyEnvelope(reservation, source, packet, options) {
  const observeLocalTuple =
    isObject(options) && Object.prototype.hasOwnProperty.call(options, 'observeLocalTuple')
      ? options.observeLocalTuple === true
      : false
  if (
    options !== undefined &&
    (!isObject(options) ||
      Reflect.ownKeys(options).length !== 1 ||
      !Object.prototype.hasOwnProperty.call(options, 'observeLocalTuple') ||
      typeof options.observeLocalTuple !== 'boolean')
  ) {
    invalid()
  }
  const reservationState = readReservation(reservation)
  ...
  try {
    ...
    if (!observeLocalTuple && !sameTuple(to, reservationState.local)) invalid()
    if (observeLocalTuple && (!to || !Number.isSafeInteger(to.port) || to.port <= 0)) invalid()
...
function decodeDhtExitReply(reservation, source, packet) {
  return decodeDhtReplyEnvelope(reservation, source, packet, { observeLocalTuple: false })
}
