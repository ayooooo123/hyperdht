...
function createPeerNativeNeighborPool(options) {
  ...
  if (onNeighborClosed !== null && typeof onNeighborClosed !== 'function') {
    throw PrivateRouteError.INVALID_ROUTE()
  }
  if (
  ...
  const poolState = {
    ...
    resolveCompletion,
    // Upkeep observer for published neighbors that end (loss, expiry, close).
    onNeighborClosed,
    destroyed: false
  }
...
async function provisionPeerNativeNeighbor(pool, options) {
  ...
  function cleanupNeighbor(reason) {
    ...
    if (neighborRecord.published && !poolState.destroyed && poolState.onNeighborClosed) {
      const observer = poolState.onNeighborClosed
      const identity32 = b4a.from(neighborRecord.peerIdentity32)
      // Deferred so the observer never re-enters this teardown.
      queueMicrotask(() => {
        try {
          observer(identity32, reason)
        } catch {}
      })
    }
...
// Neighbor admission work that happens before a neighbor record exists (the
// grant-pinned advertisement fetch) is node service traffic, so it draws on
// the pool's node service ledger and nothing else (packet §3.4).
function reservePeerNativeNeighborService(pool) {
  const poolState = isObject(pool) ? POOLS.get(pool) : null
  if (!poolState || poolState.destroyed) throw PrivateRouteError.UNAUTHORIZED()
  try {
    return reservePeerLedger(poolState.nodeServiceLedger, DISCOVERY_RESERVATION)
  } catch {
    throw PrivateRouteError.CIRCUIT_LIMIT()
  }
}
