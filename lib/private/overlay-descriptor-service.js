...
function encodeDescriptor({ destinationKeyPair, routeEntry, entryRelayPublicKey, seq, expiresAt }) {
  if (
    !destinationKeyPair ||
    ...
    !b4a.isBuffer(routeEntry) ||
    routeEntry.byteLength !== 32 ||
...
function decodeDescriptor(wire, options = {}) {
  ...
  const routeEntry = b4a.from(wire.subarray(33, 65))
