...
const { destroyEstablishedLinkState, isLinkTicketChecker } = require('./link-setup')
...
function readEstablishedLinkReservation(value) {
  const record = isObject(value) ? ESTABLISHED.get(value) : null
  if (!record) throw PrivateRouteError.UNAUTHORIZED()
  readEstablishedLink(value)
  const state = record.state
  return Object.freeze({
    endpoint: state.endpoint,
    sendHandle: state.sendHandle,
    linkHandle: state.linkHandle,
    session: state.session,
    mode: state.mode,
    absoluteDeadline: state.absoluteDeadline,
    signedExpiry: state.signedExpiry,
    link: state.link,
