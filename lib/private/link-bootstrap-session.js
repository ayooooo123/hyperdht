...
function cancelTimer(state) {
  ...
  if (state.timer !== null) {
    ...
    try {
      ...
      timerErr = unavailable()
    }
  }
  if (state.acceptTimer !== null && state.acceptTimer !== undefined) {
    const timer = state.acceptTimer
    state.acceptTimer = null
    try {
      state.cancel(timer)
    } catch {
      if (!timerErr) timerErr = unavailable()
    }
  }
...
function armAcceptTimer(state) {
  if (state.closed || state.state === 'TOMBSTONE' || state.state === 'OPEN') return
  const current = currentTime(state)
  const deadline = state.absoluteDeadline
  if (deadline <= current) {
  ...
  arming = false
  if (synchronous || state.closed || state.state === 'TOMBSTONE' || state.state === 'OPEN') {
    try {
      state.cancel(timer)
    } catch {}
    if (state.state !== 'OPEN') {
  ...
  state.acceptTimer = timer
}

function assertBootstrapDispatch(state) {
  refreshAuthority(state)
  const now = currentTime(state)
  if (
    state.closed ||
    (state.state !== 'CREATING' && !(state.mode === 'accept' && state.state === 'OPEN')) ||
    ...
    state.endpoint
      ...
      .then(
        (sent) => {
          ...
          if (state.state !== 'CREATING') return
          if (sent !== true) return fail(state, unavailable(), false)
...
function finishOpen(state, established) {
  ...
  if (state.closed || state.state !== 'OPEN' || state.established !== established) return false
  stopPunch(state, 'open')
  state.pending = 0
  state.sendPending = false
  state.pendingSetup = null
  clearRequest(state)
...
function stopPunch(state, reason) {
  if (!state.punch) return
  const punch = state.punch
  state.punch = null
  try {
    punch.stop(reason || 'session-stop')
  } catch {}
}

function consumePunchOption(options, linkHandle) {
  const attempt = options[NAT_PUNCH_ATTEMPT_OPTION]
  if (attempt === undefined) return null
  // Lazy require avoids the endpoint↔session load cycle.
  const endpoint = require('./udx-cell-endpoint')
  return endpoint.consumeNatPunchAttempt(attempt, linkHandle)
}
...
class LinkBootstrapSession {
  constructor(options = {}) {
    ...
    let punch = null
    try {
      punch = consumePunchOption(options, linkHandle)
    } catch {
      throw PrivateRouteError.UNAUTHORIZED()
    }
    const state = {
    ...
    if (punch && mode === 'accept') {
      state.punchStarted = true
      try {
        punch.start()
      } catch {
        stopPunch(state, 'start-failed')
        throw unavailable()
      }
    }
  }
  ...
  open(options = {}) {
    ...
    try {
      ...
      if (state.punch) {
        ...
        void state.punch.waitFirstOwnedSend().then(
          (ok) => {
            if (state.state !== 'CREATING') return
            if (ok !== true) {
              fail(state, unavailable(), true)
  ...
  async receive(packet) {
    ...
    try {
      ...
      try {
        const sent = await state.endpoint.send(state.sendHandle, response, {
          [UDX_SEND_DISPATCH]: () => assertBootstrapDispatch(state)
        })
        ...
        if (state.state === 'CREATING' && state.responderTicket) {
          ...
          state.responderTicket = null
          const established = install(state, ticket)
          return finishOpen(state, established) ? established : false
        }
        return true
      } finally {
  ...
  close() {
    ...
    void (async () => {
      if (state.state === 'CREATING' || state.reject) {
        fail(state, unavailable(), state.state === 'CREATING')
      } else {
        try {
          cancelTimer(state)
        } catch {}
        stopPunch(state, 'close')
        clearEstablished(state)
        state.state = 'TOMBSTONE'
      }
      state.closed = true
      const punchWait =
        state.punch && typeof state.punch.waitDone === 'function' ? state.punch.waitDone() : null
      await Promise.allSettled([
        ...Array.from(state.cancelSends),
        ...(punchWait ? [punchWait] : [])
      ])
      closeSendHandle(state)
      try {
  ...
  waitAccepted() {
    const state = SESSIONS.get(this)
    if (!state || state.mode !== 'accept') {
      return Promise.reject(stateError())
    }
    refreshAuthority(state)
    if (state.closed || state.state === 'TOMBSTONE') {
      return Promise.reject(unavailable())
    }
    if (state.acceptWaiter !== null) {
      return Promise.reject(stateError())
    }
    if (state.state === 'OPEN' && state.established) {
      state.acceptWaiter = Promise.resolve(state.established)
      return state.acceptWaiter
    }
    const current = currentTime(state)
    if (state.closed || state.state === 'TOMBSTONE') return Promise.reject(unavailable())
    const deadline = state.absoluteDeadline
    if (deadline <= current) {
      fail(state, unavailable(), false)
      return Promise.reject(unavailable())
