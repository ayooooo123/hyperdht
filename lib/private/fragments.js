...
const SURB_REPLY_MAX_DATA_BYTES = 492
const fragmentProfiles = new WeakMap()
...
function allocate(size, failure = null) {
  ...
  try {
    ...
    // Construct the captured concrete Buffer shape over a fresh, exact backing
    // store. Capturing b4a.allocUnsafeSlow is insufficient: b4a and Bare's
    // concrete slow allocator both perform later mutable property lookups.
    owned = reflectConstruct(Uint8ArrayConstructor, [size], OwnedBufferConstructor)
...
function createFragmentProfile(options) {
  options = optionsObject(options)
  const maxDataBytes = option(options, 'maxDataBytes', true)
  const maxFragments = option(options, 'maxFragments', true)
  if (
    !numberIsSafeInteger(maxDataBytes) ||
    maxDataBytes < 1 ||
    maxDataBytes > MAX_FRAGMENT_DATA ||
    !numberIsSafeInteger(maxFragments) ||
    maxFragments < 1 ||
  ...
  const profile = Object.freeze({
    maxDataBytes,
    maxFragments,
    maxMessageDataBytes: maxFragments * maxDataBytes,
    maxEncodedMessageBytes: maxFragments * (FRAGMENT_HEADER_SIZE + maxDataBytes),
    headerSize: FRAGMENT_HEADER_SIZE
  })
  fragmentProfiles.set(profile, true)
  return profile
}

const DEFAULT_ROUTE_FRAGMENT_PROFILE = createFragmentProfile({
  maxDataBytes: MAX_FRAGMENT_DATA,
  maxFragments: MAX_FRAGMENTS
})

const SURB_REPLY_FRAGMENT_PROFILE = createFragmentProfile({
  maxDataBytes: SURB_REPLY_MAX_DATA_BYTES,
  maxFragments: MAX_FRAGMENTS
})

function resolveFragmentProfile(value) {
  if (value === undefined) return DEFAULT_ROUTE_FRAGMENT_PROFILE
  if (
    value === null ||
    typeof value !== 'object' ||
    Array.isArray(value) ||
    !fragmentProfiles.has(value)
  ) {
    invalid()
  }
  return value
...
function fragment(message, options) {
  ...
  const profile = resolveFragmentProfile(option(options, 'profile'))
  const maxDataBytes = profile.maxDataBytes
  if (messageLength > profile.maxMessageDataBytes) invalid()
  ...
  const total = mathMax(1, mathCeil(messageLength / maxDataBytes))
  if (total > profile.maxFragments) invalid()
  const frames = []
  let current = null
  try {
    for (let index = 0; index < total; index++) {
      const start = index * maxDataBytes
      const end = mathMin(start + maxDataBytes, messageLength)
...
class Reassembler {
  ...
  constructor(options) {
    ...
    this.#maxDataBytes = profile.maxDataBytes
    this.#maxFragments = profile.maxFragments
    this.#maxMessageBytes = positiveLimit(
      maxMessageBytes === undefined ? profile.maxMessageDataBytes : maxMessageBytes,
      profile.maxMessageDataBytes
    )
  ...
  #pushAuthenticated(value) {
    ...
    if (total === 0 || total > this.#maxFragments || index >= total) {
      if (beforeClock) this.#remove(beforeClock)
      invalid()
    }
    if (index < total - 1 && dataLength !== this.#maxDataBytes) {
      if (beforeClock) this.#remove(beforeClock)
      invalid()
    }
    ...
    const maximumBytes = (total - 1) * this.#maxDataBytes + dataLength
    if (
      total > mathCeil(this.#maxMessageBytes / this.#maxDataBytes) ||
