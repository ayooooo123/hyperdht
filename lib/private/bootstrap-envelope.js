...
const BOOTSTRAP_SIZE = 1200
const BOOTSTRAP_CLASS = 0x80
const BOOTSTRAP_HEADER_SIZE = 150
const BOOTSTRAP_SIGNATURE_SIZE = 64
const BOOTSTRAP_MAX_BODY = 986
const BOOTSTRAP_DEADLINE = 5_000
const DEFAULT_MAX_BOOTSTRAP_PENDING = 64
const DEFAULT_MAX_BOOTSTRAP_PENDING_PER_PEER = 8
const DEFAULT_MAX_BOOTSTRAP_CACHE = 64
const DEFAULT_MAX_BOOTSTRAP_CACHE_PER_PEER = 8
const DEFAULT_MAX_BOOTSTRAP_TOMBSTONES = 128
const DEFAULT_MAX_BOOTSTRAP_TOMBSTONES_PER_PEER = 16
const TEST_ONLY_BOOTSTRAP_REQUEST_TABLE_OBSERVER = Symbol(
...
function clearEnvelope(value) {
  if (!value) return
  clear(value.senderIdentity32)
  clear(value.recipientIdentity32)
  clear(value.grantDigest32)
  clear(value.requestDigest32)
  clear(value.body)
  clear(value.packetDigest32)
}
...
function parsePacket(
  ...
  const sender =
    expectedSender ||
    (state.link.dynamicPeer === true ? senderIdentity32 : state.link.peerIdentity32)
  ...
  let ownedSender = null
  let ownedRecipient = null
  let ownedGrant = null
  let ownedRequest = null
  let ownedBody = null
  let ownedPacketDigest = null
  let verifiedSender = null
  let verifiedRecipient = null
  let verifiedGrant = null
  let verifiedRequest = null
  let verifiedBody = null
  let verifiedPacketDigest = null
  let decoded = null
  let verified = null
  try {
    ownedPacketDigest = safeHash(state.crypto, packet)
    ownedSender = copy(senderIdentity32)
    ownedRecipient = copy(recipientIdentity32)
    ownedGrant = copy(grantDigest32)
    ownedRequest = copy(requestDigest32)
    ownedBody = copy(bodyView)
    decoded = {
      type,
      requestId,
      epoch,
      senderIdentity32: ownedSender,
      recipientIdentity32: ownedRecipient,
      grantDigest32: ownedGrant,
      requestDigest32: ownedRequest,
      body: ownedBody,
      packetDigest32: ownedPacketDigest
    }
    ownedSender = null
    ownedRecipient = null
    ownedGrant = null
    ownedRequest = null
    ownedBody = null
    ownedPacketDigest = null
    verifiedSender = copy(decoded.senderIdentity32)
    verifiedRecipient = copy(decoded.recipientIdentity32)
    verifiedGrant = copy(decoded.grantDigest32)
    verifiedRequest = copy(decoded.requestDigest32)
    verifiedBody = copy(decoded.body)
    verifiedPacketDigest = copy(decoded.packetDigest32)
    verified = {
      type,
      requestId,
      epoch,
      senderIdentity32: verifiedSender,
      recipientIdentity32: verifiedRecipient,
      grantDigest32: verifiedGrant,
      requestDigest32: verifiedRequest,
      body: verifiedBody,
      packetDigest32: verifiedPacketDigest
    }
    verifiedSender = null
    ...
    verifiedBody = null
    verifiedPacketDigest = null
    VERIFIED_ENVELOPES.set(decoded, verified)
    verified = null
    return Object.freeze(decoded)
  } catch (err) {
    clearEnvelope(decoded)
    clearEnvelope(verified)
    clear(ownedSender)
    clear(ownedRecipient)
    clear(ownedGrant)
    clear(ownedRequest)
    clear(ownedBody)
    clear(ownedPacketDigest)
    clear(verifiedSender)
    clear(verifiedRecipient)
    clear(verifiedGrant)
    clear(verifiedRequest)
    clear(verifiedBody)
    clear(verifiedPacketDigest)
    if (err instanceof PrivateRouteError) throw err
    invalidRoute()
  }
}
...
class BootstrapEnvelopeCodec {
  ...
  encode(value) {
    ...
    try {
      ...
      requireLiveLink(state)
      verifiedSender = copy(state.link.localIdentity32)
      verifiedRecipient = copy(state.link.peerIdentity32)
      verifiedGrant = copy(state.link.digest32)
      verifiedRequest = copy(requestDigest32)
      verifiedBody = copy(body)
      verifiedDigest = copy(packetDigest32)
      verified = {
        type,
        requestId,
        epoch,
      ...
      verifiedBody = null
      verifiedDigest = null
      VERIFIED_PACKETS.set(packet, verified)
      verified = null
      return packet
    } catch (err) {
      clear(packet)
  ...
  decode(packet, source) {
    ...
    try {
      ...
      if (state.link.dynamicPeer === true) {
        if (decoded.type !== BOOTSTRAP_TYPE.LINK_CREATE) unauthorized()
        const endpoint = require('./udx-cell-endpoint')
        const claim =
          endpoint[Symbol.for('hyperdht-private-routes/bootstrap-accept-handle-claimer')]
        claim(state.linkHandle, decoded.senderIdentity32, source)
...
function isBootstrapEnvelopeCodecForLink(codec, linkHandle) {
...
function tableHash(state, parts) {
  let digest32 = null
  try {
    digest32 = safeHash(state.crypto, parts)
    if (state.destroyed) unavailable()
    const result = digest32
    digest32 = null
    return result
  } catch {
    clear(digest32)
...
class BootstrapRequestTable {
  ...
  begin(value) {
    ...
    try {
      ...
      let digest32 = null
      let record = null
      let ownedPeerIdentity = null
      let ownedSenderIdentity = null
      let ownedRecipientIdentity = null
      let ownedGrantDigest = null
      let ownedBody = null
      let ownedPacket = null
      let ownedRecordDigest = null
      ...
      try {
        ...
        ownedPeerIdentity = copy(peerIdentity32)
        ownedSenderIdentity = copy(encoded.senderIdentity32)
        ownedRecipientIdentity = copy(encoded.recipientIdentity32)
        ownedGrantDigest = copy(encoded.grantDigest32)
        ownedBody = copy(encoded.body)
        ownedPacket = copy(packet)
        ownedRecordDigest = copy(digest32)
        record = {
          peerIdentity32: ownedPeerIdentity,
          peerKey: peer,
          senderIdentity32: ownedSenderIdentity,
          recipientIdentity32: ownedRecipientIdentity,
          grantDigest32: ownedGrantDigest,
          body: ownedBody,
          epoch,
          requestId,
          packet: ownedPacket,
          digest32: ownedRecordDigest,
          callback: onResponse,
          token,
          deadlineAt: deadlineAt(state, startedAt)
        }
        ownedPeerIdentity = null
        ownedSenderIdentity = null
        ownedRecipientIdentity = null
        ownedGrantDigest = null
        ownedBody = null
        ownedPacket = null
        ownedRecordDigest = null
        resultPacket = copy(packet)
        resultDigest = copy(digest32)
        const result = Object.freeze({
          token,
          requestId,
          packet: resultPacket,
          digest32: resultDigest
        })
        ...
        clearRecord(record)
        clear(ownedPeerIdentity)
        clear(ownedSenderIdentity)
        clear(ownedRecipientIdentity)
        clear(ownedGrantDigest)
        clear(ownedBody)
        clear(ownedPacket)
        clear(ownedRecordDigest)
        clear(resultPacket)
        clear(resultDigest)
        if (err instanceof PrivateRouteError) throw err
        unavailable()
  ...
  respond(peerIdentity32, value, requestPacket, createResponse) {
    ...
    try {
      ...
      let digest32 = null
      try {
        digest32 = tableHash(state, requestPacket)
        ...
        let ownedBody = null
        let ownedPacket = null
        let returnedPacket = null
        let record = null
        try {
          ownedPeerIdentity = copy(peerIdentity32)
          ownedSenderIdentity = copy(decoded.senderIdentity32)
          ownedRecipientIdentity = copy(decoded.recipientIdentity32)
          ownedGrantDigest = copy(decoded.grantDigest32)
          ownedBody = copy(decoded.body)
          ownedPacket = copy(packet)
          returnedPacket = copy(packet)
          record = {
            peerIdentity32: ownedPeerIdentity,
            peerKey: peer,
            senderIdentity32: ownedSenderIdentity,
          ...
          observeTable(state, record)
          record = null
          const resultPacket = returnedPacket
          returnedPacket = null
          return resultPacket
        } catch (err) {
          clearRecord(record)
...
module.exports = {
  ...
  DEFAULT_MAX_BOOTSTRAP_TOMBSTONES_PER_PEER,
  TEST_ONLY_BOOTSTRAP_REQUEST_TABLE_OBSERVER,
