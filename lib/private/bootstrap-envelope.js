...
const BOOTSTRAP_DEADLINE = 10_000
const BOOTSTRAP_TOMBSTONE_DEADLINE = 5_000
...
function deadlineAt(state, current, duration = state.deadline) {
  if (current > Number.MAX_SAFE_INTEGER - duration) {
    failTable(state)
    unavailable()
  }
...
function addTombstone(state, key, record) {
  ...
  try {
    ...
    tombstone = {
      ...
      packet: null,
      callback: null,
      deadlineAt: deadlineAt(state, current, BOOTSTRAP_TOMBSTONE_DEADLINE)
    }
    peerIdentity32 = null
    digest32 = null
  } catch {
