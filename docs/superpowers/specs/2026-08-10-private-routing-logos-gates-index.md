...
protocol). Tor/Veilid-class, _not_ a Nym/Logos mixnet: resists local and single-relay
...
1. **C (SURBs / private routes)** — near-term. Sender anonymity (safety routes) already
   exists via circuits; C completes _receiver_ anonymity for connectionless and private-peer
