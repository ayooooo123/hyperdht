...
**Status:** internal implementation prerequisite ratified by Main. No v2 runtime or public API is enabled; implementation, native privacy acceptance, and external human cryptographic/public-release gates remain separate.

The [ratification](#prerequisite-ratification), [transport specification](#transport-specification), and [semantic specification](#semantic-specification) below are current. The intervening review record preserves rejected drafts and earlier proof boundaries as history; it is superseded where the ratified specification differs.
...
Main accepts the exact transport and semantic snapshots below as the internal implementation prerequisite. This supersedes the historical rejected drafts above. It does **not** accept a v2 implementation, prove peer-stream privacy, enable public required mode, or substitute for named external human cryptographic review of the eventual exact source and native dependency revisions.
...
Frozen standalone input SHA-256 values (the headings and cross-file references below are adapted for this single maintained document):
...
| Memory ownership                 | Complete pending reservations precede OFFER/ACCEPT and transfer at sentinels. Disjoint ARQ, receive, canonical cache, endpoint records, contiguous legacy Noise scratch, and singleton legacy pre-OPEN ingress. No hidden lifetime OPEN/OPENED byte cache. |
...
Main's final integration additionally resolved the stale formula copies, offered-versus-admitted queue calculation, cookie-tombstone retention after early challenge expiry, reverse first-object validation, and settled reliable-byte ownership. No wire body width or public API was changed by these final accounting and state corrections.
...
- Native Sodium/Python confirmation vectors agree. Actual Node24/Sodium incremental KDF and receipt MAC also match, using a384-byte hash state and canonical slices, and reject the changed receipt nonce. These are synthetic transcript construction checks, not production decoder/state/privacy tests.
...
|       0308 | `PEER_LINK_OFFER_V2`                |      302 |     64 |      374 | physical adjacency setup                                                     |
|       0309 | `PEER_LINK_ACCEPT_V2`               |      213 |     64 |      285 | physical adjacency setup                                                     |
|       030a | `PEER_REDACTED_RESPONDER_PROOF_V2`  |      306 |     64 |      378 | nested extension proof                                                       |
...
`PEER_LINK_OFFER_V2` body302 is `advertisementDigest32 | initiatorIdentity32 | responderIdentity32 | initiatorRole u8 | responderRole u8 | branchClass u8=2 | branchId16 | circuitId16 | generation u64 | extensionIndex u8 (0..2) | initiatorLinkEphemeralPublicKey32 | clientTailEphemeralPublicKey32 | clientNonce32 | payloadParametersDigest32 | requestedLimits26 | offerDeadline u64`. It is accepted only by the exact active candidate/locator owner in CIRCUIT_BUILDING, before an adjacency exists. `PEER_LINK_ACCEPT_V2` body213 is `offerDigest32 | advertisementDigest32 | responderIdentity32 | observedPredecessorEndpoint19 | responderLinkEphemeralPublicKey32 | admittedLimits26 | acceptedAt u64 | acceptNonce32`; it is accepted only against one pending byte-identical OFFER and must be downward/equal on limits and deadline. Their64-byte signatures use the matching identities and link-offer/link-accept domains.
`PEER_REDACTED_RESPONDER_PROOF_V2` body306 is `responderAdvertisementDigest32 | initiatorIdentity32 | responderIdentity32 | branchClass u8=2 | branchId16 | circuitId16 | generation u64 | extensionIndex u8 | clientTailEphemeralPublicKey32 | clientNonce32 | advertisedRouteEncryptionPublicKey32 | admittedLimitsDigest32 | expiresAt u64 | responderProofNonce32`; it is accepted only nested in the matching EXTENDED and its64-byte responder signature is verified before retention. `PEER_EXTENDED_V2` body486 is `branchClass u8=2 | branchId16 | circuitId16 | generation u64 | extensionIndex u8 | responderAdvertisementDigest32 | proofLength u16=378 | completeProof378 | extensionNonce32`; only the source-side pending extension accepts it. `PEER_TAIL_READY_V2` body210 is `branchClass u8=2 | branchId16 | circuitId16 | generation u64 | extensionIndex u8 | tailControlTranscriptDigest32 | tailIdentity32 | tailAdvertisementDigest32 | clientNonce32 | readyNonce32 | expiresAt u64`; only the source that verified matching EXTENDED accepts its64-byte tail signature.
...
The nine core adjacency layouts `0308..0310` import the exact v1 field order. V2 scalar changes are exact: branchClass=2 PEER; envelope version2; IDs above; transcript protocol scalar2; literal `/v2` domains; advertisement fixed260. `PEER_EXTEND_REQUEST_V2` uses the v1-shaped 458-byte body, then appends `currentTailForwardLimits26` and `candidateAuthorityCommitment32`, making516. Its existing `requestedLimits26` is the successor's reverse-send partition. The current tail reserves its appended forward partition and consumes the one-use candidate authority before successor LINK_OFFER; the successor authenticates/adopts the existing requested partition in LINK_ACCEPT. No v1 validator, enum, constant, or mask is widened.
...
REJECT body64 is `routeId16 | generation u64 | purpose u8 | reserved3=0 | offerNonce16 | reason u16 | reserved u16=0 | rejectNonce16`; suffix uses `preTerminalMacKey16`. Before valid OFFER authentication terminal sends nothing. Exact OFFER replay returns cached exact ACCEPT/REJECT under a fresh finalize counter and physical charge; same routeId/offerNonce with changed bytes destroys pending state.
...
| purpose2 private source application      |          `0360/757` |       `4/1297` = ACTIVATE757 + IK1 fragment172 + ACK240 + SOURCE_RECEIPT128 |     `4/752` = READY228 + IK2 fragment124 + ACCEPTED176 + PRIVATE_OPEN224 |
| purpose3 private destination application |          `0360/757` |         `4/1393` = ACTIVATE757 + IK1 fragment172 + ACK240 + PRIVATE_OPEN224 |                       `3/528` = READY228 + IK2 fragment124 + ACCEPTED176 |
...
`requiredSemanticOwnedBytes = retainedCanonicalAndResponseCacheMaximum + 33554439*localEndpointApplicationStreamCapacity + (localOwnerIsLegacyEgress ? 16777280 : 0) + legacyNoiseScratchBytes`.

The exact owner floors are purpose1 source `33572251*M`, purpose1 egress `17812*M+16777280`, purpose2 source `33556488*M`, purpose2 entry `2049*M`, purpose3 listener `726+33556360*(M-1)`, and purpose3 entry `726+1921*(M-1)`. These endpoint-side floors still bound purpose1/2 to `M<=127` and purpose3 to `M<=128` before smaller advertisement, partition, or storage limits. The endpoint-record term owns one pending outbound maximum record, one inbound assembly-or-retained-plaintext maximum record, and its3-byte guard per endpoint application stream. Entry owns no endpoint record buffer.
...
| purpose3 private destination application leg |            4 |            5 |                       0/2 | READY,IK2,ACCEPTED,OPENED forward; incoming OPEN,ACTIVATE,IK1,ACK,PRIVATE_OPEN reverse   |
...
9. Purpose3 incoming streams use terminal-even IDs. Entry→destination OPEN/ACTIVATE/IK1/ACK/PRIVATE_OPEN and application traffic are physical REVERSE; destination→entry READY/IK2/ACCEPTED/OPENED and replies are physical FORWARD.
...
| purpose2 private source                  |             `0360/757` |                       `4/1297` = ACTIVATE757 + IK1 172 + ACK240 + RECEIPT128 |              `4/752` = READY228 + IK2 124 + ACCEPTED176 + PRIVATE_OPEN224 |                             `2,049` |
| purpose3 private destination application |             `0360/757` |                  `4/1393` = ACTIVATE757 + IK1 172 + ACK240 + PRIVATE_OPEN224 |                                `3/528` = READY228 + IK2 124 + ACCEPTED176 |                             `1,921` |
...
The resulting exact local minima are: purpose1 source endpoint `(33,554,439+9,620+8,192)*M = 33,572,251*M`; purpose1 legacy egress `(9,620+8,192)*M+16,777,280 = 17,812*M+16,777,280`; purpose2 source endpoint `33,556,488*M`; purpose2 private entry bridge `2,049*M`; purpose3 listener/destination `726+33,556,360*(M-1)`; and purpose3 entry bridge `726+1,921*(M-1)`. The u32 field and checked arithmetic reject overflow before OFFER/ACCEPT. In particular, the endpoint-side formulas bound purpose1 and purpose2 to `M<=127` and purpose3 to `M<=128`; a smaller advertisement, local reservation, or authenticated partition still wins.

The semantic-owned pool has disjoint, non-borrowing subledgers for canonical/response caches, legacy contiguous-Noise scratch, each endpoint record slot, and the legacy pre-OPEN ingress slot. Transport receive/reorder windows and reliable buffers never spend this pool. Conversely, both local route owners reserve their accepted inbound outer-direction `admittedReceiveFrames` and `admittedReceiveBytes` in transport memory even though the negotiated fields are symmetric maxima; they are two physical reservations, not one shared pool. A private entry bridge reserves the accepted inbound window independently for each leg and cannot reuse either window for its other leg or direction.
...
- Semantic-only reverse: READY, IK2 fragment, ACCEPTED, PRIVATE_OPEN, `PEER_OPENED` = 5.
...
- Semantic-only forward: READY, IK2 fragment, ACCEPTED, `PEER_OPENED` = 4.
