...
const counterInternals = require('../../lib/private/counters')
const { DatagramReplayWindow, MAX_COUNTER, OrderedReceiver, ROTATE_AT, SenderCounter } =
  counterInternals
...
test('ordered receiver rejects duplicate and below-next counters after reordered delivery', (t) => {
  const receiver = new OrderedReceiver({ window: 4, gapTimeout: 50, now: () => 0 })

  t.alike(receiver.pushAuthenticated(2n, 'two'), [])
  t.alike(receiver.pushAuthenticated(1n, 'one'), [])
  expectCode(t, () => receiver.pushAuthenticated(1n, 'duplicate'), 'REPLAY')
  t.is(receiver.closed, false)
  t.alike(receiver.pushAuthenticated(0n, 'zero'), ['zero', 'one', 'two'])
  expectCode(t, () => receiver.pushAuthenticated(0n, 'below-next'), 'REPLAY')
})

test('datagram 64-counter window has an inclusive floor and rejects stale counters', (t) => {
  const receiver = new DatagramReplayWindow({ window: 64 })

  t.is(receiver.acceptAuthenticated(63n), true)
  t.is(receiver.floor, 0n)
  t.is(receiver.acceptAuthenticated(0n), true)
  expectCode(t, () => receiver.acceptAuthenticated(0n), 'REPLAY')
  t.is(receiver.acceptAuthenticated(64n), true)
  t.is(receiver.floor, 1n)
  t.is(receiver.acceptAuthenticated(1n), true)
  expectCode(t, () => receiver.acceptAuthenticated(0n), 'REPLAY')
  expectCode(t, () => receiver.acceptAuthenticated(63n), 'REPLAY')
})
