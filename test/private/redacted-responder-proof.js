...
test('successor proof assembly delegates the exact body to the two-phase responder signer', (t) => {
  const { identity, value } = fixture()
  const owner = createRelayIdentitySigningAuthority({ identitySecretKey: identity.secretKey })
  const signer = createExtensionResponderSigner(owner)
  const acceptBody = seed(0x5a, 213)
  signLinkAccept(signer, acceptBody, identity.publicKey)

  const encoded = signRedactedResponderProofWithSigner(value, signer)
  const object = decodeM3Object(encoded)
  const body = expectedBody(value)

  t.is(encoded.byteLength, REDACTED_RESPONDER_PROOF_SIZE)
  t.is(object.body.byteLength, 306)
  t.alike(object.body, body, 'signer receives the unchanged registry body')
  t.alike(
    encoded,
    signRedactedResponderProof(value, identity.secretKey),
    'signer-capability assembly preserves every proof wire byte'
  )
  t.ok(
    cryptoSuite.verify(expectedSignatureInput(body), object.authSuffix, identity.publicKey),
    'proof is signed by the responder signer identity'
  )
  t.ok(destroyExtensionResponderSigner(signer))
  t.ok(destroyRelayIdentitySigningAuthority(owner))
})

test('successor proof assembly has no raw-key fallback for the wrong responder signer', (t) => {
  const { identity, value } = fixture()
  const wrongIdentity = cryptoSuite.keyPair(seed(0x22))
  const owner = createRelayIdentitySigningAuthority({
    identitySecretKey: wrongIdentity.secretKey
  })
  const signer = createExtensionResponderSigner(owner)
  signLinkAccept(signer, seed(0x5b, 213), wrongIdentity.publicKey)

  expectCode(
    t,
    () => signRedactedResponderProofWithSigner(value, signer),
    'ERR_AUTHENTICATION',
    'the value responder identity must equal the signer-bound identity'
  )
  t.ok(destroyExtensionResponderSigner(signer))
  t.ok(destroyRelayIdentitySigningAuthority(owner))
  t.alike(identity.secretKey.subarray(0, 32), seed(0x21), 'caller identity material is untouched')
})
