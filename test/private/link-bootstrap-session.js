...
test('LinkBootstrapSession initiates in-engine punching during link open', async (t) => {
  const punches = []
  const time = clock(0)
  const authority = cryptoSuite.keyPair(seed(250))
  const initiator = cryptoSuite.keyPair(seed(251))
  ...
  const customEndpoint = (side) => ({
    send(handle, packet, options = {}) {
      if (side === 'left') {
        const owned = b4a.from(packet)
        queueMicrotask(() => void rightSession.receive(owned))
      } else {
        const owned = b4a.from(packet)
        queueMicrotask(() => void leftSession.receive(owned))
      }
      return Promise.resolve(true)
    },
    punch(target, options = {}) {
      punches.push({ side, target, options })
      return Promise.resolve({ ok: true, sent: options.rounds || 6, rounds: options.rounds || 6 })
    },
    [UDX_LINK_OPEN]() {},
    [LINK_BOOTSTRAP_REGISTER_ESTABLISHED]() {},
    [UDX_LINK_CLOSE]() {}
  ...
  const leftSession = new LinkBootstrapSession({
    now: time.now,
    schedule: time.schedule,
    cancel: time.cancel,
    absoluteDeadline: 10_000,
    ...
    linkSetup: createLinkSetupAuthority({ now: time.now, randomBytes: sequence(0x61) }),
    setup: {
      circuitId: seed(256).subarray(0, 16),
      epoch: 9n,
      initiatorIdentity: initiator.publicKey,
      responderIdentity: responder.publicKey,
      initiatorLocalId: seed(257).subarray(0, 16),
      responderLocalId: seed(258).subarray(0, 16),
      expiresAt: 60_000n,
      responderStaticKey: responderStatic.publicKey,
      initiatorIdentitySecretKey: initiator.secretKey
    },
  ...
  t.ok(established, 'session opened successfully')
  t.ok(punches.length >= 1, 'in-engine punching was initiated')
  t.is(punches[0].side, 'left')
  t.is(punches[0].options.rounds, 4)
  t.is(punches[0].options.interval, 250)
